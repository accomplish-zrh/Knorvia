...
``knorvia skills login`` opens the hub's GitHub/Google authorization page in
...
_SUCCESS_HTML = (
    ...
    "<h2>✓ 已登录 ClawHub</h2><p>令牌已交给命令行，可以关闭此页面返回终端。</p></body>"
)
_ERROR_HTML = (
...
def hub_origin_from_base(base_url: str) -> str:
    ...
    ``https://clawhub.ai/api/v1`` -> ``https://clawhub.ai``.
    """
    b = base_url.rstrip("/")
