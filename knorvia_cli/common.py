...
def _configure_windows_utf8_streams() -> None:
    """Use UTF-8 for redirected Windows CLI streams.

    Interactive Windows terminals are handled by Rich/Win32. Redirected
    streams otherwise inherit a legacy code page, which corrupts Chinese and
    symbols in logs, pipes and desktop IPC captures.
    """
...
# lives at module scope so a single ``knorvia chat`` session shares one
# ring across turns; ``knorvia run`` doesn't read it (single-shot mode),
...
async def stream_turn_as_json(*, app: KnorviaApp, turn_id: str) -> None:
...
async def render_turn_stream(*, app: KnorviaApp, turn_id: str) -> str:
...
class TurnStreamRenderer:
    ...
    async def _on_wait_for_input(self, item: dict[str, Any]) -> None:
        ...
        from knorvia.core.stream_bus import get_bus
...
def _resolve_answer(raw: str, labels: list[str], multi: bool) -> str:
    ...
    for token in tokens:
        ...
        if token.isdecimal() and labels and 1 <= int(token) <= len(labels):
