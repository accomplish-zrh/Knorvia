...
``knorvia init`` walks the user through a four-step wizard (ports → LLM →
...
review panel) lives in :mod:`knorvia_cli.init_wizard`. This module is
...
from knorvia.runtime.home import KNORVIA_HOME_ENV, get_runtime_home

from . import init_wizard as wiz


def _reset_runtime_singletons() -> None:
    """Drop cached service instances so the new KNORVIA_HOME takes effect.

    ``knorvia init`` may pass ``--home`` to target a different workspace; the
...
def _embedding_step(
    ...
    from knorvia.services.config.embedding_endpoint import (
        EMBEDDING_PROVIDER_LABELS,
        normalize_embedding_endpoint_for_display,
        redact_embedding_endpoint_for_display,
    )
    from knorvia.services.config.provider_runtime import EMBEDDING_PROVIDERS
...
def _prompt_port(label: str, *, default: int) -> int:
    """Prompt for a TCP port, tolerating non-numeric / out-of-range input.

    ``int(typer.prompt(...))`` on garbage input used to raise a raw ValueError
    ...
    while True:
        ...
        try:
            ...
            typer.echo("Invalid port: enter a number between 1 and 65535.")
            continue
        if 1 <= value <= 65535:
            return value
        typer.echo("Invalid port: enter a number between 1 and 65535.")
...
def run_init(*, cli_only: bool = False, home: str | Path | None = None) -> None:
    ...
    from knorvia.services.config import get_model_catalog_service, get_runtime_settings_service
    ...
    try:
        ...
        if not cli_only:
            ...
            system["backend_port"] = _prompt_port(
                strings["init.backend_port"],
                default=int(system.get("backend_port") or 8001),
            )
            system["frontend_port"] = _prompt_port(
                strings["init.frontend_port"],
                default=int(system.get("frontend_port") or 3782),
            )
