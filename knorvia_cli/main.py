"""CLI entry point for the standalone ``knorvia-cli`` package."""
...
from knorvia.runtime.mode import RunMode, set_mode
...
app = typer.Typer(
    name="knorvia",
    help="Knorvia CLI – agent-first interface for capabilities, tools, and knowledge.",
...
app.add_typer(skill_app, name="skills")  # alias: `knorvia skills …`
...
def run_capability(
    ...
    from knorvia.app import KnorviaApp
    ...
    maybe_run(run_turn_and_render(app=KnorviaApp(), request=request, fmt=fmt))
...
def start(
    ...
    host: str = typer.Option(
        "127.0.0.1",
        "--host",
        help=("Bind address. Non-loopback addresses are refused unless authentication is enabled."),
    ),
) -> None:
    """Launch backend + frontend together. Source installs default to production."""
    from knorvia.runtime.launcher import start as start_web

    start_web(home=home, dev=dev, host=host)
...
def serve(
    host: str = typer.Option(
        "127.0.0.1",
        help="Bind address. Defaults to loopback — pass 0.0.0.0 to expose the "
        "API (which can execute capabilities and reach knowledge bases) to the "
        "network; enable authentication first.",
    ),
    port: int | None = typer.Option(None, help="Port number."),
    reload: bool = typer.Option(False, help="Enable auto-reload for development."),
) -> None:
    """Start the Knorvia API server."""
    import asyncio
    import sys

    set_mode(RunMode.SERVER)
    from knorvia.runtime.launcher import _is_loopback_host
    from knorvia.services.config import load_auth_settings

    if not _is_loopback_host(host) and not bool(load_auth_settings()["enabled"]):
        console.print(
            "[bold red]Error:[/] Refusing a non-loopback bind while authentication "
            "is disabled. Enable authentication or use --host 127.0.0.1."
        )
        raise typer.Exit(code=2)
    if port is None:
        from knorvia.services.setup import get_backend_port
    ...
    try:
        ...
        console.print(
            ...
            "Run: pip install -U knorvia"
        )
        raise typer.Exit(code=1)

    from knorvia.services.config import HTTP_KEEP_ALIVE_TIMEOUT, get_ws_max_size
