...
async def lifespan(app: FastAPI):
    ...
    try:
        from knorvia.services.llm import get_llm_client
    ...
    try:
        from knorvia.events.event_bus import get_event_bus
    ...
    try:
        from knorvia.services.cron import get_cron_service
    ...
    try:
        from knorvia.services.memory import (
    ...
    try:
        from knorvia.services.image_studio.engine import shutdown_image_studio_runners

        await shutdown_image_studio_runners()
        logger.info("Image Studio workers stopped")
    except Exception as e:
        logger.warning(f"Failed to stop Image Studio workers: {e}")

    try:
        from knorvia.services.video_studio.engine import shutdown_video_studio_runners

        await shutdown_video_studio_runners()
        logger.info("Video Studio workers stopped")
    except Exception as e:
        logger.warning(f"Failed to stop Video Studio workers: {e}")
    ...
    try:
        from knorvia.services.llm.provider_factory import close_runtime_provider_pool
...
app = FastAPI(
    ...
    # Keep this route compatible with encoded path segments from Web clients.
...
# ones worth seeing.
#
# The `knorvia.access` logger gets its own INFO stdout handler rather than
...
_access_logger = logging.getLogger("knorvia.access")
if not any(getattr(h, "_knorvia_access_handler", False) for h in _access_logger.handlers):
    ...
    _access_handler._knorvia_access_handler = True  # type: ignore[attr-defined]
...
from knorvia.multi_user.router import router as multi_user_router
...
from knorvia.api.routers.auth import require_admin, require_auth
...
app.include_router(
    creative_library.router,
    prefix="/api/v1/library",
    tags=["creative-library"],
    dependencies=_auth,
)
app.include_router(
...
@app.get("/")
async def root():
    return {"message": "Welcome to Knorvia API"}


if __name__ == "__main__":
    from knorvia.api.run_server import main as run_server_main
