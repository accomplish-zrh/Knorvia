...
def _discover_plugins() -> list[Any]:
    try:
        from knorvia.plugins.loader import discover_plugins
    except Exception:
...
async def _execute_stream(tool_name: str, params: dict[str, Any]) -> AsyncGenerator[str, None]:
    ...
    stdout_stream = _QueueTextStream(
        event_queue, loop, stream=None, logger_name="knorvia.playground.stdout"
    )
    stderr_stream = _QueueTextStream(
        event_queue, loop, stream=None, logger_name="knorvia.playground.stderr"
    )
...
async def _execute_capability_stream(
    ...
    if capability_name == "chat" and partner_id:
        from knorvia.api.routers.partners import (
            ChatMessageRequest,
