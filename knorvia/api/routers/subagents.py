...
(Claude Code, Codex, Gemini CLI, Kimi CLI, opencode, MiMo Code, Grok Build) are installed on
...
from knorvia.multi_user.knowledge_access import current_kb_manager
from knorvia.multi_user.partner_access import assert_partner_allowed, visible_partner_cards
...
def _require_local_cli_admin(kind: str) -> None:
    if kind != PARTNER_BACKEND_KIND and not get_current_user().is_admin:
        raise HTTPException(
            status_code=403,
            detail="Local CLI subagents are restricted to administrators.",
        )


def _require_backend_enabled(kind: str) -> None:
    if not load_subagent_settings().backend(kind).enabled:
        raise HTTPException(status_code=409, detail=f"Subagent backend {kind!r} is disabled.")
...
async def backend_options():
    ...
    from knorvia.services.subagent.models import list_backend_options
...
async def sync_backend(kind: str):
    ...
    from knorvia.services.subagent.models import sync_backend_options
...
async def create_connection(payload: ConnectSubagentRequest):
    ...
    consult within one Knorvia chat lands in that one partner session.
    """
    ...
    _require_local_cli_admin(agent_kind)
    _require_backend_enabled(agent_kind)
    ...
    if agent_kind == PARTNER_BACKEND_KIND:
        ...
        from knorvia.services.subagent.process import validate_subagent_cwd

        try:
            resolved_cwd = validate_subagent_cwd(payload.cwd)
...
async def delete_connection(name: str):
    ...
    from knorvia.services.subagent.sessions import forget_connection
...
async def message_connection(name: str, payload: SubagentMessageRequest):
    ...
    live session Knorvia consults (shared via the cross-turn registry, keyed by
    ...
    if backend is None:
        raise HTTPException(status_code=400, detail=f"Unknown agent kind: {kind!r}")
    _require_local_cli_admin(kind)
    _require_backend_enabled(kind)
    if kind != PARTNER_BACKEND_KIND:
        from knorvia.services.subagent.process import validate_subagent_cwd

        try:
            cwd = validate_subagent_cwd(cwd)
        except ValueError as exc:
            raise HTTPException(status_code=400, detail=str(exc)) from exc
    ...
    async def event_stream():
        ...
        try:
            ...
            if pending:
                logger.warning("Detached a subagent message task that ignored cancellation")
