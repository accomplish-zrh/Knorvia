...
from knorvia.multi_user.model_access import allowed_llm_options
from knorvia.services.codebuddy_auth import get_codebuddy_auth_service
from knorvia.services.codex_auth import (
    CodexAuthError,
...
from knorvia.services.config.origins import normalize_origins
from knorvia.services.config.runtime_settings import (
    CHAT_ATTACHMENT_CHARS_RANGE,
    CHAT_ATTACHMENT_MAX_FILE_MB_RANGE,
    CHAT_ATTACHMENT_MAX_TOTAL_MB_RANGE,
    compute_ws_max_size,
)
from knorvia.services.embedding.client import reset_embedding_client
from knorvia.services.llm.client import reset_llm_client
from knorvia.services.llm.config import clear_llm_config_cache
...
from knorvia.services.settings.interface_settings import resolve_languages
from knorvia.tools.builtin import USER_TOGGLEABLE_TOOL_NAMES
...
DEFAULT_UI_SETTINGS = {
    ...
    **INTERFACE_DEFAULTS,
    "sidebar_description": "Knorvia AI learning and knowledge workspace",
...
def _require_codex_oauth_actor() -> None:
    ...
    """
    from knorvia.services.partners.scope import is_partner_user_id
...
def _provider_choices() -> dict[str, list[dict[str, Any]]]:
    ...
    from knorvia.services.provider_registry import PROVIDERS
...
def _mineru_settings_payload() -> dict[str, Any]:
    ...
    """
    from knorvia.services.parsing.engines.mineru.backend import local_cli_probe
...
def _document_parsing_payload() -> dict[str, Any]:
    ...
    from knorvia.services.parsing.engines._install import (
        installable_engines,
        model_downloadable_engines,
    )
    from knorvia.services.parsing.engines.factory import (
        get_parser,
        list_engines,
    )
    from knorvia.services.parsing.engines.mineru.backend import local_cli_probe
...
async def test_document_parsing(payload: DocumentParsingTest):
    ...
    _require_settings_admin()
    from knorvia.services.parsing.engines.factory import get_parser, is_engine_available
    ...
    if not cmd:
        return {
            ...
            "message": (
                ...
                f"(pip install knorvia[parse-{engine}]) so its CLI is on PATH."
            ),
        }
...
async def document_parsing_job_status(cursor: int = 0):
    _require_settings_admin()
    from knorvia.services.parsing.engines._install import get_background_job_manager
...
async def start_mineru_models_download(payload: MinerUModelDownloadPayload):
    ...
    """
    _require_settings_admin()
    from knorvia.services.parsing.engines.mineru.models import (
...
async def mineru_models_download_status(cursor: int = 0):
    _require_settings_admin()
    from knorvia.services.parsing.engines.mineru.models import get_model_download_manager
...
async def test_mineru_connection(payload: MinerUSettingsUpdate):
    ...
    _require_settings_admin()
    from knorvia.services.parsing.engines.mineru.cloud import verify_credentials
    from knorvia.services.parsing.engines.mineru.config import MinerUConfig, MinerUError

    if payload.mode == "local":
        from knorvia.services.parsing.engines.mineru.backend import (
            local_cli_probe,
            local_cli_version,
        )
...
async def fetch_models_from_provider(payload: FetchModelsPayload):
    ...
    """
    _require_settings_admin()
    from knorvia.services.llm.factory import fetch_models as fetch_llm_models
...
async def complete_tour(payload: TourCompletePayload | None = None):
    ...
    return {
        "status": "completed",
        "message": "Configuration saved. Knorvia will restart shortly.",
        "launch_at": launch_at,
