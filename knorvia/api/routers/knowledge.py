...
from knorvia.api.upload_safety import (
    get_upload_file_size as _get_upload_file_size,
)
from knorvia.api.upload_safety import (
    safe_join_raw as _safe_join_raw,
)
from knorvia.api.upload_safety import (
    sanitize_rel_subdir as _sanitize_rel_subdir,
)
from knorvia.api.upload_safety import (
    validate_upload_batch as _validate_upload_batch,
)
from knorvia.api.utils.progress_broadcaster import ProgressBroadcaster
from knorvia.api.utils.task_id_manager import TaskIDManager
from knorvia.api.utils.task_log_stream import capture_task_logs, get_task_stream_manager
from knorvia.knowledge.add_documents import DocumentAdder, remove_raw_document
from knorvia.knowledge.initializer import KnowledgeBaseInitializer
from knorvia.knowledge.kb_types import is_connected_kb, supports_local_raw_files
from knorvia.knowledge.manager import KnowledgeBaseManager
from knorvia.knowledge.naming import validate_knowledge_base_name
from knorvia.knowledge.progress_tracker import ProgressStage, ProgressTracker
from knorvia.multi_user.context import get_current_user
from knorvia.multi_user.knowledge_access import (
    assert_writable,
    current_kb_base_dir,
    current_kb_manager,
...
from knorvia.services.config import PROJECT_ROOT, load_config_with_main
from knorvia.services.file_io import atomic_write_json
from knorvia.services.rag.factory import (
    DEFAULT_PROVIDER,
    GRAPHRAG_PROVIDER,
    LIGHTRAG_PROVIDER,
    PAGEINDEX_PROVIDER,
...
from knorvia.services.rag.file_routing import FileTypeRouter
from knorvia.services.rag.linked_kb import (
    LINKABLE_PROVIDERS,
    assert_path_allowed,
    probe_linked_folder,
)
from knorvia.utils.document_extractor import (
...
def _save_zip_archive(
    ...
    from knorvia.utils.archive_extractor import ArchiveTooLargeError, safe_extract_zip
...
def _save_uploaded_files(
    ...
    from knorvia.services.pocketbase_client import is_pocketbase_enabled
...
def _upload_file_to_pb(kb_name: str, filename: str, file_path: Path) -> None:
    ...
    try:
        from knorvia.services.pocketbase_client import get_pb_client
...
def _assert_provider_ready(provider: str) -> None:
    ...
    if provider == PAGEINDEX_PROVIDER:
        from knorvia.services.rag.pipelines.pageindex.config import is_pageindex_configured
    ...
    if provider == GRAPHRAG_PROVIDER:
        from knorvia.services.rag.pipelines.graphrag.config import is_graphrag_available
        ...
        if not is_graphrag_available():
            raise HTTPException(
                ...
                detail=(
                    ...
                    "`pip install 'knorvia[graphrag]'` on the server before "
                    "creating a GraphRAG knowledge base."
                ),
            )

    if provider == LIGHTRAG_PROVIDER:
        from knorvia.services.rag.pipelines.lightrag.config import is_lightrag_available
        ...
        if not is_lightrag_available():
            raise HTTPException(
                ...
                detail=(
                    ...
                    "`pip install 'knorvia[rag-lightrag]'` on the server before "
...
def _enforce_provider_formats(provider: str, files: list[UploadFile]) -> None:
    ...
    if provider != PAGEINDEX_PROVIDER:
        return
    from knorvia.services.rag.pipelines.pageindex.pipeline import SUPPORTED_EXTENSIONS
...
def _matching_index_is_valid(kb_name: str, matching_version: dict | None) -> bool:
    ...
    if not matching_version:
        return False
    try:
        from knorvia.services.rag.index_probe import inspect_provider_version
        from knorvia.services.rag.pipelines.llamaindex.storage import (
...
async def get_rag_providers():
    ...
    try:
        from knorvia.services.config import get_kb_config_service
...
def _pageindex_config_payload() -> dict:
    ...
    from knorvia.services.config import get_runtime_settings_service
...
async def update_pageindex_pipeline_config(payload: PageIndexConfigUpdate):
    ...
    try:
        from knorvia.services.config import get_runtime_settings_service
        from knorvia.services.rag.pipelines.pageindex.config import DEFAULT_API_BASE_URL
        ...
        try:
            from knorvia.services.mcp import get_mcp_manager
...
async def get_rag_pipeline_preflight(provider: str):
    ...
    """
    try:
        from knorvia.services.rag.preflight import engine_preflight
...
async def update_kb_config(kb_name: str, config: dict):
    ...
    try:
        from knorvia.services.config import get_kb_config_service
        from knorvia.services.rag.index_probe import has_ready_provider_index
...
async def probe_lightrag_server_route(payload: ProbeLightRagServerRequest):
    ...
    """
    from knorvia.services.rag.pipelines.lightrag_server.probe import probe_server
...
async def connect_lightrag_server_route(payload: ConnectLightRagServerRequest):
    ...
    """
    from knorvia.services.rag.pipelines.lightrag_server.config import SUPPORTED_MODES
    from knorvia.services.rag.pipelines.lightrag_server.probe import probe_server
...
async def probe_ima_route(payload: ProbeImaRequest):
    ...
    Creates nothing.
    """
    from knorvia.services.rag.pipelines.ima.probe import probe_knowledge_base
...
def _resolve_kb_raw_dir(kb_name: str, *, allow_unsupported: bool = False) -> Path | None:
    ...
    Knorvia-managed raw directory.  File listing treats that as an empty
...
async def run_reindex_task(kb_name: str, base_dir: str, task_id: str, signature_hash: str) -> None:
    ...
    with capture_task_logs(task_id):
        try:
            ...
            # provider=None → RAGService resolves the KB's Knorvia-bound
...
async def reindex_knowledge_base(
    ...
    try:
        ...
        if provider_uses_embedding_versions(kb_provider):
            from knorvia.services.rag.embedding_signature import signature_from_embedding_config
            from knorvia.services.rag.index_versioning import (
