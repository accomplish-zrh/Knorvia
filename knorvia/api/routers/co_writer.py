...
from knorvia.co_writer.edit_agent import (
    EditAgent,
    append_history,
    load_history,
    print_stats,
...
def _module_config() -> dict:
    """Load ``main.yaml`` lazily, never at import time.

    Importing this module must not perform file I/O or raise: a bare import
    (tests, health checks, tooling) runs in processes whose working directory
    / KNORVIA_HOME does not own a settings tree, and an import-time
    ``FileNotFoundError`` there used to break the whole router.
    """
    try:
        return load_config_with_main("main.yaml")
