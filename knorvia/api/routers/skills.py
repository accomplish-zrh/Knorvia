...
from knorvia.multi_user.skill_access import (
...
class InstallSkillRequest(BaseModel):
    ...
    defaults to ``clawhub``). The web "import from ClawHub" flow always builds an
    ``clawhub:`` ref so a spoofed bridge message can't redirect the install to an
    arbitrary registry.
    """
...
# ── hub browse (in-app ClawHub skill browser; declared before `/{name}`) ──


@router.get("/hub/catalog")
async def hub_catalog(hub: str = "clawhub", q: str = "", limit: int = 50) -> dict[str, object]:
    """Proxy a skill hub's public catalog for the in-app browser.

    The web "Import from ClawHub" panel renders these rows in Knorvia's own
    ...
    the panel can offer a "view on ClawHub" link out.
    """
    import asyncio

    from knorvia.services.skill.hub import ClawHubProvider, HubError, get_hub_provider
...
async def hub_detail(slug: str, hub: str = "clawhub") -> dict[str, object]:
...
async def install_skill(payload: InstallSkillRequest) -> dict[str, object]:
    """Import a hub skill (e.g. from ClawHub) into the caller's own skill layer.
    ...
    lives in :func:`knorvia.services.skill.hub.install_from_hub`.
    """
    import asyncio
