...
from knorvia.core.i18n import t
from knorvia.partners.config.paths import get_partner_media_dir
from knorvia.partners.helpers import safe_filename
from knorvia.services.partners import (
    get_partner_manager,
    slugify_partner_id,
    slugify_soul_id,
)
...
from knorvia.services.partners.workspace import (
    list_assets,
    provision_assets,
    read_soul,
    remove_asset,
...
def _validate_channels_payload(channels: dict) -> None:
    ...
    from knorvia.partners.config.schema import ChannelsConfig
...
def _validate_llm_selection_payload(
    ...
    from knorvia.services.partners.model_runtime import normalize_partner_llm_selection
...
def _resolve_soul_content(soul: SoulSpec | None) -> tuple[str, dict[str, str]]:
    ...
    from knorvia.services.partners.workspace import DEFAULT_SOUL
...
# ── Group chat rooms (CLI-backed members, room-scoped sessions) ──
...
async def create_group_room(payload: RoomCreateRequest):
    """Rooms start empty; members (CLI-backed) are seated afterwards."""
    from knorvia.services.partners.group_chat import get_group_room_engine

    room = get_group_room_engine().create_room(payload.name)
    return {"room": {"id": room.id, "name": room.name, "members": []}}
...
async def add_group_member(room_id: str, payload: RoomMemberAddRequest):
    """Seat one CLI-backed agent (max 6 per room)."""
    from knorvia.services.partners.group_chat import get_group_room_engine

    try:
        room = get_group_room_engine().add_member(
            room_id,
...
@router.patch("/groups/{room_id}/member")
async def update_group_member(room_id: str, payload: RoomMemberUpdateRequest):
    """Room-local rename / identity word — the shared connection is untouched."""
    from knorvia.services.partners.group_chat import get_group_room_engine

    try:
        room = get_group_room_engine().update_member(
            room_id,
            payload.connection,
            display_name=payload.display_name,
            persona=payload.persona,
...
@router.delete("/groups/{room_id}/members/{connection}")
async def remove_group_member(room_id: str, connection: str):
    from knorvia.services.partners.group_chat import get_group_room_engine
...
async def say_in_group(room_id: str, payload: RoomMessageRequest):
    """User speaks into the room; members answer in turn (or @named ones only)."""
    from knorvia.services.partners.group_chat import get_group_room_engine
...
async def list_channel_schemas():
    ...
    from knorvia.api.routers._partners_channel_schema import all_channel_schemas
...
async def tool_options():
    ...
    """
    from knorvia.api.utils.tool_options import build_tool_options
...
async def start_partner(partner_id: str):
    ...
    # comes back in this state after a Knorvia restart (a manual /stop clears
...
async def partner_command_palette():
    from knorvia.services.partners.commands import partner_command_palette
...
def _partner_upload_caps() -> tuple[int, int]:
    try:
        from knorvia.services.config.runtime_settings import get_chat_attachment_limits
