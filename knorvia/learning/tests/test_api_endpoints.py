...
from knorvia.api.routers.mastery_path import router
from knorvia.learning.storage import LearningStore
...
def app(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        "knorvia.api.routers.mastery_path.LearningStore",
...
class TestGenerateFromNotebook:
    ...
    @patch("knorvia.services.llm.complete", new_callable=AsyncMock)
    ...
    @patch("knorvia.api.routers.mastery_path.get_response_language", return_value="en")
    @patch("knorvia.services.llm.complete", new_callable=AsyncMock)
    ...
    @patch("knorvia.api.routers.mastery_path.get_response_language", return_value="zh")
    @patch("knorvia.services.llm.complete", new_callable=AsyncMock)
