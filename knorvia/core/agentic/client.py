...
from knorvia.services.llm.reasoning_params import (
...
def _close_client_now(client: Any) -> None:
    """Best-effort synchronous close for the dead-loop fallback.

    Prefers ``close()``. An async-only client's ``aclose()`` is run to
    completion on a throwaway loop — closing transports does not depend on the
    (now dead) original loop, and no loop runs in this thread at that point.
    """
...
def _schedule_client_close(client: Any, loop: asyncio.AbstractEventLoop) -> None:
    ...
    # Eviction can race loop shutdown: creating a task on a closed loop raises
    # RuntimeError. The coroutine is created first and explicitly closed on that
    # path — an un-awaited coroutine object would otherwise be garbage-collected
    # with a RuntimeWarning ("coroutine was never awaited"). The client is then
    # closed synchronously; it is being discarded from the pool either way.
    coro = _close()
    try:
        loop.create_task(coro)
    except RuntimeError:
        coro.close()
        _close_client_now(client)
...
def _build_anthropic_adapter(config: LLMClientConfig, spec: Any) -> Any:
    from knorvia.services.llm.provider_core import AnthropicProvider
...
def _build_codex_adapter(config: LLMClientConfig, spec: Any) -> Any:
    from knorvia.services.codex_auth.constants import CODEX_DEFAULT_MODEL_ID
    from knorvia.services.llm.provider_core import OpenAICodexProvider
...
def _build_copilot_adapter(config: LLMClientConfig, spec: Any) -> Any:
    from knorvia.services.llm.provider_core import GitHubCopilotProvider
...
def _build_codebuddy_adapter(config: LLMClientConfig, spec: Any) -> Any:
    from knorvia.services.llm.provider_core.codebuddy_http_provider import (
