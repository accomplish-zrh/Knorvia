...
from knorvia.core.agentic.tool_arg_guard import (
...
from knorvia.utils.json_parser import parse_json_response
...
async def _reject_if_args_missing(
    *,
    ...
    fill in. See :mod:`knorvia.core.agentic.tool_arg_guard` for why the
...
def _prepare_tool_args(
    ...
    for tc in tool_calls:
        ...
        if not isinstance(tool_args, dict):
            tool_args = {}
        # Leading-underscore kwargs and the event transport belong to the
        # server.  Strip provider-emitted extras before the capability-specific
        # augmenter runs; otherwise an undeclared model argument could
        # impersonate approval tokens, sandbox identity, mounts, or callbacks.
        tool_args = {
            key: value
            for key, value in tool_args.items()
            if isinstance(key, str) and not key.startswith("_") and key != "event_sink"
        }
        exec_args = (
...
async def execute_tool_call(
    *,
    ...
    try:
        # `event_sink` is a reserved transport kwarg, not a tool parameter:
        # if the model supplies a tool argument with that name it must not
        # collide with the sink and raise a duplicate-keyword TypeError.
        call_args = {key: value for key, value in tool_args.items() if key != "event_sink"}
    ...
    for tool_index, ((tool_call_id, tool_name, _exec_args), result) in enumerate(
        ...
        if pause_request and not pause:
            ...
            pause_payload = {
                ...
                # Pause owners may need to bind the eventual user reply to the
                # exact operation that produced the prompt. Keep this internal
                # tool metadata out of the model-visible role=tool body while
                # carrying it through the in-process pause protocol.
                "metadata": (dict(tool_extra_meta) if isinstance(tool_extra_meta, dict) else {}),
            }
