...
def detect_kb_drift(
    ...
    # A deleted KB is still keyed in "current" (fingerprint_kb returns "" for
    # missing KBs) and remains in "stored" from the last baseline — an empty
    # fingerprint marks it as removed. The old "k not in current" check could
    # never fire: "current" is keyed by exactly book.knowledge_bases.
    removed_kbs = [k for k, v in current.items() if k in stored and not v]
    changed_kbs = [
