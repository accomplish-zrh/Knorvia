        ...
        *,
        ...
        for chapter in spine.chapters:
            ...
            if page is None:
                ...
                # Re-confirming an edited spine reuses page shells by
                # chapter_id: refresh the shell so renamed/reordered
                # chapters keep their page in sync.
                if page.title != chapter.title or page.order != chapter.order:
        ...
        # Chapters removed from the edited spine leave orphaned page shells
        # behind; _maybe_finalize_book requires ALL pages READY, so one
        # orphaned PENDING page would pin the book in COMPILING forever.
        chapter_ids = {chapter.id for chapter in spine.chapters}
        for stale in self.storage.list_pages(book_id):
            if stale.chapter_id not in chapter_ids:
                self.storage.delete_page(book_id, stale.id)
