...
from knorvia.knowledge.kb_types import (
    IMA_KB_TYPE,
...
from knorvia.knowledge.manifest import iter_kb_documents
...
from knorvia.services.rag.index_probe import (
    inspect_kb_versions,
...
# Process-wide lock serializing every load-mutate-save cycle against
# kb_config.json. Writers run on different threads (progress callbacks from
# LlamaIndex embedding adapters on the default executor, API handlers on the
# event loop), and each one rewrites the whole file - without this lock an
# interleaved pair silently drops each other's updates.
_CONFIG_WRITE_LOCK = threading.RLock()


def _serialized_config_write(method):
    """Hold the config write lock across the whole mutating method body."""
...
def _get_embedding_fingerprint() -> tuple[str, int] | None:
    ...
    try:
        from knorvia.services.embedding import get_embedding_config
...
class KnowledgeBaseManager:
    ...
    def _load_config(self) -> dict:
        ...
        if self.config_file.exists():
            try:
                ...
                # Quarantine the unreadable file so a later save cannot
                # silently overwrite the (possibly recoverable) registry.
                # Without this, every KB registration would be wiped and the
                # documents orphaned on the next update_kb_status() call.
                try:
                    backup = self.config_file.with_suffix(
                        f".corrupt-{datetime.now().strftime('%Y%m%d%H%M%S')}.json"
                    )
                    self.config_file.rename(backup)
                    logger.warning("Quarantined unreadable kb_config.json to %s", backup)
                except OSError as rename_err:
                    logger.warning(f"Could not quarantine kb_config.json: {rename_err}")
                return {"knowledge_bases": {}}
        return {"knowledge_bases": {}}
    ...
    def update_kb_status(
        ...
        if isinstance(progress, dict):
            ...
            if isinstance(raw_indexed_count, (bool, int, float)):
        ...
        if status == "ready":
            ...
            try:
                from knorvia.services.rag.embedding_signature import (
    ...
    def list_knowledge_bases(self) -> list[str]:
        ...
        if base_exists:
            for item in self.base_dir.iterdir():
                ...
                from knorvia.services.rag.index_versioning import list_kb_versions
    ...
    def register_knowledge_base(self, name: str, description: str = "", set_default: bool = False):
        ...
        # Reload before mutating: self.config may be stale (loaded at
        # construction), and a save from that snapshot would drop entries
        # other writers registered in the meantime.
        self.config = self._load_config()
        kb_dir = self.base_dir / name
        ...
        *,
        ...
        if dir_exists:
            try:
                ...
                # counts for the same KB (see :mod:`knorvia.knowledge.manifest`).
    ...
    def delete_knowledge_base(self, name: str, confirm: bool = False) -> bool:
        ...
        # The legacy per-file "default" key no longer exists (it is owned by
        # the centralized KB config service); if the deleted KB was the active
        # default, point the centralized default at another KB or clear it.
        try:
            from knorvia.services.config import get_kb_config_service

            kb_config_service = get_kb_config_service()
            if kb_config_service.get_default_kb() == name:
                remaining = sorted(self.config.get("knowledge_bases", {}).keys())
                kb_config_service.set_default_kb(remaining[0] if remaining else None)
        except Exception as e:
            logger.warning(f"Failed to repair centralized default after delete: {e}")
