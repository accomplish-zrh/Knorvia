...
from knorvia.services.config import resolve_llm_runtime_config
from knorvia.services.file_io import atomic_write_json
from knorvia.services.rag.factory import normalize_provider_name
...
class KnowledgeBaseInitializer:
    ...
    def copy_documents(self, source_files: list[str]) -> list[str]:
        ...
        Sources that share a basename must not silently overwrite each other
        (or a pre-existing file from an earlier run) — the newer content would
        be lost. Identical destinations are skipped and reported; differing
        collisions are skipped with a warning so no data is destroyed.
        """
        ...
        for source in source_files:
            ...
            if dest_path.exists():
                same = False
                try:
                    same = dest_path.stat().st_size == source_path.stat().st_size
                    if same:
                        with open(dest_path, "rb") as existing, open(source_path, "rb") as incoming:
                            same = existing.read() == incoming.read()
                except OSError as compare_err:
                    logger.warning(f"Could not compare {dest_path} against {source}: {compare_err}")
                if same:
                    logger.info(f"Source {source} already present at {dest_path} - skipping")
                else:
                    logger.warning(
                        f"Skipping {source}: {dest_path} already exists with different "
                        "content; rename the source to avoid a collision"
                    )
                    continue
            else:
    ...
    async def display_statistics_generic(self) -> None:
        ...
        from knorvia.services.rag.index_probe import inspect_kb_versions
