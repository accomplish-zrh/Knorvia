...
# Strong references to in-flight broadcast tasks. asyncio only keeps weak
# references to tasks, so an unreferenced task can be garbage-collected
# mid-execution and silently drop progress broadcasts.
...
class ProgressTracker:
    ...
    def _notify(self, progress: dict):
        ...
        from knorvia.runtime.mode import is_server
        ...
        if is_server():
            try:
                ...
                try:
                    ...
                    task = loop.create_task(broadcaster.broadcast(self.kb_name, progress))
    ...
    def update(
        ...
        try:
            ...
            fallback_logger = logging.getLogger("knorvia.ProgressTracker")
            prefix = f"[{self.task_id}]" if self.task_id else ""
        ...
        if self.task_id:
            try:
                from knorvia.api.utils.task_log_stream import get_task_stream_manager
