...
from knorvia.knowledge.manifest import KB_FILES_DEFAULT_LIMIT, KB_FILES_MAX_LIMIT
...
class BrainstormTool(_PromptHintsMixin, BaseTool):
    ...
    async def execute(self, **kwargs: Any) -> ToolResult:
        from knorvia.tools.brainstorm import brainstorm
...
class RAGTool(_PromptHintsMixin, BaseTool):
    ...
    async def execute(self, **kwargs: Any) -> ToolResult:
        from knorvia.tools.rag_tool import rag_search
...
class KbFilesTool(_PromptHintsMixin, BaseTool):
    ...
    async def execute(self, **kwargs: Any) -> ToolResult:
        from knorvia.knowledge.manifest import render_manifest_report
...
class WebSearchTool(_PromptHintsMixin, BaseTool):
    ...
    async def execute(self, **kwargs: Any) -> ToolResult:
        from knorvia.tools.web_search import web_search
...
class CodeExecutionTool(_PromptHintsMixin, BaseTool):
    ...
    it through :mod:`knorvia.services.sandbox`. No second LLM call, and the
    ...
    async def execute(self, **kwargs: Any) -> ToolResult:
        ...
        from knorvia.services.sandbox import (
            ExecRequest,
            Mount,
            ResourceLimits,
            get_sandbox_service,
        )
        from knorvia.services.sandbox.artifacts import (
...
class ReasonTool(_PromptHintsMixin, BaseTool):
    ...
    async def execute(self, **kwargs: Any) -> ToolResult:
        from knorvia.tools.reason import reason
...
class PaperSearchToolWrapper(_PromptHintsMixin, BaseTool):
    ...
    async def execute(self, **kwargs: Any) -> ToolResult:
        from knorvia.tools.paper_search_tool import ArxivSearchTool
...
class GeoGebraAnalysisTool(_PromptHintsMixin, BaseTool):
    ...
    async def execute(self, **kwargs: Any) -> ToolResult:
        from knorvia.agents.vision_solver.vision_solver_agent import VisionSolverAgent
...
class WriteMemoryTool(_PromptHintsMixin, BaseTool):
    ...
    async def execute(self, **kwargs: Any) -> ToolResult:
        from knorvia.services.memory import get_memory_store
        from knorvia.services.memory.trace import TraceEvent
...
class WebFetchTool(_PromptHintsMixin, BaseTool):
    ...
    ``knorvia.tools.web_fetch`` so this wrapper stays free of network
    ...
    async def execute(self, **kwargs: Any) -> ToolResult:
        from knorvia.tools.web_fetch import (
            DEFAULT_MAX_CHARS,
...
class ListNotebookTool(_PromptHintsMixin, BaseTool):
    ...
    async def execute(self, **kwargs: Any) -> ToolResult:
        from knorvia.tools.list_notebook import list_notebooks_or_records
...
class WriteNoteTool(_PromptHintsMixin, BaseTool):
    ...
    * ``edit`` — patch an existing record's title / body / summary.
    ...
    async def execute(self, **kwargs: Any) -> ToolResult:
        from knorvia.tools.write_note import write_note
...
class GithubTool(_PromptHintsMixin, BaseTool):
    ...
    async def execute(self, **kwargs: Any) -> ToolResult:
        from knorvia.tools.github_query import run_github_query
...
class CronTool(_PromptHintsMixin, BaseTool):
    ...
    def get_definition(self) -> ToolDefinition:
        return ToolDefinition(
            ...
            description=(
                ...
                "conversation's tasks. action='cancel' removes one; "
                "action='run' queues one immediately. Times without a "
                "timezone are server-local."
            ),
            parameters=[
                ToolParameter(
                    name="action",
                ...
                ToolParameter(
                    name="job_id",
                    type="string",
                    description="cancel/run: id of the task (from action='list').",
                    required=False,
                ),
            ],
        )
    ...
    async def execute(self, **kwargs: Any) -> ToolResult:
        from knorvia.tools.cron_tool import run_cron_action
