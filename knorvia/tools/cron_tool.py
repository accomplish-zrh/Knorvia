...
def run_cron_action(kwargs: dict[str, Any]) -> CronActionOutcome:
    ...
    if action == "run":
        job_id = str(kwargs.get("job_id") or "").strip()
        if not job_id:
            return CronActionOutcome(ok=False, text="run needs a job_id (see action='list').")
        job = service.run_now(job_id, owner_key=owner.key)
        if job:
            return CronActionOutcome(ok=True, text=f"Task `{job_id}` will run now.")
        return CronActionOutcome(ok=False, text=f"No task `{job_id}` found for this conversation.")
