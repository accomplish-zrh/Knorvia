...
# Strong references to in-flight emit tasks. asyncio only keeps weak
# references to tasks, so an unreferenced task can be garbage-collected
# mid-execution and silently drop process-log events.
_pending_emit_tasks: set[asyncio.Future] = set()
