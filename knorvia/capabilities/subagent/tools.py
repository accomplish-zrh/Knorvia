...
:class:`~knorvia.capabilities.subagent.capability.SubagentCapability`, which
...
if TYPE_CHECKING:  # avoid importing the services package at module-load time (cycle)
    from knorvia.services.subagent import SubagentEvent
...
class ConsultSubagentTool(BaseTool):
    ...
    def get_definition(self) -> ToolDefinition:
        return ToolDefinition(
            ...
            description=(
                ...
                "Code, Grok Build — or one of their partners) a focused question and get its "
    ...
    async def execute(self, **kwargs: Any) -> ToolResult:
        ...
        if not getattr(config, "enabled", False):
            return ToolResult(content=f"Subagent {name} is disabled.", success=False)
        from knorvia.multi_user.context import get_current_user
        from knorvia.services.subagent import PARTNER_BACKEND_KIND

        if spec.get("kind") != PARTNER_BACKEND_KIND and not get_current_user().is_admin:
            return ToolResult(
                content="Local CLI subagents are restricted to administrators.",
                success=False,
            )
        if spec.get("kind") != PARTNER_BACKEND_KIND:
            from knorvia.services.subagent.process import validate_subagent_cwd

            try:
                spec["cwd"] = validate_subagent_cwd(spec.get("cwd"))
        ...
        # Head this round with the question Knorvia is putting to the agent, so
        ...
        if result.session_id:
            ...
            if session_key_value:
                from knorvia.services.subagent.sessions import remember_session
