...
(resolved by :mod:`knorvia.capabilities.subagent.binding`). As a
...
from knorvia.capabilities.protocol import KnowledgeCapability, PromptBlock
from knorvia.capabilities.subagent.binding import connection_for_turn, subagent_refs
from knorvia.capabilities.subagent.tools import SUBAGENT_TOOL_NAMES
...
class SubagentCapability(KnowledgeCapability):
    ...
    def augment_kwargs(
        ...
        if conn is None:
            return kwargs
        from knorvia.services.subagent import load_subagent_settings
        from knorvia.services.subagent.sessions import get_session, session_key
        ...
        # session the user/Knorvia built up earlier (and the sidebar shares).
        ...
        backend_config = settings.backend(conn["kind"])
        if not backend_config.enabled:
            return kwargs
        config = _effective_config(backend_config)
...
_DEFAULT_CONSULT_INSTRUCTION = (
    "You are being consulted programmatically by Knorvia on the user's behalf, "
...
def _resolve_budget(context: UnifiedContext) -> int:
    ...
    """
    from knorvia.services.subagent import load_subagent_settings
    from knorvia.services.subagent.config import CONSULT_BUDGET_MAX, CONSULT_BUDGET_MIN
