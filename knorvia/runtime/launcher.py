"""Local Web launcher for the installed Knorvia app."""
...
from knorvia.runtime.banner import labels_for, print_banner, resolve_language
from knorvia.runtime.home import KNORVIA_HOME_ENV, PACKAGE_ROOT, get_runtime_home
from knorvia.runtime.memory_probe import SUPERVISOR_PID_ENV
...
SOURCE_PRODUCTION_DIST_DIR = ".next-knorvia"
SOURCE_BUILD_MARKER = ".knorvia-build.json"
...
def _backend_proxy_host(bind_host: str) -> str:
    """Address the frontend can use to reach the backend it just spawned."""

    candidate = bind_host.strip()
    if candidate.lower() == "localhost" or candidate in {"0.0.0.0", "::"}:
        return "127.0.0.1"
...
def _reset_runtime_singletons() -> None:
    """Make a just-selected KNORVIA_HOME visible to path/config singletons."""
    try:
        ...
        pass
    try:
        from knorvia.services.config.runtime_settings import RuntimeSettingsService
...
def _send_tree_signal(
    ...
    *,
    ...
    if pid is None:
        return
    if os.name == "nt":
        # Windows has no SIGKILL: KILL_SIGNAL falls back to SIGTERM there, so
        # the "graceful vs hard" distinction cannot ride on the signal value.
        # taskkill without /F is the graceful close; /F is the forced kill.
        # Use the explicit *force* flag instead of comparing signals.
...
def _terminate(proc: ManagedProcess | None) -> None:
    ...
    try:
        ...
        try:
            _send_tree_signal(proc.process.pid, proc.pgid, KILL_SIGNAL, force=True)
        except Exception:
            pass
    ...
    for port, entries in listeners.items():
        ...
        if _port_accepts_connection(port):
            for pid, _command in entries:
                try:
                    _send_tree_signal(pid, None, KILL_SIGNAL, force=True)
                except Exception:
                    pass
...
def _wait_for_http(
    *,
    ...
    while time.monotonic() < deadline:
        ...
        try:
            with urlrequest.urlopen(url, timeout=1):  # nosec B310 - http(s) health-check URL constructed by caller
...
def _http_ready(url: str, *, timeout: float) -> bool:
    try:
        with urlrequest.urlopen(url, timeout=timeout):  # nosec B310 - launcher health check
            return True
    except (urlerror.URLError, TimeoutError, OSError):
        return False
...
def _packaged_web_dir() -> Path | None:
    try:
        ...
        return None
    path = Path(knorvia_web.__file__).resolve().parent
...
def _copy_packaged_web_if_needed(
    ...
    *,
    ...
    marker = cache / ".knorvia-web-runtime.json"
...
def _ensure_web_dependencies(source: Path, npm: str) -> None:
    ...
    if result.returncode != 0:
        raise SystemExit(
            ...
            "Fix the error above, then retry `knorvia start`."
        )
...
def _source_production_env(*, api_base: str, auth_enabled: bool) -> dict[str, str]:
    ...
    env = os.environ.copy()
    env["KNORVIA_NEXT_DIST_DIR"] = SOURCE_PRODUCTION_DIST_DIR
...
def _source_build_fingerprint(source: Path, env: dict[str, str]) -> str:
    ...
    version_file = PACKAGE_ROOT / "knorvia" / "__version__.py"
...
def _ensure_source_production_build(
    ...
    *,
    ...
    Long-running ``knorvia start`` processes must not use ``next dev``: its
...
def _resolve_frontend(
    ...
    *,
    ...
    if packaged is not None:
        if not node:
            raise SystemExit("Node.js 20+ is required to run the packaged Knorvia Web app.")
    ...
    raise SystemExit(
        "Knorvia Web assets are not installed. Install the full app with `pip install -U knorvia`, "
...
def start(
    ...
    *,
    ...
    os.environ[KNORVIA_HOME_ENV] = str(runtime_home)
    ...
    if not bind_host:
        raise ValueError("The bind host cannot be empty")
    if not _is_loopback_host(bind_host) and not auth_enabled:
        raise RuntimeError(
            "Refusing to expose Knorvia on a non-loopback address while "
            "authentication is disabled. Enable authentication first or use "
            "--host 127.0.0.1."
        )
    ...
    backend_url = f"http://{_backend_proxy_host(bind_host)}:{backend_port}"
    api_base = (
    ...
    common_env[KNORVIA_HOME_ENV] = str(runtime_home)
    ...
    common_env["KNORVIA_API_BASE_URL"] = backend_url
    common_env["KNORVIA_AUTH_ENABLED"] = "true" if auth_enabled else "false"
    common_env["PYTHONUNBUFFERED"] = "1"
    common_env["PYTHONIOENCODING"] = "utf-8:replace"
    # Anchor for knorvia.runtime.memory_probe: the backend and the frontend
    # are siblings under this process, so only the supervisor's pid identifies
    # the tree that is "Knorvia". Without it the probe can only measure the
    # backend itself.
    common_env[SUPERVISOR_PID_ENV] = str(os.getpid())
    _apply_single_user_allocator_env(common_env)
    if frontend.kind == "source-production":
        common_env["KNORVIA_NEXT_DIST_DIR"] = SOURCE_PRODUCTION_DIST_DIR

    backend_cmd = [
        sys.executable,
        "-m",
        "uvicorn",
        "knorvia.api.main:app",
        "--host",
        bind_host,
        "--port",
        str(backend_port),
        ...
        # middleware (knorvia/api/main.py) surfaces only non-200s, so routine
