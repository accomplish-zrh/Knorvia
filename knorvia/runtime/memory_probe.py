"""Resident-memory snapshot of the running Knorvia process tree.

The counterpart to :mod:`knorvia.runtime.memory_reclaim`: that module *acts*
...
Two things make "how much memory does Knorvia use" more than one ``getrusage``
call:

* The backend is only one of the processes. ``knorvia.runtime.launcher``
  ...
  ``uvicorn knorvia.api.main:app`` run would climb into the developer's shell
...
# find the root of the Knorvia process tree it belongs to.
SUPERVISOR_PID_ENV = "KNORVIA_SUPERVISOR_PID"
...
class ProcessMemory:
    """One process in the Knorvia tree."""
...
def _classify(pid: int, name: str, cmdline: str, root_pid: int | None = None) -> str:
    ...
    the supervisor runs as a bare ``knorvia`` console script with nothing in
    ...
    if "next" in lowered or "node" in lowered:
        return "web"
    if "uvicorn" in lowered or "knorvia.api" in lowered:
        return "backend"
...
def _scan_proc() -> tuple[list[ProcessMemory], bool]:
    """psutil-free fallback for Linux — the platform Knorvia ships in Docker on."""
