"""Branded banner + localized labels for ``knorvia start`` / ``knorvia init``.
...
from knorvia.__version__ import __version__
...
LABELS: dict[str, dict[str, str]] = {
    "en": {
        ...
        "lab": "AI learning and knowledge workspace",
        # init
        ...
        "init.next_step": "Run `knorvia start` to launch Knorvia.",
        "init.choice": "Choice",
        ...
        "start.frontend_restart_failed": (
            ...
            "Stop PID {pid} and run `knorvia start` again."
        ),
        ...
        "start.port_in_use": (
            "Knorvia cannot start because port(s) already in use: {ports}. "
    ...
    "zh": {
        ...
        "init.next_step": "运行 `knorvia start` 启动 Knorvia。",
        "init.choice": "请选择",
        ...
        "start.frontend_restart_failed": (
            ...
            "请先停止 PID {pid},然后重新运行 `knorvia start`。"
        ),
...
def resolve_language(default: str = "en") -> str:
    ...
    """
    try:
        from knorvia.services.settings.interface_settings import get_ui_language
...
def render_banner(language: str | None, *, mode_key: str | None = None) -> Panel:
    ...
    return Panel(
        Align.left(body),
        title="[bold bright_cyan]Knorvia[/]",
