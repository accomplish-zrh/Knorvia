...
An *external provider* is something that contributes tools Knorvia did not
...
``load_tools`` (see :mod:`knorvia.runtime.registry.deferred_tools`). What
they do *not* share is authorisation, so that stays per-kind and explicit in
:mod:`~knorvia.runtime.providers.authorize`.
...
upwards. Import it directly (``from knorvia.runtime.providers.view import
...
from knorvia.runtime.providers.text import (
