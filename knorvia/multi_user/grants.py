...
def normalize_grant(user_id: str, payload: dict[str, Any] | None) -> dict[str, Any]:
    """Coerce any stored/submitted grant payload into the v4 shape.

    v1 grants normalize losslessly for everything that was ever enforced:
    ``models.embedding`` / ``models.search`` / ``spaces`` had no runtime
    consumers and are dropped; old grants gain empty image/video assignments.
    """
    ...
    image_items = models.get("imagegen") if isinstance(models, dict) else []
    if not isinstance(image_items, list):
        image_items = []
    base["models"]["imagegen"] = [dict(item) for item in image_items if isinstance(item, dict)]
    video_items = models.get("videogen") if isinstance(models, dict) else []
    if not isinstance(video_items, list):
        video_items = []
    base["models"]["videogen"] = [dict(item) for item in video_items if isinstance(item, dict)]
