...
from knorvia.services.skill.service import SkillService
...
def _admin_catalog_summary() -> dict[str, list[dict[str, Any]]]:
    ...
    out: dict[str, list[dict[str, Any]]] = {"llm": [], "imagegen": []}
...
async def put_user_grants(
    ...
    log_admin_action(
        ...
        summary={
            "model_count": sum(
                len(grant.get("models", {}).get(name, []) or []) for name in ("llm", "imagegen")
            ),
...
async def admin_install_skill(
    ...
    lives in :func:`knorvia.services.skill.hub.install_from_hub`; this
