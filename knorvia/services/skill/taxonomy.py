...
The single Python source of truth for ClawHub's skill classification — the
required ``track`` (one-of) plus the optional faceting dimensions
(``language``, ``domains``, ``stages``, ``forms``, ``audiences``). It mirrors
the clawhub web's ``src/data/taxonomy.ts`` (now maintained in zzhtx258/clawhub,
``packages/clawhub-web``) value-by-value so the interactive
