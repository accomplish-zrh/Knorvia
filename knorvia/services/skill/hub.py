...
A hub skill is the same artefact Knorvia already speaks natively — a
...
``clawhub:socratic-tutor@1.2.0``; the hub prefix defaults to ``clawhub``).
...
# A bare ``install <slug>`` with no ``<hub>:`` prefix resolves to ClawHub.
DEFAULT_HUB = "clawhub"
...
# override any entry (e.g. point ``clawhub`` at a local dev server).
...
class ClawHubProvider:
    ...
    def web_origin(self) -> str:
        ...
        Used to build "view on ClawHub" links for the in-app skill browser.
        """
        marker = "/api/"
    ...
    def fetch(self, slug: str, *, version: str | None = None) -> FetchedSkill:
        ...
        tmp = Path(tempfile.mkdtemp(prefix="knorvia-skill-"))
        try:
    ...
    def catalog(
        ...
        Knorvia-native browser renders without a second round-trip.
        """
        if query.strip():
    ...
    def detail(self, slug: str) -> dict[str, Any]:
        ...
        # ClawHub's ``tags`` field is a dist-tags map; the topical labels live in
    ...
    def publish(
        ...
        *,
        ...
        Read-only ClawHub mirrors will reject this; clawhub (same /api/v1 shape)
...
def default_hub() -> str:
    ...
    Settings-driven (``"default": "clawhub"`` in skill_hubs.json) so a
    deployment can point the bare ``install <slug>`` at its own hub without
    forking the code; falls back to the built-in ``clawhub``.
    """
...
def get_hub_provider(name: str) -> SkillHubProvider:
    ...
    The built-in ``clawhub`` provider works with no configuration; entries
    ...
    *,
    ...
    try:
        result = service.install_tree(
            ...
            # grouped (e.g. an ``clawhub`` tag) in the user's Skills list.
            extra_tags=[hub],
            origin={
                "hub": hub,
                "slug": slug,
...
def publish_to_hub(
    ...
    *,
    ...
    ``slug``/``version``/``track`` are what clawhub requires (track is
