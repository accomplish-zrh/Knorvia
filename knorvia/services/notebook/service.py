...
class NotebookManager:
    ...
    @staticmethod
    def _safe_notebook_id(notebook_id: str) -> str | None:
        # Ids reach file paths directly, so a value like "../../victim" must
        # never resolve - it would allow reading, overwriting or deleting
        # files outside the notebook store. Only simple path-free names pass.
        safe = str(notebook_id or "").strip()
    ...
    def _get_notebook_file(self, notebook_id: str) -> Path | None:
        safe_id = self._safe_notebook_id(notebook_id)
        if safe_id is None:
            return None
    ...
    def _save_notebook(self, notebook: dict) -> None:
        ...
        if filepath is None:
            raise ValueError(f"Invalid notebook id: {notebook.get('id')!r}")
