"""Nanobot-style normalized runtime configuration for Knorvia."""
...
from knorvia.services.model_selection import LLMSelection, apply_llm_selection_to_catalog
from knorvia.services.provider_registry import (
...
class SearchProviderSpec:
    ...
    its own answer or needs :mod:`knorvia.services.search.consolidation` to
    ...
    The provider registry (``knorvia/services/search/providers``), the
    Settings dropdown (``knorvia/api/routers/settings.py``) and the web
...
# Video-generation providers. Text-to-video has no synchronous standard, so
# each provider declares the task protocol and request shaping it actually uses.
VIDEOGEN_PROVIDERS: dict[str, GenerationProviderSpec] = {
    ...
    "kling": GenerationProviderSpec(
        label="Kling AI (Kling)",
        default_api_base="https://api.klingai.com",
        adapter="kling_async_task",
        default_model="kling-v2-master",
    ),
    "wan": GenerationProviderSpec(
...
# Retired-but-resolvable providers. OpenAI Videos shuts down on 2026-09-24 and
# is no longer offered in the presets/dropdown (the Settings provider list only
# iterates VIDEOGEN_PROVIDERS), but catalogs saved before the retirement keep
# resolving here so existing user configurations run unchanged until the
# adapter itself is removed next major version.
VIDEOGEN_LEGACY_PROVIDERS: dict[str, GenerationProviderSpec] = {
    "openai": GenerationProviderSpec(
        label="OpenAI Videos (deprecated Sep 24, 2026)",
        default_api_base="https://api.openai.com/v1",
        adapter="openai_videos",
        default_model="sora-2",
    ),
}
VIDEOGEN_ADAPTERS = frozenset(
    {
        "async_task",
        "openai_videos",
...
def _as_str_list(value: Any) -> list[str]:
    """Coerce a catalog field into a clean, de-duplicated list of non-empty strings.

    Accepts a JSON array, a single string, or a CSV string — the catalog UI may
    store any of these shapes; order is preserved and blanks/duplicates drop.
    """
    if value is None:
        return []
    if isinstance(value, str):
        try:
            ...
            parsed = [part for part in value.split(",")]
        value = parsed
...
def _with_personal_llm_profiles(catalog: dict[str, Any]) -> dict[str, Any]:
    ...
    the shared one (see :mod:`knorvia.multi_user.personal_models`), so
...
def resolve_tts_runtime_config(
    ...
    *,
    ...
    return TTSConfig(
        ...
        adapter=_as_str((model or {}).get("adapter")) or spec.adapter,
        ...
        voice=voice,
        # §Phase D4: gateway-advertised custom voice ids feed the enumeration
        # endpoint; they ride the config only as metadata.
        custom_voices=_as_str_list((model or {}).get("custom_voices")),
...
def resolve_imagegen_runtime_config(
    ...
    *,
    ...
    connection = catalog_service.get_profile_connection(loaded, profile)
    resolved_model = _as_str((model or {}).get("model"))
    if not resolved_model:
        raise ValueError(
    ...
    api_base = (
        _as_str(connection.get("base_url"))
        or _as_str((profile or {}).get("base_url"))
        or spec.default_api_base
    )
    api_key = _as_str(connection.get("api_key")) or _as_str((profile or {}).get("api_key"))
    if not api_key and spec.is_local:
        api_key = "sk-no-key-required"
    ...
    return ImagegenConfig(
        ...
        api_key=api_key,
        base_url=api_base,
        api_version=_as_str(connection.get("api_version"))
        or _as_str((profile or {}).get("api_version"))
        or None,
        extra_headers=_to_headers(
            connection.get("extra_headers") or (profile or {}).get("extra_headers")
        ),
        ...
        image_size=_as_str((model or {}).get("image_size")),
        background=_as_str((model or {}).get("background")),
        compression=(
            int((model or {}).get("compression"))
            if str((model or {}).get("compression") or "").isdigit()
            else None
        ),
    )
...
def resolve_videogen_runtime_config(
    ...
    *,
    ...
    if provider == "custom":
        # Retired providers (OpenAI Videos) stay resolvable for catalogs saved
        # before their removal from the offered presets.
        legacy_key = _as_str((profile or {}).get("binding")).strip().lower().replace("-", "_")
        if legacy_key in VIDEOGEN_LEGACY_PROVIDERS:
            provider = legacy_key
    spec = VIDEOGEN_PROVIDERS.get(provider) or VIDEOGEN_LEGACY_PROVIDERS[provider]
    api_base = (
        _as_str(connection.get("base_url"))
        or _as_str((profile or {}).get("base_url"))
        or spec.default_api_base
    ...
    requested_adapter = _as_str((model or {}).get("adapter")) or _as_str(
        (profile or {}).get("adapter")
    )
    if requested_adapter and requested_adapter not in VIDEOGEN_ADAPTERS:
        raise ValueError(f"Unsupported video-generation adapter: {requested_adapter}")
    adapter = requested_adapter or spec.adapter
