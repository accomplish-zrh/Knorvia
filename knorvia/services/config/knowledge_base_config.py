...
# ``knorvia/services/config/__init__.py``) which resolves the path lazily.
...
class KnowledgeBaseConfigService:
    ...
    def _save(self) -> None:
        ...
        # Atomic replace so a crash mid-write can never leave a truncated
        # kb_config.json behind (it holds every KB registration).
