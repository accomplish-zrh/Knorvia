...
class ConfigTestRunner:
    ...
    async def _test_llm(self, run: TestRun, catalog: dict[str, Any]) -> None:
        from knorvia.services.llm import clear_llm_config_cache, get_token_limit_kwargs
        from knorvia.services.llm import complete as llm_complete
    ...
    async def _test_embedding(
        ...
        from knorvia.services.embedding.client import EmbeddingClient
        from knorvia.services.embedding.config import EmbeddingConfig
        ...
        probe_texts = [
            "Knorvia embedding smoke test",
            "Knorvia retrieval batch probe",
        ]
    ...
    def _test_search(self, run: TestRun, catalog: dict[str, Any]) -> None:
        from knorvia.services.search import web_search
        ...
        run.emit("info", "Running search query: Knorvia configuration health check")
        result = web_search("Knorvia configuration health check", provider=provider)
        run.emit(
            "response",
    ...
    async def _test_tts(self, run: TestRun, catalog: dict[str, Any]) -> None:
        ...
        sample = "Knorvia voice check. 这是一段语音合成测试。"
    ...
    async def _test_stt(self, run: TestRun, catalog: dict[str, Any]) -> None:
        ...
        from knorvia.services.voice import transcribe_audio
    ...
    async def _test_videogen(self, run: TestRun, catalog: dict[str, Any]) -> None:
        ...
        from knorvia.services.video_studio.provider import get_video_studio_adapter
        ...
        adapter = get_video_studio_adapter(resolved.adapter)
        task_id = await adapter.submit(
            "A short test clip of a calm ocean wave.",
            resolved,
            inputs=[],
            parameters={
            ...
            idempotency_key=f"config-probe-{uuid4().hex}",
        )
        run.emit(
            "response",
