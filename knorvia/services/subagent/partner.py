...
*partner session key*. The first consult of a Knorvia chat session has none,
so we mint a fresh ``dt-…`` key and return it; the cross-turn registry
(:mod:`knorvia.services.subagent.sessions`) remembers it against
(chat session, connection), so every later consult in the same Knorvia chat —
within one turn or across turns — resumes the SAME partner session. The partner
page then sees one complete history session per Knorvia chat, titled from the
...
class PartnerBackend(SubagentBackend):
    ...
    async def consult(
        ...
        *,
        ...
        # a Knorvia chat → mint a stable, colon-free key; the registry threads
