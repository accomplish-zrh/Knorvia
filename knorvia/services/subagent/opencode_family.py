...
HTTP server (managed by :mod:`knorvia.services.subagent.opencode_server`),
...
from knorvia.services.subagent.opencode_server import acquire_server
from knorvia.services.subagent.process import probe_version
...
class OpencodeFamilyBackend(SubagentBackend):
    ...
    async def _create_session(self, client: httpx.AsyncClient) -> str:
        response = await client.post("/session", json={"title": "Knorvia consult"})
        response.raise_for_status()
        data = response.json()
