...
* ``consult_budget`` — the user-facing "max rounds": the maximum number of times
  Knorvia may put a question to the subagent in one turn. Enforced
...
* per-backend ``permission_mode`` / ``sandbox`` / ``approval`` — how the local
  agent runs non-interactively. Defaults are deliberately read-only/fail-closed;
  an administrator must explicitly opt into autonomous host writes.
"""
...
class BackendConfig:
    ...
    system_prompt: str = ""
    # Claude Code: --permission-mode value. The normal permission boundary is
    # kept by default; bypassPermissions remains an explicit admin choice.
    permission_mode: str = "default"
    # Codex: --sandbox / approval_policy. Read-only avoids turning a connected
    # CLI into an implicit host-write primitive before an admin opts in.
    ...
    # Kimi / opencode / MiMo: permission requests fail closed by default.
