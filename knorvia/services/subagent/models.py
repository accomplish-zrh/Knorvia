...
reasoning effort Knorvia drives each backend with. Those lists change over
...
* **Grok Build** lists models via ``grok models``; ``--reasoning-effort``
  uses the documented scale (none / minimal / low / medium / high / xhigh / max).
...
from knorvia.services.subagent.process import probe_version, subagent_environment
from knorvia.services.subagent.registry import get_backend
...
# Grok Build: ``--reasoning-effort`` / ``--effort``.
_GROK_EFFORTS = ("none", "minimal", "low", "medium", "high", "xhigh", "max")
...
async def _claude_options() -> BackendOptions:
    from knorvia.services.subagent.claude_models import load_cached_claude_models
...
async def _grok_build_options(*, refresh: bool = False) -> BackendOptions:
    ok, version = await _probe("grok_build")
    default = ""
    pairs: list[tuple[str, str]] = []
    if ok:
        listed = await _list_grok_models(refresh=refresh)
        default, pairs = listed
    if not pairs:
        pairs = list(_GROK_FALLBACK_MODELS)
    return BackendOptions(
        kind="grok_build",
        ...
        models=[
            ModelOption(slug=slug, display_name=name, efforts=list(_GROK_EFFORTS))
            for slug, name in pairs
        ],
        efforts=list(_GROK_EFFORTS),
        allow_custom_model=True,
        detail="" if ok else (version or "grok CLI not found on PATH"),
    )
...
async def _list_grok_models(*, refresh: bool = False) -> tuple[str, list[tuple[str, str]]]:
    ...
    try:
        ...
        out, _ = await asyncio.wait_for(process.communicate(), timeout=30.0 if refresh else 15.0)
    except FileNotFoundError:
        return "", []
    except (TimeoutError, asyncio.TimeoutError):
        return "", []
    except Exception:  # pragma: no cover - defensive
        logger.warning("failed to enumerate grok models", exc_info=True)
        return "", []
    return parse_grok_models((out or b"").decode("utf-8", "replace"))
...
_PROVIDERS: dict[str, Callable[..., Awaitable[BackendOptions]]] = {
    ...
    "mimo": _mimo_options,
    "grok_build": _grok_build_options,
}
...
async def sync_backend_options(kind: str) -> BackendOptions:
    ...
    """
    if kind == "claude_code":
        from knorvia.services.subagent.claude_models import sync_claude_models
    ...
    if kind == "grok_build":
        return await _grok_build_options(refresh=True)
