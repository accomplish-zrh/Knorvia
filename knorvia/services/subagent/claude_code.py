...
class ClaudeCodeBackend(SubagentBackend):
    ...
    async def detect(self) -> DetectResult:
        ...
        ok, detail = enhanced_detail(
            self.cli_command,
            lambda: "claude CLI not found on PATH",
            ok,
            text,
        )
        return DetectResult(
    ...
    async def consult(
        ...
        *,
        ...
        try:
            ...
            mcp_path = write_claude_mcp_config(collect_mcp_servers(owner_id=current_mcp_owner_id()))
        except Exception:
            logger.debug("Could not export MCP config for Claude Code", exc_info=True)
