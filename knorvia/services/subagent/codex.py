...
class CodexBackend(SubagentBackend):
    ...
    async def detect(self) -> DetectResult:
        ...
        ok, detail = enhanced_detail(
            self.cli_command,
            lambda: "codex CLI not found on PATH",
            ok,
            text,
        )
        return DetectResult(
    ...
    async def consult(
        ...
        *,
        ...
        try:
            ...
            mcp_flags, mcp_env = codex_mcp_overrides(
                collect_mcp_servers(owner_id=current_mcp_owner_id())
            )
        except Exception:
            logger.debug("Could not export MCP config for Codex", exc_info=True)
        ...
        try:
            async for channel, line in stream_process_lines(cmd, cwd=cwd, env=mcp_env):
                if channel == "exit":
