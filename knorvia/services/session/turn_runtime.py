...
from knorvia.services.session.protocol import SessionStoreProtocol
...
def _llm_selection_dict(value: Any) -> dict[str, str] | None:
    from knorvia.services.model_selection import LLMSelection
...
@dataclass
class _PendingReply:
    """One bounded, live ask-user handoff for a turn."""
...
class TurnRuntimeManager:
    ...
    def __init__(self, store: SessionStoreProtocol | None = None) -> None:
        ...
        # Per-turn reply handoffs used by tools that pause the agentic loop.
        # The holder exists for the turn lifetime, but ``accepting`` becomes
        # true only while the pipeline is actually blocked on ask_user. This
        # rejects early, duplicate, and post-completion replies.
        ...
        self._reply_queues: dict[str, _PendingReply] = {}
    ...
    async def start_turn(self, payload: dict[str, Any]) -> tuple[dict[str, Any], dict[str, Any]]:
        ...
        if not payload.get("language"):
            from knorvia.services.settings.interface_settings import (
        ...
        try:
            from knorvia.runtime.request_contracts import validate_capability_config
        ...
        if llm_selection:
            try:
                from knorvia.multi_user.model_access import apply_allowed_llm_selection
            ...
            from knorvia.multi_user.model_access import (
            ...
            if not current_user.is_admin:
                ...
                if not assigned_llms:
                    # Granted but none currently available: a clear terminal
                    # error beats an IndexError 500 the user cannot act on.
                    raise RuntimeError(
                        "No available LLM model is assigned to your account. "
        ...
        if llm_selection:
            ...
            from knorvia.services.config import get_model_catalog_service
        ...
        if payload.get("tools") is None:
            try:
                from knorvia.api.routers.settings import get_enabled_optional_tools
        ...
        # capability's turn.
        from knorvia.multi_user.tool_access import allowed_optional_tools
    ...
    async def submit_user_reply(
        ...
        *,
        ...
        ``role=tool`` message.
        """
        # Lookup, state check and enqueue are atomic with waiter teardown.
        async with self._lock:
            pending = self._reply_queues.get(turn_id)
            if pending is None or not pending.accepting or pending.queue.full():
                return False
            payload: dict[str, Any] = {"text": text or "", "answers": answers}
            # Consume the accepting state at enqueue time, not after the
            # waiter resumes, so a second submit cannot win the small race
            # between queue.get() and the waiter's finally block.
            pending.accepting = False
            pending.queue.put_nowait(payload)
        return True
    ...
    async def _run_turn(self, execution: _TurnExecution) -> None:
        ...
        # One bounded handoff per turn. It is registered before the
        # orchestrator starts, but rejects submissions until the waiter below
        # explicitly enters the accepting state.
        pending_reply = _PendingReply()
        async with self._lock:
            self._reply_queues[turn_id] = pending_reply

        async def _wait_for_user_reply() -> dict[str, Any] | None:
            async with self._lock:
                current = self._reply_queues.get(turn_id)
                if current is not pending_reply or pending_reply.accepting:
                    return None
                pending_reply.accepting = True
            try:
                return await pending_reply.queue.get()
            finally:
                async with self._lock:
                    if self._reply_queues.get(turn_id) is pending_reply:
                        pending_reply.accepting = False
        ...
        try:
            ...
            from knorvia.services.session.context_builder import ContextBuilder
            from knorvia.services.skill import get_skill_service
            ...
            attachments = [
                Attachment(
                    ...
                    studio_asset_id=str(r.get("studio_asset_id") or ""),
                    studio_job_id=str(r.get("studio_job_id") or ""),
                    studio_project_id=str(r.get("studio_project_id") or ""),
                )
            ...
            from knorvia.multi_user.skill_access import assigned_skill_ids
            from knorvia.services.persona import PersonaService, get_persona_service
            from knorvia.services.skill.service import SkillService, render_skills_manifest
            ...
            if stream_done_sent:
                # The turn already delivered its terminal DONE and persisted the
                # assistant message; the only work left was post-turn metadata
                # (title generation). Re-persisting here would duplicate the
                # assistant message, and flipping the status to "cancelled"
                # would mislabel a turn the user already saw complete.
                raise
            ...
            # accumulating on a dead turn. Under the same lock the submitter
            # uses, so the check-and-enqueue there is atomic with this pop.
            async with self._lock:
            ...
            from knorvia.runtime.memory_reclaim import schedule_memory_reclaim
