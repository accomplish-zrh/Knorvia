...
#: How many times the legacy-DB move is attempted before failing loudly.
_LEGACY_MIGRATION_ATTEMPTS = 5
#: Delay between migration retries (Windows file locks are transient).
_LEGACY_MIGRATION_RETRY_DELAY_S = 0.5
...
class SQLiteSessionStore:
    ...
    def _migrate_legacy_db(self, path_service) -> None:
        ...
        The move is retried because on Windows an OS-level rename fails while
        any process still holds the file open (a second Knorvia instance
        shutting down, an antivirus scan, ...). Retrying briefly covers that
        transient lock. If it still cannot move the file we raise instead of
        silently initializing an empty database at the new path: a silent
        empty DB would satisfy the ``db_path.exists()`` guard above on every
        later start, stranding the user's chat history at the legacy path
        forever. Failing loudly here leaves the legacy file in place so the
        ...
        for attempt in range(_LEGACY_MIGRATION_ATTEMPTS):
            try:
                os.replace(legacy_path, self.db_path)
                return
            except OSError as exc:
                last_error = exc
                if attempt < _LEGACY_MIGRATION_ATTEMPTS - 1:
                    time.sleep(_LEGACY_MIGRATION_RETRY_DELAY_S)
        raise RuntimeError(
            "Could not move the legacy chat database into place "
            f"({legacy_path} -> {self.db_path}): {last_error}. "
            "Close any other running Knorvia instance and restart; "
            "your chat history is still intact at the legacy path."
        ) from last_error
