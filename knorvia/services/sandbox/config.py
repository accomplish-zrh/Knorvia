...
* ``KNORVIA_SANDBOX_RUNNER_URL`` set ⇒ runner sidecar (Docker deployment);
* else on Linux with a functional ``bwrap`` ⇒ bwrap (bare-metal);
* else, only when ``KNORVIA_SANDBOX_ALLOW_SUBPROCESS=1`` ⇒ restricted
...
:mod:`knorvia.tools.exec_tool`). Per-user quotas live in
:mod:`knorvia.services.sandbox.quota`.
"""
...
from knorvia.services.sandbox.backends import (
    BwrapBackend,
    RestrictedSubprocessBackend,
    RunnerSidecarBackend,
    SandboxBackend,
)
from knorvia.services.sandbox.spec import ResourceLimits

RUNNER_URL_ENV = "KNORVIA_SANDBOX_RUNNER_URL"
ALLOW_SUBPROCESS_ENV = "KNORVIA_SANDBOX_ALLOW_SUBPROCESS"

# Per-user execution quotas (see quota.py). Conservative defaults; override
# via the matching env vars.
MAX_CONCURRENT_ENV = "KNORVIA_SANDBOX_MAX_CONCURRENT"
MAX_PER_MINUTE_ENV = "KNORVIA_SANDBOX_MAX_PER_MINUTE"
...
def build_backend(settings: SandboxSettings) -> SandboxBackend | None:
    ...
    backend's ``health()`` — see :mod:`knorvia.services.sandbox.service`.
    """
    import sys
