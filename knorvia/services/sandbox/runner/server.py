...
(:class:`knorvia.services.sandbox.backends.RunnerSidecarBackend`).
...
Wire contract (must match ``RunnerSidecarBackend``):
  ...
  ``POST /exec``   -> request/response JSON described by the dataclasses in
                      :mod:`knorvia.services.sandbox.spec`. Request::
...
Mounts note:
  ...
  (``KNORVIA_RUNNER_ALLOWED_WORKDIRS``) as defence in depth.
"""
...
_ALLOWED_WORKDIR_ROOTS = [
    ...
    for root in os.environ.get(
        "KNORVIA_RUNNER_ALLOWED_WORKDIRS",
