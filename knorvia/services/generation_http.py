...
def raise_for_provider(resp: httpx.Response, action: str) -> None:
    ...
    if resp.status_code < 400:
        return
    detail = sanitize_provider_detail((resp.text or "").strip())[:400]
    raise GenerationProviderError(
        f"{action} failed with HTTP {resp.status_code}" + (f": {detail}" if detail else ".")
    )


def sanitize_provider_detail(value: str) -> str:
    """Remove common credential forms from provider-controlled diagnostics."""
    safe = re.sub(
        r'(["\'](?:api_?key|access_?token|token|secret|password)["\']\s*:\s*["\'])[^"\']+',
        r"\1[REDACTED]",
        value,
        flags=re.I,
    )
    safe = re.sub(
        r"([?&](?:api_?key|access_?token|token|key|secret)=)[^&\s]+",
        r"\1[REDACTED]",
        safe,
        flags=re.I,
    )
    return re.sub(r"(Bearer\s+)[A-Za-z0-9._~-]+", r"\1[REDACTED]", safe, flags=re.I)
