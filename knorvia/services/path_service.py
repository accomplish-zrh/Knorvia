...
from knorvia.runtime.home import PACKAGE_ROOT, get_runtime_data_root
...
class PathService:
    ...
    def _safe_id_component(value: str) -> str:
        # Id-based getters compose filesystem paths from caller-supplied ids
        # (book_id, doc_id, task_id, notebook_id, ...). A value containing
        # path separators or ".." would resolve outside this service's root,
        # letting HTTP-supplied ids read, overwrite or delete arbitrary files.
        safe = str(value or "").strip()
        if not safe or safe in {".", ".."}:
            raise ValueError(f"Invalid path id component: {value!r}")
        if any(ch in safe for ch in ("/", "\\", ":")) or ".." in safe:
            raise ValueError(f"Invalid path id component: {value!r}")
        return safe
    ...
    def get_parse_cache_root(self) -> Path:
        ...
        ``(source_hash, parser_signature)`` — see ``knorvia/services/parsing``.
        """
    ...
    def get_task_dir(self, module: str, task_id: str) -> Path:
        return self.get_agent_dir(module) / self._safe_id_component(task_id)
    ...
    def get_notebook_file(self, notebook_id: str) -> Path:
        return self.get_notebook_dir() / f"{self._safe_id_component(notebook_id)}.json"
    ...
    def get_co_writer_doc_root(self, doc_id: str) -> Path:
        ...
        return self.get_co_writer_docs_dir() / f"doc_{self._safe_id_component(doc_id)}"
    ...
    def get_book_root(self, book_id: str) -> Path:
        ...
        return self.get_book_dir() / f"book_{self._safe_id_component(book_id)}"
    ...
    def get_book_page_file(self, book_id: str, page_id: str) -> Path:
        return self.get_book_pages_dir(book_id) / f"{self._safe_id_component(page_id)}.json"
