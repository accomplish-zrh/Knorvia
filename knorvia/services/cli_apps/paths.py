...
from knorvia.services.cli_apps.models import ENTRY_POINT_RE, AppRuntime
...
def cli_apps_root() -> Path:
    ...
    from knorvia.multi_user.paths import ADMIN_WORKSPACE_ROOT
...
def _venv_bin_name() -> str:
    """Directory holding a venv's executables on this platform.

    ``venv`` puts console scripts under ``Scripts`` on Windows and ``bin``
    everywhere else. ``bin_dir`` and the installer must agree on this or
    installs succeed while the executable check (and every later invocation)
...
def bin_dir(app_id: str, runtime: AppRuntime) -> Path:
    if runtime is AppRuntime.NODE:
        # `npm install -g --prefix <dir>` creates <dir>/bin on POSIX.
        return runtime_dir(app_id, runtime) / "bin"
    return runtime_dir(app_id, runtime) / _venv_bin_name()
