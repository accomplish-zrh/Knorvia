...
entry's provenance (see :class:`~knorvia.services.cli_apps.models.AppTrust`).

What this module does *not* do:

* **Run the registry's install command.** It runs an argv assembled from an
  :class:`~knorvia.services.cli_apps.models.InstallPlan`. See ``models``.
...
from knorvia.services.cli_apps.state import (
    InstalledApp,
    forget_install,
    load_installed,
    now_stamp,
...
INSTALL_TIMEOUT_S = int(os.environ.get("KNORVIA_CLI_APP_INSTALL_TIMEOUT_S", "900"))
...
def _steps(entry: CliAppEntry, env_dir) -> list[tuple[list[str], dict[str, str]]]:
    ...
    if plan.runtime is AppRuntime.PYTHON:
        ...
        return [
            ...
            (
                [
                    str(env_dir / _venv_bin_name() / "python"),
                    "-m",
                    "pip",
                    "install",
                    "--no-input",
...
def _pin_of(entry: CliAppEntry) -> str:
    ...
    if entry.install.kind is InstallKind.PINNED_HARNESS:
        from knorvia.services.cli_apps.catalog import catalog_pin
