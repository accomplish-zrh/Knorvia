...
exec path in Knorvia whose arguments are model-authored *and* whose program is
...
from knorvia.services.cli_apps.paths import abi_stamp, bin_dir, executable_path, runtime_dir
from knorvia.services.cli_apps.state import InstalledApp
from knorvia.services.sandbox.spec import ExecRequest, ExecResult, Mount, ResourceLimits
...
async def run_app(
    ...
    *,
    ...
    from knorvia.services.sandbox import get_sandbox_service
...
def _env_for(app: InstalledApp) -> dict[str, str]:
    ...
    return {
        "PATH": _path_for(app),
        "HOME": "/tmp",  # nosec B108 - tmpfs inside the sandbox, not a host path
        "LANG": "C.UTF-8",
        "PYTHONUNBUFFERED": "1",
