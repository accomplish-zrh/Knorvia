...
Documents are turned into a MinerU-style ``content_list`` by Knorvia's shared
parse layer (``knorvia/services/parsing``) — the same bridge the question
...
from knorvia.services.rag.kb_paths import resolve_kb_dir
...
class LightRagPipeline:
    ...
    def _ensure_available(self) -> None:
        if not lr_config.is_lightrag_available():
            raise lr_config.LightRagNotAvailableError(
                ...
                "`pip install 'knorvia[rag-lightrag]'` to use LightRAG knowledge bases."
            )
    ...
    async def _ingest(
        ...
        *,
        ...
        """
        from knorvia.services.parsing import ParserError, get_parse_service
    ...
    async def _run_indexing(
        ...
        Knorvia-owned network calls and progress callbacks cross back through
