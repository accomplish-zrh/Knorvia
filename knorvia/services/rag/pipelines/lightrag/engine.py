...
fix. All imports are lazy so Knorvia runs fine without the optional dependency
installed.

A RAG-Anything instance is built from Knorvia's LLM/vision/embedding adapters
...
def build_rag(working_dir: Path, *, io_bridge: OwnerLoopBridge | None = None) -> Any:
    ...
    # Knorvia always feeds RAG-Anything a pre-parsed ``content_list`` (the
    # parse layer runs upstream via Knorvia's own ParseService), so
