...
KB only drops Knorvia's pointer (handled by the manager) and must never touch
...
from knorvia.services.rag.provider_binding import load_kb_config_entry
...
class ImaPipeline:
    ...
    async def initialize(self, kb_name: str, file_paths: List[str], **kwargs) -> bool:
        raise RuntimeError(
            "Tencent IMA knowledge bases are indexed by IMA; Knorvia does not "
...
def _sources_from_items(items: list[dict[str, Any]]) -> list[dict[str, Any]]:
    """Map IMA search items into Knorvia's ``sources`` shape.
