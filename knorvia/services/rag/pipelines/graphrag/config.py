"""Bridge Knorvia's runtime config into a GraphRAG ``settings.yaml``.
...
Knorvia's already-resolved LLM + embedding runtime config and let
...
* GraphRAG talks to its models via LiteLLM with ``model_provider: openai`` + a
  custom ``api_base`` — which is exactly how Knorvia reaches any
...
class GraphRagNotConfiguredError(RuntimeError):
    """Raised when Knorvia's LLM / embedding config can't back GraphRAG."""
...
def is_graphrag_available() -> bool:
    ...
    extra: ``pip install 'knorvia[graphrag]'``. Until it is installed the
...
def query_config_from_settings() -> GraphRagQueryConfig:
    ...
    try:
        from knorvia.services.config import load_graphrag_settings
...
def build_settings(*, llm_cfg: Any = None, embedding_cfg: Any = None) -> dict[str, Any]:
    """Assemble the GraphRAG ``settings.yaml`` payload from Knorvia config.

    ``llm_cfg`` / ``embedding_cfg`` are injectable for tests; in production they
    are resolved from Knorvia's catalog. Raises
    ...
    return {
        ...
        # Plain-text input: Knorvia's ingestion writes parsed ``.txt`` files
        ...
        # GraphRAG/LanceDB defaults to 3072 dimensions; Knorvia must stamp the
