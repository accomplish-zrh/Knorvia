...
(its optional ``markitdown`` reader is just a converter). So Knorvia owns the
"document → text" step and hands GraphRAG ready ``.txt`` files. We deliberately
reuse Knorvia's existing extraction primitives here so multimodal/parsing
stays a Knorvia-side concern (matching how the LlamaIndex pipeline already
