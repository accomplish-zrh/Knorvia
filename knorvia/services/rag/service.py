...
# Strong references to in-flight raw-log emit tasks. asyncio only keeps weak
# references to tasks, so an unreferenced task can be garbage-collected
# mid-execution and silently drop log events.
_pending_raw_log_tasks: set[asyncio.Future] = set()
...
class RAGService:
    ...
    Knorvia's authoritative KB config, with metadata as a legacy fallback.
    """
    ...
    def _resolve_provider(self, kb_name: Optional[str]) -> str:
        """Pick the provider for ``kb_name`` from Knorvia's binding."""
    ...
    def _capture_raw_logs(self, event_sink):
        ...
        from knorvia.logging import ProcessLogEvent, capture_process_logs
        from knorvia.logging.formatters import ContextFilter
        ...
        def should_skip_noisy_retrieve_log(event: ProcessLogEvent) -> bool:
            ...
            if logger_name == "nano-vectordb" and (
                ...
                return True
            if (
                logger_name.startswith("knorvia.services.embedding.")
                and (
        ...
        class _NamedRawLogHandler(logging.Handler):
            ...
            def emit(self, record: logging.LogRecord) -> None:
                try:
                    ...
                    _pending_raw_log_tasks.add(task)
                    task.add_done_callback(_pending_raw_log_tasks.discard)
                except Exception:
