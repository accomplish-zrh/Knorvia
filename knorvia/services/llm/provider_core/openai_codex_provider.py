"""OpenAI Codex Responses provider backed by Knorvia's private OAuth service."""
...
from knorvia.services.codex_auth import CodexAuthError, get_codex_oauth_service
from knorvia.services.codex_auth.constants import CODEX_DEFAULT_MODEL_ID, CODEX_RESPONSES_URL
from knorvia.services.codex_auth.contracts import CodexToken
from knorvia.services.llm.openai_http_client import disable_ssl_verify_enabled
from knorvia.services.llm.provider_core.base import LLMProvider, LLMResponse, ToolCallRequest
from knorvia.services.llm.provider_core.openai_responses import (
    consume_sse,
    convert_messages,
    convert_tools,
)

DEFAULT_ORIGINATOR = "Knorvia"
...
def _build_headers(account_id: str, token: str) -> dict[str, str]:
    if not token:
        raise RuntimeError(
            "OpenAI Codex is not logged in. Run `knorvia provider login openai-codex`."
        )
    headers = {
        "Authorization": f"Bearer {token}",
        "OpenAI-Beta": "responses=experimental",
        "originator": DEFAULT_ORIGINATOR,
        "User-Agent": "Knorvia (python)",
