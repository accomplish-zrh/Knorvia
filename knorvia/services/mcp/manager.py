...
Knorvia's chat runs as per-turn tasks inside one event loop, while MCP
...
from knorvia.services.mcp.pageindex_server import with_builtin_servers
...
def _consume_future_result(future: "asyncio.Future[Any]") -> None:
    """Retrieve a detached future's terminal exception to keep the loop quiet."""
...
class MCPConnectionManager:
    ...
    async def _call_watching_connection(
        ...
        """
        call = asyncio.ensure_future(self._call_once(conn, tool_name, arguments, on_progress))
        watcher = conn.task
        watched = {call}
    ...
    async def _abandon(call: "asyncio.Task[str]", conn: _ServerConnection) -> None:
        ...
        """
        call.cancel()
        done, pending = await asyncio.wait({call}, timeout=_TASK_CANCEL_GRACE_S)
        if pending:
            logger.warning("MCP call for %r ignored cancellation; detaching task", conn.name)
            call.add_done_callback(_consume_future_result)
            return
        for finished in done:
            _consume_future_result(finished)
    ...
    def _signature(cls, cfg: MCPServerConfig, owner: str) -> str:
        ...
        :mod:`knorvia.services.mcp.secrets`). Rotating a credential therefore
    ...
    async def _connect(
        ...
        *,
        ...
        try:
            ...
            await self._cancel_connection_task(conn)
    ...
    def _materialize(cfg: MCPServerConfig, owner: str) -> MCPServerConfig:
        ...
        one transport open. See :mod:`knorvia.services.mcp.secrets` for why the
        stored config holds references instead.
        """
        from knorvia.services.mcp.secrets import resolve_references, resolve_url_references
    ...
    async def _open_transport(
        ...
        *,
        ...
        if self_service:
            ...
            from knorvia.services.mcp.network import validate_mcp_url_async
        ...
        oauth_auth = None
        if cfg.auth == "oauth":
            from knorvia.services.mcp.oauth import build_auth, oauth_redirect_uri
    ...
    async def _disconnect(self, conn: _ServerConnection) -> None:
        ...
        if conn.task is not None:
            done, pending = await asyncio.wait({conn.task}, timeout=10)
            if pending:
                await self._cancel_connection_task(conn)
            for task in done:
                with contextlib.suppress(asyncio.CancelledError, Exception):
    ...
    @staticmethod
    async def _cancel_connection_task(conn: _ServerConnection) -> None:
        task = conn.task
        if task is None:
            return
        if task.done():
            ...
            return
        task.cancel()
        done, pending = await asyncio.wait({task}, timeout=_TASK_CANCEL_GRACE_S)
        if pending:
            logger.warning("MCP server %r ignored cancellation; detaching task", conn.name)
...
def _needs_authorization(exc: BaseException) -> bool:
    ...
    """
    from knorvia.services.mcp.oauth import AuthorizationRequired
