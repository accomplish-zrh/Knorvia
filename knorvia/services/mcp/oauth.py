...
sibling module :mod:`knorvia.services.mcp.secrets` holds static credentials in
...
#: How Knorvia introduces itself on a consent screen.
CLIENT_NAME = "Knorvia"
CLIENT_URI = "https://localhost/knorvia"
...
def _store_path(owner_id: str, server: str) -> Path:
    from knorvia.multi_user.paths import owner_secrets_dir
...
_PUBLIC_URL_ENV = "KNORVIA_PUBLIC_URL"
...
def oauth_redirect_uri(origin: str = "") -> str:
    ...
    *origin* is the origin the person is actually browsing, which the route that
    ...
    deployment work with no configuration. ``KNORVIA_PUBLIC_URL`` overrides it
...
def client_metadata(redirect_uri: str) -> "Any":
    """How Knorvia registers itself with an authorization server."""
