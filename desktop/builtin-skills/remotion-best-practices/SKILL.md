...
description: Create and edit videos with React and Remotion; animation, captions, media, Studio preview and rendering. Load the relevant reference on demand.
version: 4.0.522
---

## Knorvia integration

This is an optional video creation skill, separate from Knorvia's existing model-based video studio and article-video workflow. Use it when the user asks for Remotion or programmatic video creation. Work in the current task workspace, inspect existing files first, and preserve user changes. The bundled skill contains guidance and assets, not an installed Remotion renderer. Check project dependencies before running commands. Do not invoke paid image, video, voice or cloud services without user authorization. Use existing Knorvia library tools to save completed artifacts when requested; open local previews in the work panel when that capability is available. Do not replace an existing video workflow automatically.
