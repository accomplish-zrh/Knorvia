...
export interface MessageAttachment {
  ...
  size_bytes?: number;
  /** Image Studio ids so chat can reopen the same project and iterate. */
...
export function UnifiedChatProvider({
  ...
  const retryTimersRef = useRef<
    Map<ReturnType<typeof setTimeout>, string>
  >(new Map());
  const outboundEpochRef = useRef<Map<string, number>>(new Map());
  // Covers the gap between optimistic STREAM_START and the first successful
  // socket send. React state has not necessarily committed during that gap,
  // so a double submit must be rejected synchronously here.
  const pendingStartTurnsRef = useRef<Set<string>>(new Set());
  const branchSelectionDraftRef = useRef<
    Map<string, Record<string, number>>
  >(new Map());
  const branchPersistQueueRef = useRef<Map<string, Promise<void>>>(new Map());
  ...
  useEffect(
    () => () => {
      ...
      retryTimersRef.current.forEach((_key, id) => clearTimeout(id));
      retryTimersRef.current.clear();
      pendingStartTurnsRef.current.clear();
    },
    [],
  );
  ...
  const sendThroughRunner = useCallback(
    (key: string, msg: ChatMessage) => {
      ...
      const epoch = (outboundEpochRef.current.get(key) || 0) + 1;
      outboundEpochRef.current.set(key, epoch);
      const dispatchToRunner = (attempt = 0) => {
        if (outboundEpochRef.current.get(key) !== epoch) {
          releasePendingStart();
          return;
        }
        ...
        if (!runner.client.connected) {
          ...
          const timerId = setTimeout(() => {
            ...
            dispatchToRunner(attempt + 1);
          }, 200);
          retryTimersRef.current.set(timerId, key);
          return;
        }
        try {
          runner.client.send(msg);
          // The reducer's STREAM_START commit will be visible by the next task;
          // retain the synchronous lock until then to absorb double-click/Enter.
          if (startsTurn) window.setTimeout(releasePendingStart, 0);
        } catch (error) {
          releasePendingStart();
          dispatch({ type: "STREAM_END", key, status: "failed" });
  ...
  const loadSession = useCallback(
    async (
      ...
      const selectedBranches = normalizeSelectedBranches(
        session.preferences?.selected_branches,
      );
      if (!branchPersistQueueRef.current.has(key)) {
        branchSelectionDraftRef.current.set(key, selectedBranches);
      }
      dispatch({
    ...
    const timer = setInterval(() => {
      ...
      for (const [key, session] of Object.entries(current.sessions)) {
        ...
        if (runner) {
          ...
          window.setTimeout(() => runner.client.disconnect(), 100);
          runnersRef.current.delete(key);
        }
        outboundEpochRef.current.set(
          key,
          (outboundEpochRef.current.get(key) || 0) + 1,
        );
        retryTimersRef.current.forEach((timerKey, timerId) => {
  ...
  const cancelStreamingTurn = useCallback(() => {
    ...
    pendingStartTurnsRef.current.delete(key);
    if (runner) {
    ...
    async (messageId: number, newContent: string) => {
      ...
      try {
        original = await resolvePersistedMessage(
          ...
          async () =>
            session.sessionId
              ? await loadSession(session.sessionId, { revalidate: true })
              : undefined,
        );
      } catch {
        return;
      ...
      if (
        ...
        stateRef.current.sessions[key]?.isStreaming
      ) {
        return;
      }
  ...
  const switchBranch = useCallback(
    (parentMessageId: number | null, childId: number) => {
      ...
      const nextSelections = {
        ...(branchSelectionDraftRef.current.get(sessionId) ||
          session.selectedBranches),
        [parentKey]: childId,
      };
      branchSelectionDraftRef.current.set(sessionId, nextSelections);
      // Serialize writes per session so a slower older PATCH cannot overwrite
      // the user's newest branch choice on the server.
      const previous = branchPersistQueueRef.current.get(sessionId) ||
        Promise.resolve();
      const queued = previous
        .catch(() => undefined)
        .then(() => updateBranchSelection(sessionId, nextSelections));
      branchPersistQueueRef.current.set(sessionId, queued);
      queued
        .catch((err) => {
          console.warn("Failed to persist branch selection:", err);
        })
        .finally(() => {
          if (branchPersistQueueRef.current.get(sessionId) === queued) {
            branchPersistQueueRef.current.delete(sessionId);
          }
        });
    },
    [],
