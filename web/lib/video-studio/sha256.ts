...
const FILE_HASH_CHUNK_BYTES = 4 * 1024 * 1024
...
export class IncrementalSha256 {
  ...
  update(input: Uint8Array): this {
    ...
    while (offset < input.byteLength) {
      const length = Math.min(SHA256_BLOCK_BYTES - this.blockLength, input.byteLength - offset)
...
export async function hashFileSha256(
  ...
  for (let offset = 0; offset < file.size; offset += FILE_HASH_CHUNK_BYTES) {
    ...
    const bytes = new Uint8Array(
      await file.slice(offset, Math.min(file.size, offset + FILE_HASH_CHUNK_BYTES)).arrayBuffer()
    )
    hash.update(bytes)
    onProgress?.(Math.min(1, (offset + bytes.byteLength) / file.size))
    // Yield between CPU-heavy blocks so large video hashing does not freeze
    // progress painting, cancellation, or the rest of the workbench UI.
