...
export class UnifiedWSClient {
  ...
  private removeDesktopListener: (() => void) | null = null;
  ...
  connect(): void {
    ...
    if (desktop) {
      if (this.desktopConnected || this.desktopSocketId) return;
      this.intentionalClose = false;
      const socketId = crypto.randomUUID();
      this.desktopSocketId = socketId;
      this.removeDesktopListener = desktop.onWsEvent(socketId, (event) => {
        if (event.type === "open") {
          this.desktopConnected = true;
          this.reconnectAttempt = 0;
          this.lastReceivedAt = Date.now();
          if (this.activeTurnId) {
            this.send({ type: "resume_from", turn_id: this.activeTurnId, seq: this.lastSeq });
          }
          ...
          this.removeDesktopListener = null;
          if (!this.intentionalClose) this.attemptReconnect();
        }
      });
      desktop.wsOpen(socketId, "/api/v1/ws");
      return;
    }
  ...
  private handleMessage(data: string): void {
      ...
      try {
        ...
        console.warn("Unparseable WS message:", data);
      }
  }

  send(msg: ChatMessage): void {
    if (this.desktopConnected && this.desktopSocketId && window.knorviaDesktop) {
      window.knorviaDesktop.wsSend(this.desktopSocketId, JSON.stringify(msg));
      return;
    }
  ...
  disconnect(): void {
    ...
    this.ws?.close();
    this.ws = null;
    if (this.desktopSocketId && typeof window !== "undefined" && window.knorviaDesktop) {
      window.knorviaDesktop.wsClose(this.desktopSocketId);
    }
    this.desktopSocketId = null;
    this.desktopConnected = false;
    this.removeDesktopListener?.();
  ...
  get connected(): boolean {
    return this.desktopConnected || this.ws?.readyState === WebSocket.OPEN;
  }
