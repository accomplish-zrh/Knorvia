 ...
 * `KNORVIA_API_BASE_URL` (exported by the container entrypoint from
...
/**
 * Parse a "KNORVIA_AUTH_ENABLED"-style flag at runtime.
 *
...
// The browser bundle never sees `KNORVIA_AUTH_ENABLED` (it isn't a
...
export async function apiFetch(
  ...
  if (desktop) {
    const request = new Request(input, { credentials: "include", ...fetchInit });
    ...
    const reply = await desktop.fetch({
      method: request.method,
      path: `${url.pathname}${url.search}`,
      headers: Object.fromEntries(request.headers.entries()),
      body: bytes.length ? btoa(binary) : undefined,
    });
    const decoded = atob(reply.body || "");
    const body = Uint8Array.from(decoded, (char) => char.charCodeAt(0));
