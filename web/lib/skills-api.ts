 ...
 * `ref` is a `<hub>:<slug>[@version]` reference — the ClawHub import flow always
 * builds an `clawhub:` ref. Surfaces the hub's security verdict so callers can
...
// ── ClawHub / hub browsing ───────────────────────────────────────────────
// Powers the in-app "Import from ClawHub" browser. The backend proxies the
// hub's public catalog (no login, no iframe), so the panel can render hub
// skills in Knorvia's own UI and download them with one click.
...
export interface HubCatalog {
  hub: string;
  /** The hub's website origin (for a "view on ClawHub" link out). */
  webUrl: string;
...
/** List skills available on a hub (default ClawHub). `query` filters server-side. */
