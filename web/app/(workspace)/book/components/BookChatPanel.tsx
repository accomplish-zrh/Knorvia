...
export default function BookChatPanel({
  ...
  useEffect(() => {
    const raw = window.localStorage.getItem("knorvia.bookChat.width");
  ...
  useEffect(() => {
    window.localStorage.setItem("knorvia.bookChat.width", String(width));
  }, [width]);
  ...
  // Key of the backing page this panel is currently reset against. The
  // backend answers a first send with a `session` event before any content;
  // `onSessionResolved` echoes that id back through `initialSessionId`, which
  // must NOT tear down the live connection streaming the first reply.
  const resetKeyRef = useRef<string>("");

  useEffect(() => {
    const resetKey = `${book?.id ?? ""}:${page?.id ?? ""}:${open ? "open" : "closed"}`;
    if (resetKey === resetKeyRef.current) {
      // Only the resolved session id changed (null → real id from the
      // backend's `session` event). Adopt it without disconnecting or
      // clearing messages; the turn that is streaming owns the socket.
