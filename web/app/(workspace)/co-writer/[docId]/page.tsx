...
const SPLIT_RATIO_KEY = "knorvia.co_writer.split_ratio";
const SYNC_SCROLL_KEY = "knorvia.co_writer.sync_scroll";
const LOCAL_DRAFT_PREFIX = "knorvia.co_writer.draft.";
...
export default function CoWriterPage() {
  ...
  // Monotonic sequence for autosave responses: only the response of the most
  // recently *started* save may update saved-state, so a slow older response
  // can neither overwrite newer content nor delete the newer local draft.
  ...
  useEffect(() => {
    ...
    autosaveTimerRef.current = setTimeout(async () => {
      ...
      try {
        ...
        if (isUnmountedRef.current || seq !== saveSeqRef.current) return;
        lastSavedContentRef.current = snapshot;
        setDocTitle(updated.title || "");
        setLastSavedAt(
          updated.updated_at ? updated.updated_at * 1000 : Date.now(),
        );
        // Only drop the local mirror when it still holds exactly what we just
        // saved — if the user kept typing, the mirror contains newer content.
        if (draftStorageKey && snapshot === markdownRef.current) {
          try {
        ...
        if (!isUnmountedRef.current && seq === saveSeqRef.current) {
