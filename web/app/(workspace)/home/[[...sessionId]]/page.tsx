...
export default function ChatPage() {
  ...
  const pendingLibraryRef = useRef<string | null | undefined>(undefined);
  if (pendingLibraryRef.current === undefined) {
  ...
  // How many times Knorvia may consult the selected agent this turn. Seeded
  ...
  const handleSend = useCallback(
    async (content: string) => {
      ...
      // (how many times Knorvia may ask it) so the subagent capability uses it.
  ...
  useEffect(() => {
    const id = pendingLibraryRef.current;
    if (!id) return;
    let tries = 0;
    const tick = window.setInterval(() => {
      ...
      if (prefillInputRef.current) {
        prefillInputRef.current(
          t("Use library entry {{id}} and help me create from it.", { id }),
        );
