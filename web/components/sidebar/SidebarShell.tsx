...
const PRIMARY_LEADING: NavEntry[] = [
  {
    href: "/home",
    label: "Home",
    icon: House,
...
const CREATION_DESK_CHILDREN: NavEntry[] = [
  {
    href: "/co-writer",
    label: "Co-Writer",
    icon: PenLine,
  ...
  },
  {
    ...
    label: "Image Studio",
    icon: Images,
    tooltipKey: "Image Studio tooltip",
    requires: "imagegen",
  },
  {
    href: "/video-studio",
    label: "Video Studio",
    icon: Clapperboard,
    tooltipKey: "Video Studio tooltip",
    requires: "videogen",
  },
];

const PRIMARY_TRAILING: NavEntry[] = [
  {
    href: "/partners",
    label: "Partners",
    icon: HeartHandshake,
...
const RECENTS_COLLAPSED_KEY = "knorvia.sidebar.recentsCollapsed";
...
export function SidebarShell({
  ...
  const deskPath = creationDeskNavState(pathname);
  const [deskOpen, setDeskOpen] = useState(deskPath.open);
  const effectiveDeskOpen = deskPath.open || deskOpen;
  ...
  const renderExpandedItem = (item: NavEntry) => {
  ...
  const renderCollapsedItem = (item: NavEntry) => {
    const active = pathname.startsWith(item.href);
    const locked = navLocked(item);
    ...
    return (
      <aside className="group/sb relative flex h-dvh w-[60px] shrink-0 flex-col items-center bg-[var(--secondary)] py-3 transition-all duration-200">
        ...
        <nav className="mt-1 flex w-full flex-col items-center gap-1 px-1.5">
          {PRIMARY_LEADING.map((item) => renderCollapsedItem(item))}
          <div className="flex w-full flex-col items-center" data-creation-desk="">
            <Tooltip
              label={t("Creation desk")}
              description={t("Creation desk tooltip")}
              side="right"
            >
              <button
                type="button"
                aria-label={t("Creation desk")}
                aria-expanded={effectiveDeskOpen}
              ...
              >
                <Wand2 size={18} strokeWidth={deskPath.child ? 2 : 1.6} />
              </button>
            </Tooltip>
            <div
              className={`grid w-full transition-[grid-template-rows] duration-200 ease-out ${
                effectiveDeskOpen ? "grid-rows-[1fr]" : "grid-rows-[0fr]"
              }`}
            >
              <div className="overflow-hidden">
                <div
                  data-creation-desk-children=""
                  className="mt-1 flex flex-col items-center gap-1"
                >
                  {CREATION_DESK_CHILDREN.map((item) => {
                    const active = pathname.startsWith(item.href);
                    const locked = navLocked(item);
                    if (locked) {
                      return (
                        ...
                        >
                          <div
                            data-creation-desk-child={item.href}
                            aria-disabled
                            className="relative flex h-9 w-9 cursor-not-allowed items-center justify-center rounded-xl text-[var(--muted-foreground)]/40"
                          >
                            <item.icon size={18} strokeWidth={1.6} />
          ...
          {PRIMARY_TRAILING.map((item) => renderCollapsedItem(item))}
        </nav>
    ...
    <aside className="flex w-[220px] h-dvh shrink-0 flex-col bg-[var(--secondary)] transition-all duration-200">
      ...
      <nav className="px-2 pt-1">
        <div className="space-y-px">
          {PRIMARY_LEADING.map((item) => renderExpandedItem(item))}
          <div data-creation-desk="">
            <button
              type="button"
              aria-expanded={effectiveDeskOpen}
              aria-label={t("Creation desk")}
            ...
            >
              <Wand2 size={16} strokeWidth={deskPath.child ? 1.9 : 1.5} />
              <span className="min-w-0 flex-1 text-left">{t("Creation desk")}</span>
              <ChevronDown
                size={14}
                strokeWidth={1.7}
                className={`shrink-0 text-[var(--muted-foreground)] transition-transform ${
                  effectiveDeskOpen ? "rotate-0" : "-rotate-90"
                }`}
              />
            </button>
            <div
            ...
            >
              <div className="overflow-hidden">
                <div data-creation-desk-children="" className="mt-px ml-4 space-y-px border-l border-[var(--border)]/50 pl-2">
                  {CREATION_DESK_CHILDREN.map((item) => {
                    const active = pathname.startsWith(item.href);
                    const locked = navLocked(item);
                    if (locked) {
                      return (
                        ...
                        >
                          <div
                            data-creation-desk-child={item.href}
                            aria-disabled
                            className="flex cursor-not-allowed items-center gap-2 rounded-lg px-2.5 py-1.5 text-[12.5px] text-[var(--muted-foreground)]/40"
                          >
                            <item.icon size={15} strokeWidth={1.5} />
                            <span>{t(item.label)}</span>
                            <Lock size={12} strokeWidth={1.8} className="ml-auto" />
                          </div>
                        </Tooltip>
                      );
                    }
          ...
          {PRIMARY_TRAILING.map((item) => renderExpandedItem(item))}
        </div>
      </nav>
        ...
        >
          ...
          {!recentsCollapsed && (
            <div className="min-h-0 flex-1 overflow-y-auto px-2 pb-2 pt-0.5">
              {sessionsError ? (
                <div className="px-1 py-2 text-[11px] text-[var(--destructive)]">
                  <p>{sessionsError}</p>
                  {onRetrySessions ? (
                    <button
                      type="button"
                      onClick={onRetrySessions}
                      className="mt-1 rounded-md border border-[var(--border)] px-2 py-0.5 text-[var(--foreground)]"
                    >
                      {t("Retry")}
                    </button>
                  ) : null}
