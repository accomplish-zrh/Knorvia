...
import type { MessageAttachment, MessageRequestSnapshot } from '@/context/UnifiedChatContext'
...
/** "Knorvia_Introduction.pdf" → "Knorvia Introduction" — the card title
                ...
                <span className="flex shrink-0 items-center gap-2 text-[11px]">
                  ...
                  {a.url ? (
                    <a
                      href={
                        a.studio_project_id
                          ? `/image-studio?project=${encodeURIComponent(String(a.studio_project_id))}&source=${encodeURIComponent(a.url)}`
                          : `/image-studio?source=${encodeURIComponent(a.url)}`
                      }
                      className="flex items-center gap-1 rounded-md border border-[var(--border)] px-2 py-1 text-[var(--foreground)]"
                    >
                      <Images size={12} />
...
const AssistantMessage = memo(function AssistantMessage({
  ...
  const researchInProgress = outlineStatus === 'researching' || outlineStatus === 'done'
  ...
  return (
    <>
      {/* Activity block pinned to the TOP: the status header
          ("Knorvia Exploring… · 8s" → "Knorvia responded. · 10s") with
          the exploring trace nested beneath it — expanded while Knorvia is
...
function PlayAudioButton({
  ...
  const play = useCallback(async () => {
    ...
    try {
      ...
      if (controller.signal.aborted || playEpochRef.current !== epoch) return
      if (!resp.ok) {
        cleanup()
        setState('idle')
        return
      ...
      await audio.play()
      if (controller.signal.aborted || playEpochRef.current !== epoch) {
        cleanup()
        return
      }
      setState('playing')
  ...
  }, [cancelPlayback, markPrompted, play, shouldPromptOnFirstPlay, state])
