...
type TraceMetadata = {
  ...
  // Image Studio job the tool just queued — chat renders the same card
...
function getTraceHeader(
  ...
  if (
    ...
    // top-level status row carries the verbose "Knorvia Reflecting…"
 ...
 *  outside Knorvia that the turn is talking to. */
...
function detectStreamingMode(
  ...
  for (let idx = events.length - 1; idx >= 0; idx -= 1) {
    ...
    if (event.type === "content" && callKind === "llm_final_response") {
      ...
      // still open; keep the top-level title on "Knorvia Exploring…" until
      // the bus moves on.
      if (event.stage === "exploring") return "exploring";
...
export function StreamingStatus({
  ...
  const name = agentName?.trim() || "Knorvia";
  ...
  else if (mode === "responded") modeLabel = t("Knorvia responded.");
 ...
 * reasoning trace once Knorvia stops working and starts (or has finished)
 * its answer.
 *
 ...
 * ("Knorvia Exploring… · 8s", settling to "Knorvia responded. · 10s")
 * with the exploring trace nested directly beneath it.
 *
 * The trace is expanded by default while Knorvia is still reasoning /
...
export function AssistantActivity({
  ...
  if (!isStreaming && !hasFinalContent && !hasTrace && !hasTaskCards)
    return null;
