...
import { PreviewBody } from "@/components/chat/preview/PreviewBody";
import { usePreviewRevision } from "@/components/chat/preview/usePreviewRevision";
import { withPreviewRevision } from "@/lib/preview-revision";
import {
  ActivityBody,
...
function FileTabBody({ source }: { source: FilePreviewSource }) {
  const { t } = useTranslation();
  const sourceUrl = useMemo(() => resolveSourceUrl(source, apiUrl), [source]);
  const revision = usePreviewRevision(sourceUrl, Boolean(sourceUrl));
  const previewUrl = withPreviewRevision(sourceUrl, revision);
  ...
  const fileUrl = useMemo(() => {
    ...
    return null;
  }, [sourceUrl, source.base64, source.mimeType]);
