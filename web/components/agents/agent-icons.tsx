...
// Grok Build: black tile with a white spark, so it reads as itself next to
// the other CLI marks without pretending to be an official lockup.
export function GrokBuildGlyph({ size = 16, ...props }: GlyphProps) {
  return (
    <svg
      width={size}
      height={size}
    ...
    >
      ...
      <path
        d="M12 4.6 13.2 9l4.6 1.2L13.2 11.4 12 15.8 10.8 11.4 6.2 10.2 10.8 9z"
        fill="#fff"
      />
    </svg>
  );
...
export function agentGlyph(kind: string | undefined): AgentGlyph | null {
  ...
  if (kind === "grok_build") return GrokBuildGlyph;
