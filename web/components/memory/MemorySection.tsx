        ...
        <div className="relative flex items-start gap-2 rounded-xl border border-[var(--border)] bg-[var(--muted)] px-4 py-3 pr-10 text-[13px]">
          ...
          <div>
            ...
            <p className="mt-0.5 text-[var(--muted-foreground)]">
              {t(
                "Stored at memory/backup/{{name}}. v2 starts fresh — interact with Knorvia and click Update on each doc to build memory.",
                { name: latestBackup },
              )}
            </p>
          </div>
        ...
        {stages.map((s, i) => (
          ...
          >
            ...
            {typeof s.ops_emitted === "number" && (
              <div className="mt-1 text-[var(--muted-foreground)]">
                {t("ops_emitted")}={s.ops_emitted} · {t("turns")}={s.turns_used ?? "?"}
                {s.tools_used
            ...
            {s.report && (
              <div className="mt-1 text-[var(--muted-foreground)]">
                {t("accepted")}={String(s.report.accepted)}
