...
import type { SshRead, SshSession } from '@/lib/native-ssh';
...
export function SshTerminalPanel({ sessionId, active, onClose, onNew }: {
  sessionId: string; active: boolean; onClose: () => void; onNew: () => void;
}) {
  ...
  const [session, setSession] = useState<SshSession>();
  ...
  useEffect(() => {
    ...
    const flush = async () => {
      ...
      try {
        while (queued && ready && !disposed) {
          ...
          await request('ssh/write', { ...scope, data, seq: ++seq });
        }
    ...
    const init = async () => {
      ...
      const opened = await request<SshSession>('ssh/read', { ...scope, cursor: 0 });
      if (disposed) return;
      ...
      const resize = () => {
        ...
        resizeTimer = setTimeout(() => { if (term && !disposed && ready) void request('ssh/resize', { ...scope, cols: term.cols, rows: term.rows }).catch(stopInput); }, 80);
      };
      observer = new ResizeObserver(resize); observer.observe(host.current!); resize();
      const poll = async () => {
        try {
          const value = await request<SshRead>('ssh/read', { ...scope, cursor });
          ...
          if (!value.hasMore) { ready = value.status === 'ready'; term.options.disableStdin = !ready; }
          if (['connecting', 'ready'].includes(value.status) || value.hasMore) timer = setTimeout(() => void poll(), value.hasMore ? 0 : activeRef.current ? 80 : 400);
        } catch { stopInput(); }
      };
      await poll();
  ...
  return <section className="nw-terminal" aria-label={t('终端会话', 'Terminal session')} data-session-id={sessionId} data-status={failure ? 'disconnected' : session?.status ?? 'connecting'}>
    <header className="nw-terminal-toolbar"><TerminalSquare size={15} /><span title={session?.cwd}>{session?.name ?? t('终端', 'Terminal')}<small>{session?.cwd ?? t('正在连接…', 'Connecting…')}</small></span>
      ...
      <button className="nw-icon" disabled={failure || session?.status !== 'ready'} onClick={() => { input.current('\x03'); terminal.current?.focus(); }} title={t('中断命令 · Ctrl+C', 'Interrupt command · Ctrl+C')} aria-label={t('中断命令', 'Interrupt command')}><Square size={12} /></button>
    ...
    {failure && <div className="nw-terminal-notice" role="alert">{t('终端连接中断，未确认的输入不会自动重发。', 'Terminal disconnected. Unconfirmed input will not be replayed.')}<button onClick={() => setAttempt(value => value + 1)}><RefreshCw size={13} />{t('重新读取会话', 'Read session again')}</button></div>}
    {!failure && session && ['failed', 'closed', 'disconnected'].includes(session.status) && <div className="nw-terminal-notice" role="status">{t('SSH 连接已结束。重新连接会打开新会话。', 'The SSH connection ended. Reconnecting starts a fresh session.')}<button onClick={onNew}>{t('重新连接', 'Reconnect')}</button></div>}
