...
import { FolderOpen, Network, Server, TerminalSquare } from 'lucide-react';
...
export function RemoteWorkspace({ active = true, threadId, initialSessionId }: { active?: boolean; threadId?: string; initialSessionId?: string }) {
  ...
  const [view, setView] = useState<'terminal' | 'files' | 'forwards'>('terminal');
  return <div className="nw-remote-workspace">
    {sessionId ? <><div className="nw-usage-toolbar" role="group" aria-label={t('远程工作空间', 'Remote workspace')}><button className="nw-button" onClick={() => setSessionId(undefined)}><Server size={14} />{t('主机', 'Hosts')}</button><button className="nw-button" aria-pressed={view === 'terminal'} onClick={() => setView('terminal')}><TerminalSquare size={14} />{t('终端', 'Terminal')}</button><button className="nw-button" aria-pressed={view === 'files'} onClick={() => setView('files')}><FolderOpen size={14} />{t('文件', 'Files')}</button><button className="nw-button" aria-pressed={view === 'forwards'} onClick={() => setView('forwards')}><Network size={14} />{t('转发', 'Forwards')}</button></div>
      ...
      {view === 'forwards' && <SshForwardsPanel sessionId={sessionId} />}
