...
export function SideChatPanel({ parent, tabId, existingThreadId, onThreadCreated }: { parent: ThreadSnapshot; tabId: string; existingThreadId?: string; onThreadCreated: (id: string) => void }) {
  const { t, request, readThread, sendTurn, snapshots, connection, setError, pinThread } = useWorkbench();
  ...
  // B12: the side chat's session stays pinned in the bounded cache while mounted.
  useEffect(() => { if (!threadId) return; return pinThread(threadId); }, [threadId, pinThread]);
  ...
  return <section className="nw-side-chat" aria-label={t('侧边聊天', 'Side chat')}>
    ...
    {threadId && !thread ? <button className="nw-button" onClick={() => void readThread(threadId).catch(error => setError(errorText(error)))}>{t('重新读取侧边聊天', 'Reload side conversation')}</button> : <TaskComposer allowGoal={false} thread={thread} projectId={parent.workspaceId} draftScope={`side:${parent.id}:${tabId}`} onCreate={async (input, options) => {
