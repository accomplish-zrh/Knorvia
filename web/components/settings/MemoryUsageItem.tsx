 ...
 *
 * Backed by `/api/v1/system/memory` (knorvia/runtime/memory_probe.py), which
 * walks the whole Knorvia process tree — backend, the Next.js server, and
    ...
    <Fragment>
      ...
      <div className="flex flex-wrap items-baseline gap-x-3 gap-y-1.5 text-[12px] leading-none text-[var(--muted-foreground)]">
        ...
        {usage.partial && (
          <span
            title={t(
              "Only the backend and its own children. Start Knorvia through the launcher to include the web server.",
            )}
          >
            {t("(backend only)")}
          </span>
