...
const DIAGNOSTICS_RESULTS_KEY = 'knorvia.settings.diagnosticsResults.v1'
...
function nextModelName(models: CatalogModel[], language: UiSettings['language']): string {
...
/**
 * Parse a JSON response, throwing the backend's `detail` message on non-2xx.
 *
 * `apiFetch` does not reject on HTTP error statuses, so callers that unpack
 * `payload.catalog` directly used to set the catalog to `undefined` on a
 * 400/422/500 — which then crashed the settings page on the next render.
 */
...
export function SettingsProvider({ children }: { children: ReactNode }) {
  ...
  const registerExtension = useCallback((key: string, ext: SettingsExtension | null) => {
    ...
    if (ext === null) {
      ...
      map.delete(key)
      setHasDirtyExtension(Array.from(map.values()).some(extension => extension.dirty))
      return
    }
  ...
  const saveCatalog = useCallback(async () => {
    ...
    try {
      ...
      const payload = await jsonOrThrow<{ catalog: Catalog }>(response)
      setCatalog(payload.catalog)
      setDraft(cloneCatalog(payload.catalog))
  ...
  const applyCatalog = useCallback(async () => {
    ...
    try {
      ...
      if (catalogEditable) {
        ...
        invalidateLLMOptionsCache()
        const statusResponse = await apiFetch(apiUrl('/api/v1/system/status'))
        setStatus(await jsonOrThrow<SystemStatus>(statusResponse))
      }
  ...
  useEffect(() => {
    setLlmContextDetection(current => {
      ...
      if (current.profileId === llmActiveProfileId && current.modelId === llmActiveModelId) {
        return current
      }
      return null
    })
