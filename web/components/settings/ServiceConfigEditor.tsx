...
import { VIDEO_CONNECTION_TEMPLATES } from '@/lib/settings/video-connection-templates'
...
export function ServiceConfigEditor({ service }: { service: ServiceName }) {
  ...
  const updateImageCapability = (
    field: 'operations' | 'max_inputs' | 'supports_context',
    value: string[] | number | boolean
  ) => {
    mutateCatalog(next => {
      const model = getActiveModel(next, 'imagegen')
      if (!model) return
      model.capabilities = { ...(model.capabilities || {}), [field]: value }
      if (field === 'operations') {
        const operations = value as string[]
        model.capabilities.supports_mask = operations.includes('inpaint')
      }
    })
  }
  ...
  const updateVideoCapability = (
    ...
    mutateCatalog(next => {
      const model = getActiveModel(next, 'videogen')
      if (!model) return
      if (field === 'max_inputs') {
        model.capabilities = {
          ...(model.capabilities || {}),
          max_inputs: value as { image: number; video: number; audio: number; total: number },
        }
        return
      }
      model.capabilities = { ...(model.capabilities || {}), [field]: value }
  ...
  const parseCapabilityList = (value: string): string[] =>
    Array.from(
      new Set(
        value
          .split(',')
  ...
  const parseNumberCapabilityList = (value: string): number[] =>
    Array.from(
      new Set(
        value
          .split(',')
  ...
  const videoInputLimits = (() => {
    const limits = activeModel?.capabilities?.max_inputs
    if (typeof limits === 'number') {
      return { image: limits, video: limits, audio: limits, total: limits }
    }
    return {
      image: Number(limits?.image || 0),
      video: Number(limits?.video || 0),
      audio: Number(limits?.audio || 0),
      total: Number(limits?.total || 0),
    }
  })()

  const updateVideoInputLimit = (kind: 'image' | 'video' | 'audio' | 'total', raw: number) => {
    const value = Math.max(0, Math.min(50, Number(raw) || 0))
    updateVideoCapability('max_inputs', { ...videoInputLimits, [kind]: value })
  }

  type VideoCapabilityPreset = {
    id: string
    label: string
    description: string
    capabilities: NonNullable<CatalogModel['capabilities']>
  }

  const [videoPresets, setVideoPresets] = useState<VideoCapabilityPreset[]>([])
  useEffect(() => {
    if (service !== 'videogen') return
    let cancelled = false
    apiFetch(apiUrl('/api/v1/video-studio/capability-presets'))
      .then(async response => {
        if (!response.ok) return
        const payload = (await response.json()) as { presets?: VideoCapabilityPreset[] }
        if (!cancelled) setVideoPresets(payload.presets || [])
      })
      .catch(() => {})
    return () => {
      cancelled = true
  ...
  const applyVideoPreset = (presetId: string) => {
    ...
    setToast(t('Preset applied: {{label}}', { label: preset.label }))
  }

  // C7 connection templates: fill base URL + protocol and APPEND recommended
  // model rows. Existing model rows are never cleared and the API key stays
  // untouched — credentials are always the user's own.
  const [videoTemplateId, setVideoTemplateId] = useState<string | null>(null)
  const appliedVideoTemplate = videoTemplateId
    ? VIDEO_CONNECTION_TEMPLATES.find(item => item.id === videoTemplateId)
    : undefined
  const videoTemplateGuideZh = i18n.language?.toLowerCase().startsWith('zh')

  const applyVideoConnectionTemplate = (templateId: string) => {
    const template = VIDEO_CONNECTION_TEMPLATES.find(item => item.id === templateId)
    if (!template) return
    mutateCatalog(next => {
      const target = next.services.videogen
      const profile = target.profiles.find(item => item.id === target.active_profile_id)
      if (!profile) return
      profile.base_url = template.baseUrl
      const stamp = Date.now()
      const appended: CatalogModel[] = template.models.map((entry, index) => {
        const preset =
          entry.presetId != null
            ? videoPresets.find(item => item.id === entry.presetId)
            : undefined
        return {
          id: `videogen-model-${stamp}-${index}`,
          name: entry.label,
          model: entry.modelId,
      ...
      profile.models.push(...appended)
      target.active_model_id = appended[0]?.id ?? target.active_model_id
    })
    setVideoTemplateId(template.id)
    setToast(t('Template applied: {{label}}', { label: template.label }))
  }

  const schemaKey = `${activeProfile?.id || 'none'}:${activeModel?.id || 'none'}`
  const [videoSchemaKey, setVideoSchemaKey] = useState(schemaKey)
  const [videoSchemaDraft, setVideoSchemaDraft] = useState(() =>
    JSON.stringify(activeModel?.capabilities?.parameter_schema || {}, null, 2)
  )
  if (videoSchemaKey !== schemaKey) {
    setVideoSchemaKey(schemaKey)
    setVideoSchemaDraft(JSON.stringify(activeModel?.capabilities?.parameter_schema || {}, null, 2))
  }
  ...
  if (lastProfileKey !== profileKey) {
    ...
    // A connection template's auth guide belongs to the profile it was applied
    // to — drop it when the user switches profiles or services.
        ...
        <div className="grid grid-cols-[200px_minmax(0,1fr)] items-start gap-5">
          ...
          <div className="min-w-0 space-y-5">
            <div className="rounded-xl border border-[var(--border)] p-5">
              ...
              {service === 'videogen' && (
                <div className="mb-4 rounded-lg border border-[var(--border)]/60 bg-[var(--muted)]/20 p-3">
                  <div className="flex flex-wrap items-center gap-2">
                    ...
                    <select
                      ...
                      value=""
                      onChange={event => {
                        if (event.target.value) applyVideoConnectionTemplate(event.target.value)
                      }}
                    >
                      <option value="">{t('Apply a connection template…')}</option>
                      {VIDEO_CONNECTION_TEMPLATES.map(template => (
                    ...
                    <span className="text-[11px] text-[var(--muted-foreground)]">
                      {t(
                        'Fills the base URL, protocol and recommended model rows — the API key stays empty for you.'
                      )}
                    </span>
                  </div>
                  {appliedVideoTemplate && (
                    <p className="mt-2 text-[11px] leading-relaxed text-[var(--muted-foreground)]">
                      <span className="font-medium text-[var(--foreground)]/80">
                        {appliedVideoTemplate.label}
                      </span>
                      {' — '}
                      {videoTemplateGuideZh
                        ? appliedVideoTemplate.authGuide.zh
                        : appliedVideoTemplate.authGuide.en}
                    </p>
                  )}
                </div>
              )}
              <ProfileFields
                ...
                onProviderChanged={(provider, previousProvider) => {
                  ...
                  const crossesCodeBuddyBoundary =
                    provider.value === 'codebuddy' || previousProvider === 'codebuddy'
                      ...
                      <>
                        <div>
                          ...
                          <div className="relative">
                            <select
                              className={selectClass}
                              value={activeModel.adapter || ''}
                              onChange={e => updateModelField(service, 'adapter', e.target.value)}
                            >
                              <option value="">{t('Provider default')}</option>
                              <option value="openai_compat">{t('OpenAI Images')}</option>
                              <option value="openai_responses">{t('OpenAI Responses')}</option>
                              <option value="chat_completions">{t('Chat Completions')}</option>
                              <option value="gemini_interactions">
                                {t('Gemini Interactions')}
                              </option>
                            </select>
                            <ChevronDown className="pointer-events-none absolute right-3 top-1/2 h-3.5 w-3.5 -translate-y-1/2 text-[var(--muted-foreground)]" />
                          </div>
                        ...
                        <div>
                          <div className="mb-1.5 text-[12px] text-[var(--muted-foreground)]">
                            {t('Default output resolution')}
                          </div>
                          <div className="relative">
                            <select
                              className={selectClass}
                              value={activeModel.image_size || ''}
                              onChange={e =>
                                updateModelField(service, 'image_size', e.target.value)
                              }
                            >
                              <option value="">{t('Provider default')}</option>
                          ...
                          <p className="mt-1.5 text-[11px] text-[var(--muted-foreground)]">
                            {t('Used by models that expose 1K, 2K, or 4K output levels.')}
                          </p>
                        </div>
                        <div>
                          <div className="mb-1.5 text-[12px] text-[var(--muted-foreground)]">
                        ...
                        <div className="sm:col-span-2">
                          <div className="mb-2 text-[12px] text-[var(--muted-foreground)]">
                            {t('Image model capabilities')}
                          </div>
                          <div className="flex flex-wrap gap-x-5 gap-y-2 rounded-lg border border-[var(--border)] p-3 text-[12px]">
                            {(['generate', 'edit', 'inpaint'] as const).map(operation => {
                              const enabled = (
                                activeModel.capabilities?.operations || ['generate']
                              ).includes(operation)
                              return (
                                <label key={operation} className="flex items-center gap-2">
                                  <input
                                    type="checkbox"
                                    checked={enabled}
                                    disabled={operation === 'generate'}
                                    onChange={event => {
                                      const current = activeModel.capabilities?.operations || [
                                        'generate',
                                      ]
                                      const next = event.target.checked
                                        ? Array.from(new Set([...current, operation]))
                                        : current.filter(item => item !== operation)
                                      updateImageCapability('operations', next)
                                    }}
                                  />
                                  {t(operation)}
                                </label>
                            ...
                            <label className="flex items-center gap-2">
                              <input
                                type="checkbox"
                                checked={activeModel.capabilities?.supports_context === true}
                                onChange={event =>
                                  updateImageCapability('supports_context', event.target.checked)
                                }
                              />
                              {t('Continuous context')}
                            </label>
                            <label className="ml-auto flex items-center gap-2">
                              {t('Maximum reference images')}
                              <input
                                type="number"
                                min={0}
                                max={4}
                                value={
                                  typeof activeModel.capabilities?.max_inputs === 'number'
                                    ? activeModel.capabilities.max_inputs
                                    : 0
                                }
                                onChange={event =>
                          ...
                          <p className="mt-1.5 text-[11px] text-[var(--muted-foreground)]">
                            {t(
                              'These switches control which tools appear in Image Studio and are validated again by the server.'
                            )}
                          </p>
                        </div>
                      </>
                      ...
                      <>
                        <div>
                          ...
                          <div className="relative">
                            ...
                            >
                              <option value="">{t('Provider default')}</option>
                              <option value="openai_videos">{t('OpenAI Videos API')}</option>
                              <option value="volcengine_async_task">
                                {t('Volcengine Seedance API')}
                              </option>
                              <option value="async_task">{t('Asynchronous task API')}</option>
                            </select>
                            <ChevronDown className="pointer-events-none absolute right-3 top-1/2 h-3.5 w-3.5 -translate-y-1/2 text-[var(--muted-foreground)]" />
                          </div>
                        </div>
                        ...
                        <div className="sm:col-span-2">
                          <div className="mb-2 text-[12px] text-[var(--muted-foreground)]">
                            {t('Video model capabilities')}
                          </div>
                          <div className="space-y-3 rounded-lg border border-[var(--border)] p-3 text-[12px]">
                            {videoPresets.length > 0 && (
                              <div className="flex flex-wrap items-center gap-2">
                                ...
                                <select
                                  ...
                                  value=""
                                  onChange={event => {
                                    if (event.target.value) applyVideoPreset(event.target.value)
                                  }}
                                >
                                  <option value="">{t('Apply a preset…')}</option>
                                ...
                                <span className="text-[11px] text-[var(--muted-foreground)]">
                                  {t(
                                    'Fills the fields below; point base URL and model id at your own gateway.'
                                  )}
                                </span>
                              </div>
                            )}
                            <div className="flex flex-wrap gap-x-5 gap-y-2">
                              {(
                                ...
                                const enabled = (
                                  activeModel.capabilities?.operations || ['text_to_video']
                                ).includes(operation)
                                return (
                                  <label key={operation} className="flex items-center gap-2">
                                    <input
                              ...
                              <label className="flex items-center gap-2">
                                <input
                                  type="checkbox"
                                  checked={activeModel.capabilities?.supports_cancel !== false}
                                  onChange={event =>
                                    updateVideoCapability('supports_cancel', event.target.checked)
                                  }
                                />
                                {t('Provider cancellation')}
                              </label>
                              <label className="flex items-center gap-2">
                                <input
                                  type="checkbox"
                                  checked={activeModel.capabilities?.supports_seed === true}
                                  onChange={event =>
                                    updateVideoCapability('supports_seed', event.target.checked)
                                  }
                                />
                                {t('Deterministic seed')}
                              </label>
                            </div>
                            <div className="grid gap-2 sm:grid-cols-3">
                              <input
                                className={inputClass}
                                value={(activeModel.capabilities?.durations || []).join(', ')}
                                onChange={event =>
                                  updateVideoCapability(
                                    'durations',
                                    parseNumberCapabilityList(event.target.value)
                                  )
                                }
                                placeholder={t('Durations: 5, 10')}
                              />
                              <input
                                className={inputClass}
                                value={(activeModel.capabilities?.aspect_ratios || []).join(', ')}
                                onChange={event =>
                                  updateVideoCapability(
                                    'aspect_ratios',
                                    parseCapabilityList(event.target.value)
                                  )
                                }
                                placeholder={t('Ratios: 16:9, 9:16')}
                              />
                              <input
                                className={inputClass}
                                value={(activeModel.capabilities?.resolutions || []).join(', ')}
                                onChange={event =>
                                  updateVideoCapability(
                                    'resolutions',
                                    parseCapabilityList(event.target.value)
                                ...
                                placeholder={t('Resolutions: 720p, 1080p')}
                              />
                            </div>
                            <div className="grid gap-2 sm:grid-cols-3">
                              <input
                                className={inputClass}
                                value={(activeModel.capabilities?.fps || []).join(', ')}
                                onChange={event =>
                                  updateVideoCapability(
                                    'fps',
                                    parseNumberCapabilityList(event.target.value)
                                ...
                                placeholder={t('FPS: 24, 30')}
                              />
                              <input
                                className={inputClass}
                                value={(activeModel.capabilities?.audio_modes || []).join(', ')}
                                onChange={event =>
                                  updateVideoCapability(
                                    'audio_modes',
                                    parseCapabilityList(event.target.value)
                                ...
                                placeholder={t('Audio modes: none, generate')}
                              />
                              <input
                                className={inputClass}
                                value={(activeModel.capabilities?.reference_modes || []).join(', ')}
                                onChange={event =>
                                  updateVideoCapability(
                                    'reference_modes',
                                    parseCapabilityList(event.target.value)
                                ...
                                placeholder={t('Reference modes: first, last')}
                              />
                            </div>
                            <div className="grid gap-2 sm:grid-cols-4">
                              {(['image', 'video', 'audio', 'total'] as const).map(kind => (
                                <label key={kind} className="space-y-1">
                                  <span className="text-[11px] text-[var(--muted-foreground)]">
                                    {t(`Maximum ${kind} inputs`)}
                                  </span>
                                  <input
                                    type="number"
                                    min={0}
                                    max={50}
                                    value={videoInputLimits[kind]}
                                    onChange={event =>
                                      updateVideoInputLimit(kind, Number(event.target.value))
                                    }
                                    className={inputClass}
                                  />
                                </label>
                            ...
                            <div className="grid gap-2 sm:grid-cols-2">
                              <label className="space-y-1">
                                <span className="text-[11px] text-[var(--muted-foreground)]">
                                  {t('Maximum prompt length')}
                                </span>
                                <input
                                  type="number"
                                  min={1}
                                  max={20000}
                                  value={activeModel.capabilities?.max_prompt_length || 20000}
                                  onChange={event =>
                                    updateVideoCapability(
                                      'max_prompt_length',
                                      Math.max(1, Math.min(20000, Number(event.target.value) || 1))
                                    )
                                  }
                                  className={inputClass}
                                />
                              ...
                              <label className="space-y-1">
                                <span className="text-[11px] text-[var(--muted-foreground)]">
                                  {t('Maximum provider input bytes')}
                                </span>
                                <input
                                  type="number"
                                  min={1}
                                  value={activeModel.capabilities?.max_input_bytes || 67108864}
                                  onChange={event =>
                                    updateVideoCapability(
                                      'max_input_bytes',
                            ...
                            <label className="block space-y-1">
                              <span className="text-[11px] text-[var(--muted-foreground)]">
                                {t('Advanced parameter schema (JSON Schema)')}
                              </span>
                              <textarea
                                className={`${inputClass} min-h-32 font-mono text-[11px]`}
                                value={videoSchemaDraft}
                                onChange={event => setVideoSchemaDraft(event.target.value)}
                                onBlur={() => {
                                  try {
                                    const schema = JSON.parse(videoSchemaDraft) as unknown
                          ...
                          <p className="mt-1.5 text-[11px] text-[var(--muted-foreground)]">
                            {t(
                              'Video Studio reads these capabilities from the server catalog and rejects unsupported controls again before submission.'
                            )}
                          </p>
                        </div>
                      </>
...
function formatVoiceBadge(value: string | undefined): string {
  ...
  const tail = voice.includes(':') ? voice.slice(voice.lastIndexOf(':') + 1) : voice
...
function ProfileFields({
  ...
  const { t } = useTranslation()
  const { providers, updateProfileField, updateModelField, draft, mutateCatalog } = useSettings()
  ...
  const activeConnection =
    service === 'imagegen'
      ? connections.find(connection => connection.id === profile.connection_id)
      : undefined
  const connectionBaseUrl = activeConnection?.base_url ?? profile.base_url
  const connectionApiKey = activeConnection?.api_key ?? profile.api_key
  const connectionApiVersion = activeConnection?.api_version ?? profile.api_version
  const connectionExtraHeaders = activeConnection?.extra_headers ?? profile.extra_headers

  function updateConnectionField(
    field: 'base_url' | 'api_key' | 'api_version' | 'extra_headers',
    value: string
  ) {
    if (!activeConnection) {
      updateProfileField(service, field, value)
      return
    }
    mutateCatalog(next => {
      const connection = (next.connections || []).find(item => item.id === activeConnection.id)
      if (connection) (connection[field] as string | Record<string, string>) = value
    })
  }

  function chooseConnection(connectionId: string) {
    mutateCatalog(next => {
      const target = getActiveProfile(next, service)
      if (target) target.connection_id = connectionId || undefined
    })
  }
  ...
  function createSharedConnection() {
    mutateCatalog(next => {
      ...
      next.connections.push({
        id,
        name: target.name || providerOption?.label || t('Image connection'),
        provider: target.binding || 'openai',
        base_url: target.base_url || '',
        api_key: target.api_key || '',
        api_version: target.api_version || '',
        extra_headers: target.extra_headers || {},
      })
      target.connection_id = id
    })
  }
  ...
  return (
    <div className="grid gap-4 sm:grid-cols-2">
      ...
      {service === 'imagegen' && (
        <div className="sm:col-span-2 rounded-xl border border-[var(--border)]/60 bg-[var(--muted)]/20 p-3">
          <div className="mb-1.5 text-[12px] text-[var(--muted-foreground)]">
            {t('Shared provider connection')}
          </div>
          <div className="flex gap-2">
            ...
            >
              <option value="">{t('Legacy profile credentials')}</option>
            ...
            <button
              type="button"
              onClick={createSharedConnection}
              className="rounded-lg border border-[var(--border)] px-3 text-[12px]"
            >
              {t('New connection')}
            </button>
          </div>
          <p className="mt-1.5 text-[11px] text-[var(--muted-foreground)]">
            {t('The credential is stored once and can be reused by multiple image models.')}
          </p>
        </div>
      )}
      {isCodeBuddyAuth && (
      ...
      {fields.baseUrl && (
        <div className="sm:col-span-2">
          ...
          <input
            ...
            onChange={e => updateConnectionField('base_url', e.target.value)}
            placeholder={
              service === 'embedding'
          ...
          {service === 'embedding' && (
            <p className="mt-1.5 text-[11px] text-[var(--muted-foreground)]">
              {t(
                'Embedding requests are sent to this URL exactly; Knorvia does not append /embeddings or /api/embed at request time.'
              )}
            </p>
          )}
        ...
        <div className="sm:col-span-2">
          ...
          <div className="relative">
            <input
              ...
              onChange={e => updateConnectionField('api_key', e.target.value)}
              placeholder={t("sk-...")}
            />
            <button
              type="button"
          ...
          {extraOpen && (
            <div className="grid gap-4 border-t border-[var(--border)]/60 px-3.5 py-4 sm:grid-cols-2">
              <div>
                ...
                <input
                  className={inputClass}
                  value={connectionApiVersion}
                  onChange={e => updateConnectionField('api_version', e.target.value)}
              ...
              {service === 'search' ? (
                ...
                <div className="sm:col-span-2">
                  ...
                  <textarea
                    ...
                    value={stringifyExtraHeaders(connectionExtraHeaders)}
                    onChange={e => updateConnectionField('extra_headers', e.target.value)}
