...
// Defaults mirror BackendConfig in knorvia/services/subagent/config.py so the
...
const SYSTEM_PROMPT_HINT: Record<string, Lang> = {
  ...
  grok_build: {
    zh: "追加到该智能体的系统提示（--rules）。",
    en: "Appended to the agent's system prompt (--rules).",
  },
};
  ...
  return (
    <div>
      <SettingsPageHeader
        title={displayName}
        description={tr({
          zh: `Knorvia 通过 consult_subagent 调用本机 ${displayName} 时使用的模型、推理强度与运行参数。设置后即覆盖 CLI 的默认值；留空表示沿用 CLI 默认。`,
          en: `Model, reasoning effort, and run parameters Knorvia drives the local ${displayName} with when consulting it. These override the CLI defaults; leave blank to keep the CLI's own default.`,
        })}
      />
      ...
      {!loading && options && (
        <>
          ...
          <SettingSection
            ...
            description={tr({
              zh: "Knorvia 调用该智能体时使用的模型与推理强度。",
              en: "The model and reasoning effort Knorvia consults this agent with.",
            })}
          >
            <SettingRow
              title={tr({ zh: "启用", en: "Enabled" })}
              description={tr({
                zh: "关闭后，Knorvia 不会在对话中调用该智能体。",
                en: "When off, Knorvia won't consult this agent in chat.",
              })}
              control={
                <Toggle
          ...
          {features.systemPrompt && (
            <SettingSection
              ...
              description={`${tr(
                ...
                zh: "留空则使用 Knorvia 的默认委派提示。",
                en: "Blank uses Knorvia's default delegate instruction.",
              })}`}
            >
              <div className="py-4">
                <textarea
          ...
          <SettingSection
            ...
            description={tr({
              zh: "Knorvia 无人值守地驱动该智能体——默认值确保它不会卡在等待确认上。",
              en: "Knorvia drives the agent unattended — the defaults ensure it never stalls waiting for an approval prompt.",
            })}
          >
            ...
            {features.forwardImages && (
              <SettingRow
                ...
                description={tr({
                  zh: "允许 Knorvia 把本轮对话中的图片附件转发给该智能体。",
                  en: "Let Knorvia forward image attachments from the chat turn to this agent.",
                })}
                control={
                  <Toggle
