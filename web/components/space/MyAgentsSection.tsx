...
export default function MyAgentsSection() {
  ...
  const refreshAgent = useCallback(
    async (agent: ImportAgent) => {
      ...
      setNote("");
      try {
        if (agent.detectedHome || !agent.handle) {
          const res = await importDetectedHome(agent.source, {
            id: agent.id,
            name: agent.name,
          });
        ...
        if (!(await ensureReadPermission(agent.handle))) {
        ...
        <div ref={menuRef} className="relative">
          ...
          {menuOpen && (
            ...
            >
              ...
              />
              {!agent.detectedHome && agent.handle ? (
                <MenuItem
