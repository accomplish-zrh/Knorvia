...
// Where the ClawHub site lives, for "view on ClawHub" links. The catalog
...
export default function ClawHubImportModal({
  onClose,
  onInstalled,
  installedNames,
}: {
  ...
  const [webUrl, setWebUrl] = useState<string>(CLAWHUB_FALLBACK);
  ...
  const install = useCallback(
    async (slug: string, opts?: { force?: boolean }) => {
      ...
      try {
        // The `clawhub:` hub is hardcoded so the install can never be redirected
        // to another registry, and the slug comes from our own proxied catalog.
        const result = await installSkillFromHub(`clawhub:${slug}`, {
          force: opts?.force,
        });
        setInstallState((s) => ({
          ...s,
        ...
        <div className="flex items-center justify-between gap-3 border-b border-[var(--border)] px-5 py-3">
          <div className="flex min-w-0 items-center gap-2">
            ...
            <h3 className="truncate text-[14px] font-semibold text-[var(--foreground)]">
              {tr("从 ClawHub 导入技能", "Import skills from ClawHub")}
            </h3>
          </div>
      ...
      <div className="min-h-0 flex-1 overflow-y-auto px-5 py-4">
        {loading ? (
          ...
          <div className="mx-auto max-w-md rounded-lg border border-amber-500/20 bg-amber-500/10 px-4 py-3 text-center text-[12.5px] text-amber-700 dark:text-amber-400">
            ...
            <p>{tr("无法连接 ClawHub。", "Couldn't reach ClawHub.")}</p>
          ...
          <div className="py-16 text-center text-[13px] text-[var(--muted-foreground)]">
            {total === 0
              ? tr("ClawHub 上暂时还没有技能。", "No skills on ClawHub yet.")
    ...
    <>
      <div className="flex items-center justify-between gap-3 border-b border-[var(--border)] px-5 py-2.5">
        ...
        <div className="flex items-center gap-1.5">
          ...
          >
            <ExternalLink size={13} />
            {tr("在 ClawHub 查看", "View on ClawHub")}
          </a>
          <InstallButton
            state={installState}
            installed={installed}
