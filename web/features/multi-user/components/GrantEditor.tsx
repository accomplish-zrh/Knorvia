...
function hasModel(
  grant: GrantPayload,
  capability: 'llm' | 'imagegen',
  profileId: string,
  modelId?: string
) {
  return grant.models[capability].some(item => {
...
export function GrantEditor({ userId }: { userId: string }) {
  ...
  const partnerIds = useMemo(
    () => new Set(grant.partners.map(item => String(item.partner_id || item.id || ''))),
    [grant.partners]
  )

  const selectedModelCount = useMemo(
    () =>
      [...grant.models.llm, ...grant.models.imagegen].reduce((total, item) => {
        if (Array.isArray(item.model_ids)) return total + item.model_ids.length
        return total + 1
      }, 0),
    [grant.models.llm, grant.models.imagegen]
  )

  const saving = saveState === 'saving'
  const controlsDisabled = loading || saving

  function toggleModel(capability: 'llm' | 'imagegen', profileId: string, modelId: string) {
    setGrant(current => {
      const next = structuredClone(current) as GrantPayload
      const items = next.models[capability]
      ...
      next.models[capability] = items.filter(item =>
  ...
  function togglePartner(partnerId: string, name: string) {
    setGrant(current => {
      ...
      next.partners = exists
        ? next.partners.filter(item => String(item.partner_id || item.id || '') !== partnerId)
        : [...next.partners, { partner_id: partnerId, name, source: 'admin' }]
      return next
    })
  }

  function setToolList(key: 'enabled_tools' | 'mcp_tools', value: string[] | null) {
  ...
  const toolsSummary =
    grant.enabled_tools === null ? 'all tools' : `${grant.enabled_tools.length} tools`
  ...
  const mcpSummary = grant.mcp_tools === null ? 'no MCP' : `${grant.mcp_tools.length} MCP`
      ...
      <div className="flex h-[620px] max-h-[calc(100vh-170px)] min-h-[420px] flex-col overflow-hidden rounded-2xl border border-[var(--border)] bg-[var(--card)] shadow-sm">
        <div className="shrink-0 border-b border-[var(--border)] px-5 py-4">
          <div className="flex flex-wrap items-start justify-between gap-3">
            <div>
              <h2 className="text-sm font-semibold text-[var(--foreground)]">Assign access</h2>
            ...
            <div className="flex flex-wrap gap-1.5 text-[11px] text-[var(--muted-foreground)]">
              ...
              <span className="rounded-full bg-[var(--muted)]/60 px-2 py-1">{kbIds.size} KBs</span>
              <span className="rounded-full bg-[var(--muted)]/60 px-2 py-1">
              ...
              <span className="rounded-full bg-[var(--muted)]/60 px-2 py-1">{toolsSummary}</span>
              <span className="rounded-full bg-[var(--muted)]/60 px-2 py-1">{mcpSummary}</span>
            </div>
          </div>
        </div>
          ...
          <div className="grid gap-5 md:grid-cols-3">
            <section className="min-w-0">
              ...
              <div className="space-y-1.5 text-xs">
                {[
                  ...
                  ...(resources?.models.imagegen || []).map(profile => ({
                    capability: 'imagegen' as const,
                    profile,
                  })),
                  ...
                  >
                    ...
                    {(profile.models || []).map(model => (
                      ...
                      >
                        <input
                          type="checkbox"
                          checked={hasModel(grant, capability, profile.profile_id, model.model_id)}
                          disabled={controlsDisabled}
                          onChange={() =>
                            toggleModel(capability, profile.profile_id, model.model_id)
                          }
                        />
                ...
                {(resources?.partners || []).length === 0 ? (
                  ...
                  (resources?.partners || []).map(partner => (
                    <CheckRow
                      ...
                      disabled={controlsDisabled}
                      onToggle={() =>
                        togglePartner(partner.partner_id, partner.name || partner.partner_id)
                      }
                    />
                  ))
                )}
                ...
                <div className="space-y-1.5 text-xs">
                  {(resources?.tools || []).map(tool => (
                    <CheckRow
                      ...
                      disabled={controlsDisabled}
                      onToggle={() => toggleGrantTool('enabled_tools', tool.name)}
                    />
                  ))}
                </div>
              )}
            ...
            <section className="min-w-0">
              ...
              {grant.mcp_tools === null ? (
                <p className="px-1 text-[11px] leading-relaxed text-[var(--muted-foreground)]">
                  MCP tools proxy host-side capabilities, so they stay denied by default. Switch to
                  Custom to assign specific services.
                </p>
              ) : null}
              {grant.mcp_tools !== null &&
