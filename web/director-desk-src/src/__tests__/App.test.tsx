...
it("returns to a real home page that lists director desks 1 through 4", async () => {
  ...
  expect(screen.getByRole("heading", { name: "选择一个白模预演开始摆场景" })).toBeInTheDocument();
  for (const number of [1, 2, 3, 4]) {
    expect(screen.getByText(`白模预演 ${number} 号`)).toBeInTheDocument();
  }
  ...
  expect(screen.getByRole("heading", { name: "共同完善 白模预演" })).toBeInTheDocument();
...
it("keeps the selected director desk in the URL so refresh opens the same desk", async () => {
  ...
  render(<App />);
  await user.selectOptions(screen.getByRole("combobox", { name: "选择白模预演" }), "desk_4");
...
it("renders the director desk header and view mode switch", () => {
  ...
  expect(screen.getByText("白模预演")).toBeInTheDocument();
