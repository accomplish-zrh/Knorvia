...
const HOME_QUICK_START_STEPS = [
  ["选择白模预演", "打开已有白模预演，或点击“新建白模预演”创建一个空场景。"],
...
const HOME_TOOL_GROUPS = [
  ["顶部", "首页、切换白模预演、导演/第一视角、运镜工作台、视角手感"],
...
export default function App() {
  ...
  function handleDeleteDesk(desk: DirectorDeskRecord) {
    if (!window.confirm(`删除「${desk.name}」？这个白模预演里的本地场景也会一起删除。`)) return;
    ...
    return (
      <main className="director-home-shell">
        <section className="director-home-hero">
          <div>
            <p className="director-home-kicker">Knorvia White-model Previs</p>
            <h1>选择一个白模预演开始摆场景</h1>
            <p>
              每个白模预演独立保存，重启后先回到这里选择，不会再直接打开上一次的无名工程。
            </p>
          </div>
        ...
        {directorDesks.length ? (
          <section className="director-home-grid" aria-label="白模预演列表">
          ...
          <section className="director-home-empty" aria-label="空白模预演列表">
            <Boxes aria-hidden="true" size={28} strokeWidth={1.6} />
            <h2>还没有白模预演</h2>
            <p>点击“新建白模预演”创建一个干净的 3D 场景。</p>
          </section>
        )}
        ...
        <section className="director-home-contributors" aria-labelledby="director-home-contributors-title">
          <header className="director-home-section-heading">
            ...
            <div>
              <h2 id="director-home-contributors-title">共同完善 白模预演</h2>
    ...
    <div className="app-shell">
      <header className="top-bar">
        <div className="top-bar-left">
          ...
          <div className="director-desk-switcher" aria-label="白模预演选择器">
            <select
