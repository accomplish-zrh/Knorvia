...
it("applies editable quick blockout templates without deleting manual props", () => {
  useDirectorStore.setState(createInitialDirectorState());
  useDirectorStore.getState().addGeometryPrimitive("sphere");

  useDirectorStore.getState().applyBlockoutTemplate("room");
  let objects = useDirectorStore.getState().project.objects;
  expect(objects.filter((item) => item.id.startsWith("blockout_room_"))).toHaveLength(4);
  expect(objects.some((item) => item.geometryType === "sphere")).toBe(true);

  useDirectorStore.getState().applyBlockoutTemplate("corridor");
  objects = useDirectorStore.getState().project.objects;
  expect(objects.some((item) => item.id.startsWith("blockout_room_"))).toBe(false);
  expect(objects.filter((item) => item.id.startsWith("blockout_corridor_"))).toHaveLength(4);
  expect(objects.some((item) => item.geometryType === "sphere")).toBe(true);
});
