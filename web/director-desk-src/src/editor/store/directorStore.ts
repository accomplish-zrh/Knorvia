...
import { normalizeCameraOptics, verticalFovToFocalLength } from "../schema/cameraOptics";
...
function migrateDirectorProject(project: DirectorProject): DirectorProject {
  return {
    ...
    animationAssets: Array.isArray(project.animationAssets)
      ...
      : [],
    cameras: project.cameras.map((camera) => normalizeCameraOptics({
      ...camera,
...
export function createDefaultDirectorProject({
  ...
  const camera: DirectorCameraShot = {
    ...
    lensPreset: "standard",
    focalLengthMm: verticalFovToFocalLength(DEFAULT_DIRECTOR_CAMERA_VIEW_SNAPSHOT.fov, "super-35"),
    aperture: 2.8,
...
const BLOCKOUT_TEMPLATES: Record<BlockoutTemplateId, BlockoutPart[]> = {
  studio: [
    { name: "地台", position: [0, 0, 0], scale: [8, 0.1, 8] },
    { name: "背景墙", position: [0, 0, -4], scale: [8, 4, 0.15] },
  ],
  room: [
    { name: "地面", position: [0, 0, 0], scale: [8, 0.1, 8] },
    { name: "后墙", position: [0, 0, -4], scale: [8, 3.2, 0.15] },
    { name: "左墙", position: [-4, 0, 0], scale: [0.15, 3.2, 8] },
    { name: "右墙", position: [4, 0, 0], scale: [0.15, 3.2, 8] },
  ],
  corridor: [
    { name: "地面", position: [0, 0, 0], scale: [4, 0.1, 14] },
    { name: "左墙", position: [-2, 0, 0], scale: [0.15, 3.5, 14] },
    { name: "右墙", position: [2, 0, 0], scale: [0.15, 3.5, 14] },
    { name: "尽头墙", position: [0, 0, -7], scale: [4, 3.5, 0.15] },
  ],
  street: [
    { name: "道路", position: [0, 0, 0], scale: [6, 0.1, 18] },
    { name: "左楼前", position: [-5, 0, -4], scale: [3, 6, 5] },
    { name: "左楼后", position: [-5, 0, 4], scale: [3, 4, 5] },
    { name: "右楼前", position: [5, 0, -4], scale: [3, 4.5, 5] },
    { name: "右楼后", position: [5, 0, 4], scale: [3, 7, 5] },
  ],
  stage: [
    { name: "舞台", position: [0, 0, -1], scale: [8, 0.5, 5] },
    { name: "背景板", position: [0, 0, -3.5], scale: [8, 4, 0.2] },
    { name: "左侧台", position: [-4.5, 0, -1], scale: [1, 1.1, 5] },
    { name: "右侧台", position: [4.5, 0, -1], scale: [1, 1.1, 5] },
  ],
};
...
export const useDirectorStore = create<DirectorStore>((set, get) => {
  ...
  return {
    ...
    applyBlockoutTemplate: (templateId) =>
      commitMutation((state) => {
        const parts = BLOCKOUT_TEMPLATES[templateId];
        const retainedObjects = state.project.objects.filter((item) => !item.id.startsWith("blockout_"));
        const templateObjects: DirectorObject[] = parts.map((part, index) => ({
          id: `blockout_${templateId}_${index + 1}`,
          name: `白模·${part.name}`,
          kind: "prop",
          visible: true,
          locked: false,
          geometryType: "box",
          color: GEOMETRY_PRIMITIVE_COLOR,
          transform: createTransform(part.position, part.rotation ?? [0, 0, 0], part.scale),
        }));
        const selectedObjectId = templateObjects[0]?.id ?? null;
        return {
          ...state,
          selectedObjectId,
          ...
          project: {
            ...state.project,
            objects: [...retainedObjects, ...templateObjects],
          },
        };
      }),
    addCameraShot: (snapshot) => {
      ...
      commitMutation((state) => {
        ...
        const nextCamera: DirectorCameraShot = {
          ...
          sensorPreset: "super-35",
          lensPreset: "standard",
          focalLengthMm: verticalFovToFocalLength(snapshot?.fov ?? 50, "super-35"),
          aperture: 2.8,
          transform,
          targetMode: "manual",
    ...
    ensureMotionCamera: (snapshot) => {
      ...
      commitUiMutation((state) => {
        ...
        const motionCamera: DirectorCameraShot = {
          ...
          sensorPreset: "super-35",
          lensPreset: "standard",
          focalLengthMm: verticalFovToFocalLength(viewSnapshot.fov, "super-35"),
          aperture: 2.8,
