...
import { setDirectorDeskHostActive } from "./hostVisibility";
...
interface HostVisibilityPayload {
  active?: unknown;
}
...
function handleHostProtocolMessage(message: DirectorDeskTransportMessage) {
  ...
  if (message.type === "storyai:director-desk-visibility") {
    const payload = (message.payload || {}) as HostVisibilityPayload;
    setDirectorDeskHostActive(payload.active !== false);
    return;
  }
