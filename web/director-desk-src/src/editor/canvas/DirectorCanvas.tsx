...
import { useDirectorDeskHostActive } from "../io/hostVisibility";
import {
...
export function DirectorCanvas() {
  const hostActive = useDirectorDeskHostActive();
  ...
  return (
    <div className="canvas-frame" data-benchmark-mode={benchmarkMode ?? "off"}>
      <div className="director-canvas" data-testid="director-canvas" ref={viewportContainerRef}>
        <Canvas
          frameloop={hostActive ? "always" : "never"}
      ...
      />
      {hostActive && !isCameraPiloting && !isCameraPreviewing ? (
      ...
      />
      {hostActive && motionStudioOpen && (activeCameraMotionPath?.keyframes.length ?? 0) >= 2 && !isCameraPiloting && !referenceVideoRendering ? (
        <MotionMonitor
          antialias={contextPerformanceConfig.antialias}
