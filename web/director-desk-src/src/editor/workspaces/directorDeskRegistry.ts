...
function normalizeDirectorDeskName(name: string) {
  ...
  const defaultNameMatch = trimmedName.match(/^白模预演\s*(\d+)\s*号$/);
  return defaultNameMatch ? `白模预演 ${Number(defaultNameMatch[1])} 号` : trimmedName;
}
...
function getNextDirectorDeskNumber(records: DirectorDeskRecord[]) {
  ...
  for (const record of records) {
    ...
    const nameMatch = record.name.match(/^白模预演\s*(\d+)\s*号$/);
...
export function createDirectorDeskRecord(records: DirectorDeskRecord[], name?: string) {
  ...
  return {
    ...
    name: name?.trim() || `白模预演 ${nextNumber} 号`,
    createdAt: timestamp,
    updatedAt: timestamp,
...
export function ensureDirectorDeskRecords() {
  ...
  const first = createDirectorDeskRecord([], "白模预演 1 号");
  const storage = getStorageSafe();
...
export function ensureDirectorDeskRecordForId(records: DirectorDeskRecord[], id: string) {
  ...
  const record: DirectorDeskRecord = {
    id,
    name: id.startsWith(DIRECTOR_DESK_ID_PREFIX) ? `白模预演 ${id.slice(DIRECTOR_DESK_ID_PREFIX.length)} 号` : id,
    createdAt: timestamp,
    updatedAt: timestamp,
  };
