...
import {
  ...
  type DirectorCameraBody,
  type DirectorCameraSensor,
  type DirectorLensPreset,
} from "../schema/cameraOptics";
...
export function CameraPanel() {
  ...
  const cameraOptics = normalizeCameraOptics(currentCamera);
    ...
    >
      {activeTab === "properties" ? (
        <>
          ...
          <InspectorSection title="摄影机系统">
            <InspectorSelectField
              label="机身"
              ariaLabel="摄影机机身类型"
              options={CAMERA_BODY_OPTIONS.map(({ value, label }) => ({ value, label }))}
              value={cameraOptics.cameraBody ?? "cinema"}
              onChange={(value) => updateCamera(currentCamera.id, { cameraBody: value as DirectorCameraBody })}
            />
            <InspectorSelectField
              label="画幅"
              ariaLabel="摄影机感光元件画幅"
              options={CAMERA_SENSOR_OPTIONS.map(({ value, label }) => ({ value, label }))}
              value={cameraOptics.sensorPreset ?? "super-35"}
              onChange={(value) => {
                const sensorPreset = value as DirectorCameraSensor;
                updateCamera(currentCamera.id, {
                  sensorPreset,
                  fov: focalLengthToVerticalFov(cameraOptics.focalLengthMm ?? 35, sensorPreset),
                });
              }}
            />
            <InspectorSelectField
              label="镜头"
              ariaLabel="摄影机镜头预设"
              options={CAMERA_LENS_OPTIONS.map(({ value, label }) => ({ value, label }))}
              value={cameraOptics.lensPreset ?? "standard"}
              onChange={(value) => {
                const option = CAMERA_LENS_OPTIONS.find((item) => item.value === value);
                if (!option) return;
                updateCamera(currentCamera.id, {
                  lensPreset: value as DirectorLensPreset,
                  focalLengthMm: option.focalLengthMm,
                  fov: focalLengthToVerticalFov(option.focalLengthMm, cameraOptics.sensorPreset),
                });
              }}
            />
            <InspectorRangeNumberField
              label="焦距 (mm)"
              rangeAriaLabel="摄影机焦距滑杆"
              numberAriaLabel="摄影机焦距"
              max="200"
              min="8"
              step="1"
              value={cameraOptics.focalLengthMm ?? 35}
              onValueChange={(value) => {
                const focalLengthMm = Number(value);
                updateCamera(currentCamera.id, {
                  focalLengthMm,
                  fov: focalLengthToVerticalFov(focalLengthMm, cameraOptics.sensorPreset),
                });
              }}
            />
            <InspectorRangeNumberField
              label="光圈 (f/)"
              rangeAriaLabel="摄影机光圈滑杆"
              numberAriaLabel="摄影机光圈"
              max="22"
              min="1.2"
              step="0.1"
              value={cameraOptics.aperture ?? 2.8}
              onValueChange={(value) => updateCamera(currentCamera.id, { aperture: Number(value) })}
            />
            <p className="inspector-help-text">光圈作为镜头规划和下游生成参数保存；白模视口暂不模拟景深。</p>
          </InspectorSection>
          <InspectorAxisGroup
            label="位置"
            axes={[
          ...
          <InspectorRangeNumberField
            ...
            onValueChange={(value) => {
              ...
              updateCamera(currentCamera.id, {
                fov,
                focalLengthMm: verticalFovToFocalLength(fov, cameraOptics.sensorPreset),
              });
            }}
          />
