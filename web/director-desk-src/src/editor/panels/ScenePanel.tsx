...
import type { BlockoutTemplateId } from "../store/directorStore.types";
...
export function ScenePanel() {
  ...
  const applyBlockoutTemplate = useDirectorStore((state) => state.applyBlockoutTemplate);
  ...
  return (
    <InspectorPanel title="3D场景" ariaLabel="3D场景右侧属性面板" className="scene-inspector">
      <InspectorSection title="快速白模搭景">
        <p className="inspector-help-text">一键生成可编辑的基础几何体；再次选择模板会替换上一次的快速白模，不影响手动添加的角色和道具。</p>
        <div className="blockout-template-grid" role="group" aria-label="快速白模场景模板">
          {([
            ["studio", "摄影棚"],
            ["room", "室内房间"],
            ["corridor", "走廊"],
            ["street", "街景"],
            ["stage", "舞台"],
          ] as Array<[BlockoutTemplateId, string]>).map(([id, label]) => (
            <button key={id} type="button" onClick={() => applyBlockoutTemplate(id)}>{label}</button>
          ))}
        </div>
      </InspectorSection>
      <InspectorRangeNumberField
        ...
        <div className="panorama-control-card">
          ...
          <div className="panorama-action-row">
            ...
            >
              ...
              {panoramaImporting ? "正在处理..." : panoramaAsset ? "更换参考图" : "导入参考图"}
            </button>
            {panoramaAsset ? (
              <button
                aria-label="恢复全景默认方向"
