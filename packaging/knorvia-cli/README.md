...
CLI-only Knorvia distribution. It installs the `knorvia` command and the
...
FastAPI/Uvicorn server dependencies used by `knorvia start`.
...
the `knorvia` command at these source files.
