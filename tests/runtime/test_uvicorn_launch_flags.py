...
correct only if *every* launch point passes it, and Knorvia has five (two
Dockerfile stages, the ``knorvia start`` launcher, the CLI, run_server). A
...
_LAUNCH_POINTS = [
    ("knorvia/runtime/launcher.py", '"uvicorn",', _CLI_FLAGS),
    ("knorvia/api/run_server.py", "uvicorn.run(", _PYTHON_FLAGS),
    ("knorvia_cli/main.py", "uvicorn.run(", _PYTHON_FLAGS),
]
...
def test_dockerfile_launch_points_wire_serving_flags() -> None:
    ...
    lines = [
        ...
        if "-m uvicorn knorvia.api.main:app" in line
    ]
...
def test_keep_alive_outlasts_the_proxy_socket_reaper() -> None:
    ...
    """
    from knorvia.services.config import HTTP_KEEP_ALIVE_TIMEOUT
