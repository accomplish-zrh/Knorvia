...
from knorvia.api import run_server
...
def uvicorn_kwargs(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> dict[str, Any]:
    ...
    from knorvia import logging as knorvia_logging
    from knorvia.runtime import mode
    from knorvia.services import setup

    monkeypatch.setattr(knorvia_logging, "configure_logging", lambda: None)
...
def test_run_server_disables_reload_by_default(
    ...
    monkeypatch.delenv("KNORVIA_DEV_RELOAD", raising=False)
...
def test_run_server_reload_remains_available_for_development(
    ...
    monkeypatch.setenv("KNORVIA_DEV_RELOAD", "true")
    (tmp_path / "data").mkdir()
