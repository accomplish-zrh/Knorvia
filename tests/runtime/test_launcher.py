...
from knorvia.runtime import launcher
...
def test_source_production_build_is_reused_until_an_input_changes(
    ...
    def _run(command, cwd, env, **_kwargs):
        calls.append((list(command), Path(cwd), env["KNORVIA_NEXT_DIST_DIR"]))
...
def test_start_uses_ipv4_loopback_for_frontend_proxy(
    ...
    from knorvia.services import config as config_module
    from knorvia.services import setup as setup_module
    ...
    assert captured_env["KNORVIA_API_BASE_URL"] == (f"http://127.0.0.1:{resolved_backend_port}")


def test_start_refuses_non_loopback_without_authentication(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    from knorvia.services import config as config_module
    ...
    monkeypatch.setattr(launcher, "_reset_runtime_singletons", lambda: None)
    monkeypatch.setattr(config_module, "ensure_runtime_settings_files", lambda: None)
    monkeypatch.setattr(config_module, "load_launch_settings", lambda _home: object())
    ...
    with pytest.raises(RuntimeError, match="authentication is disabled"):
        launcher.start(tmp_path, host="0.0.0.0")


def test_bind_host_helpers_are_fail_closed() -> None:
    assert launcher._is_loopback_host("127.0.0.1") is True
    assert launcher._is_loopback_host("::1") is True
    assert launcher._is_loopback_host("localhost") is True
    assert launcher._is_loopback_host("0.0.0.0") is False
    assert launcher._is_loopback_host("example.test") is False
    assert launcher._backend_proxy_host("0.0.0.0") == "127.0.0.1"
