...
from knorvia.runtime.providers.scope import ToolScope
from knorvia.runtime.providers.view import build_tool_view
from knorvia.runtime.registry.scoped_registry import ScopedToolRegistry
...
def _stub_providers(monkeypatch) -> _FakeManager:
    ...
    monkeypatch.setattr("knorvia.services.mcp.get_mcp_manager", lambda: manager)
    monkeypatch.setattr("knorvia.services.mcp.load_loaded_tools", lambda session_id: set())
...
def _grant(monkeypatch, value: set[str] | None) -> None:
    monkeypatch.setattr("knorvia.multi_user.tool_access.allowed_mcp_tools", lambda: value)
...
async def test_a_failing_provider_degrades_instead_of_killing_the_turn(
    ...
    monkeypatch.setattr("knorvia.services.mcp.get_mcp_manager", _boom)
    view = await build_tool_view(base_registry=registry, scope=ToolScope(session_id="s"))
    assert view.loader is None
    assert view.pool == ()
...
async def test_attach_adds_loaded_schemas_and_binds_the_live_list(registry, monkeypatch) -> None:
    _grant(monkeypatch, None)
    monkeypatch.setattr(
        "knorvia.services.mcp.load_loaded_tools", lambda session_id: {"mcp_gh_search"}
    )
    view = await build_tool_view(base_registry=registry, scope=ToolScope(session_id="s"))
...
async def test_a_slow_personal_server_costs_only_its_own_tools(
    ...
    A third-party host that hangs must not present to the user as Knorvia
    hanging, so the scope connect is bounded and the turn proceeds without it.
    """
    from knorvia.runtime.providers import view as view_module
