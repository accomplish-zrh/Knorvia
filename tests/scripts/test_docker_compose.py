...
def test_render_docker_env_reads_json_only(tmp_path: Path) -> None:
    ...
    assert values == {
        "KNORVIA_DOCKER_BACKEND_PORT": "9001",
        "KNORVIA_DOCKER_FRONTEND_PORT": "4000",
        "KNORVIA_DOCKER_POCKETBASE_PORT": "19090",
    }
    saved = output_path.read_text(encoding="utf-8")
    assert "\nBACKEND_PORT=" not in saved
    assert "KNORVIA_DOCKER_BACKEND_PORT=9001" in saved
...
def test_render_docker_env_uses_defaults_for_missing_or_invalid_json(tmp_path: Path) -> None:
    ...
    assert values["KNORVIA_DOCKER_BACKEND_PORT"] == "8001"
    assert values["KNORVIA_DOCKER_FRONTEND_PORT"] == "3782"
    assert values["KNORVIA_DOCKER_POCKETBASE_PORT"] == "8090"


def test_compose_files_do_not_consume_legacy_env_names() -> None:
    root = Path(__file__).resolve().parents[2]
    for name in ("docker-compose.yml", "docker-compose.image.yml"):
...
def test_default_compose_files_do_not_reserve_codex_callback_ports() -> None:
    ...
    root = Path(__file__).resolve().parents[2]
    for name in ("docker-compose.yml", "docker-compose.image.yml", "compose.yaml"):
...
def test_codex_oauth_overlay_forwards_loopback_callbacks_to_frontend() -> None:
    ...
    assert set(ports) == {
        "127.0.0.1:1455:${KNORVIA_DOCKER_FRONTEND_PORT:-3782}",
        "127.0.0.1:1457:${KNORVIA_DOCKER_FRONTEND_PORT:-3782}",
    }
...
def test_ghcr_compose_persists_complete_data_tree() -> None:
    ...
    root = Path(__file__).resolve().parents[2]
    volumes = _compose_service(root, "docker-compose.image.yml")["volumes"]
...
def test_container_docs_use_temporary_codex_oauth_bridge() -> None:
    ...
    for base_file in ("docker-compose.yml", "docker-compose.image.yml"):
        assert (
            f"-f {base_file} -f compose.codex-oauth.yaml up -d --force-recreate knorvia"
        ) in normalized_section
    assert (
        "podman compose -f compose.yaml -f compose.codex-oauth.yaml up -d --force-recreate knorvia"
    ) in normalized_section
...
def test_dockerfile_is_json_driven_without_bundle_sed() -> None:
    ...
    (including KNORVIA_API_BASE_URL / KNORVIA_AUTH_ENABLED) and web/proxy.ts
    ...
    assert "KNORVIA_IGNORE_PROCESS_ENV_OVERRIDES=1" in content
...
def test_supervisord_runs_as_root_with_unprivileged_children() -> None:
    ...
    supervisord to the unprivileged ``knorvia`` user via ``gosu`` made child
    ...
    ``user=knorvia`` directive instead, which keeps them unprivileged in both
    ...
    assert "gosu knorvia /usr/bin/supervisord" not in content
    # Every supervisord program drops to the unprivileged knorvia user, so the
    ...
    for block in program_blocks:
        ...
        assert "user=knorvia" in section, (
            f"supervisord program '{name}' must run as knorvia (user=knorvia)"
        )
...
def test_frontend_api_is_url_agnostic_passthrough() -> None:
    ...
    assert "KNORVIA_API_BASE_URL" in proxy_ts
