"""
Test harness: the StreamBus reference implementation.

The legacy event bus was removed from the production package (zero
production importers since the Kernel cutover); the duck-typed stream
surface it implements is the contract the domain libraries and the daemon
adapter still speak. This copy lives under ``tests/_harness`` so the
production wheel ships no StreamBus while the test suite keeps exercising
the surface with a real implementation (history/replay semantics included).
"""
