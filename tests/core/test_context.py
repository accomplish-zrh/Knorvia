...
from knorvia.core.errors import (
    ConfigurationError,
...
class TestErrors:
    def test_base_error_str_without_details(self) -> None:
        err = KnorviaError("boom")
        assert str(err) == "boom"
        assert err.details == {}

    def test_base_error_str_with_details(self) -> None:
        err = KnorviaError("boom", details={"key": "val"})
        assert "key" in str(err)

    def test_hierarchy(self) -> None:
        assert issubclass(ConfigurationError, KnorviaError)
        assert issubclass(ValidationError, KnorviaError)
        assert issubclass(ServiceError, KnorviaError)
