...
from knorvia.core.agentic import client as agentic_client
from knorvia.core.agentic.client import (
...
from knorvia.services.llm.provider_core.base import LLMResponse, ToolCallRequest
...
def test_build_openai_client_routes_anthropic_backend_through_adapter(monkeypatch) -> None:
    ...
    monkeypatch.setattr(
        "knorvia.services.llm.provider_core.AnthropicProvider",
        FakeProvider,
    )
...
def test_build_openai_client_routes_oauth_backend_through_adapter(monkeypatch) -> None:
    ...
    monkeypatch.setattr(
        "knorvia.services.llm.provider_core.OpenAICodexProvider",
        FakeProvider,
    )
...
def test_build_openai_client_routes_github_copilot_backend_through_adapter(monkeypatch) -> None:
    ...
    monkeypatch.setattr(
        "knorvia.services.llm.provider_core.GitHubCopilotProvider",
        FakeProvider,
    )
...
def test_build_openai_client_routes_codebuddy_backend_through_adapter(monkeypatch) -> None:
    ...
    monkeypatch.setattr(
        "knorvia.services.llm.provider_core.codebuddy_http_provider.CodeBuddyHTTPProvider",
        FakeProvider,
    )
...
def test_schedule_client_close_falls_back_on_a_closed_loop() -> None:
    """Eviction racing loop shutdown must still close the client — including
    async-only clients whose aclose() has to be awaited somewhere — and must
    never leak an un-awaited coroutine (RuntimeWarning)."""
    import asyncio
    import warnings

    loop = asyncio.new_event_loop()
    loop.close()  # the shutdown race: create_task now raises RuntimeError

    async_only_closed: list[bool] = []

    class AsyncOnlyClient:
        async def aclose(self) -> None:
            async_only_closed.append(True)
    ...
    with warnings.catch_warnings():
        # Any RuntimeWarning (e.g. "coroutine ... was never awaited") from the
        # fallback path fails this test.
        warnings.simplefilter("error", RuntimeWarning)
        agentic_client._schedule_client_close(AsyncOnlyClient(), loop)
        agentic_client._schedule_client_close(SyncOnlyClient(), loop)

    assert async_only_closed == [True], "async-only client was never awaited"
