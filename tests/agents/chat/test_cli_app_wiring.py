...
def test_a_cli_app_call_is_given_a_workdir_and_a_writable_mount(
    ...
    # Path comparison: on Windows the separator is a backslash, so a string
    # `.endswith("/cli")` fails even though the directory is correct.
    assert Path(workdir).name == "cli"
