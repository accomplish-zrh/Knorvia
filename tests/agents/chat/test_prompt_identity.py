...
PROMPTS = {
    "general": "You are Knorvia, an interactive tutor.",
...
def test_chat_turn_keeps_product_identity():
    ...
    assert content == "You are Knorvia, an interactive tutor."
...
def test_blank_identity_falls_back_to_product():
    ...
    assert _general_block(context) == "You are Knorvia, an interactive tutor."


def test_shipped_yaml_carries_partner_templates():
    root = Path(__file__).resolve().parents[3] / "knorvia/agents/chat/prompts"
    for lang in ("en", "zh"):
        # Explicit UTF-8: the zh YAML is UTF-8 and Path.read_text() would use
        # the platform default encoding (GBK on Chinese Windows).
