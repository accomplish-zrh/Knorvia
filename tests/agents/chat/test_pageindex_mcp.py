...
def _prepare(monkeypatch, docs: dict[str, dict[str, str]]) -> AgenticChatPipeline:
    ...
    monkeypatch.setattr("knorvia.services.mcp.get_mcp_manager", lambda: FakeManager())
    monkeypatch.setattr("knorvia.services.mcp.load_loaded_tools", lambda _sid: set())
    # Non-admin user without an MCP grant: fail-closed empty whitelist.
    monkeypatch.setattr("knorvia.multi_user.tool_access.allowed_mcp_tools", lambda: set())
