...
from knorvia.agents.chat.agent_loop import InlineThinkFilter
from knorvia.agents.chat.agentic_pipeline import AgenticChatPipeline
from knorvia.capabilities.explore_context import explorer as explorer_mod
from knorvia.capabilities.mastery import MASTERY_TOOL_NAMES
...
@pytest.mark.asyncio
async def test_imagegen_pause_issues_one_shot_server_confirmation(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    fingerprint = "a" * 64
        ...
        async def execute(self, name: str, **kwargs):
            ...
            if len(self.executed) == 1:
                return ToolResult(
                    ...
                    success=True,
                    metadata={"confirmation_fingerprint": fingerprint},
                    pause_for_user={
                        "questions": [
                            {
                                "id": "studio_confirm",
                                "prompt": "Proceed?",
                                "options": [{"label": "Proceed (Recommended)"}],
                            }
                        ]
                    },
                )
            return ToolResult(content="generated", success=True)
        ...
        "arguments": json.dumps(
            {
                "prompt": "a mural",
                "n": 2,
                # A provider may emit undeclared keys.  This model-authored
                # value must never stand in for the server approval handoff.
                "_studio_confirmation_fingerprint": "b" * 64,
            }
        ),
    }
    client = _ScriptedChatClient(
        [
            [_llm_chunk(tool_calls=[tool_call])],
            [_llm_chunk(tool_calls=[{**tool_call, "id": "image-call-2"}])],
            [_llm_chunk(content="Done.")],
        ]
    )
    pipeline = AgenticChatPipeline(language="en")
    pipeline.registry = registry
    monkeypatch.setattr(pipeline, "_compose_enabled_tools", lambda _context: ["imagegen"])
    monkeypatch.setattr(pipeline, "_build_openai_client", lambda: client)

    async def _waiter():
        return {"answers": [{"questionId": "studio_confirm", "text": "Proceed (Recommended)"}]}

    await _run(
        pipeline,
        UnifiedContext(
            session_id="s-image",
            user_message="Create two images",
            enabled_tools=["imagegen"],
            metadata={"wait_for_user_reply": _waiter},
        ),
    )

    assert "_studio_confirmation_fingerprint" not in registry.executed[0]["kwargs"]
    assert registry.executed[1]["kwargs"]["_studio_confirmation_fingerprint"] == fingerprint
    assert "confirmed" not in registry.executed[1]["kwargs"]


@pytest.mark.asyncio
async def test_videogen_pause_issues_one_shot_server_confirmation(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    fingerprint = "c" * 64
    request_id = "agent-video-test-request"

    class _VideoRegistry(_Registry):
        async def execute(self, name: str, **kwargs):
            self.executed.append({"name": name, "kwargs": kwargs})
            if len(self.executed) == 1:
                return ToolResult(
                        ...
                        "questions": [
                            {
                                ...
                                "prompt": "Submit?",
                                "options": [{"label": "Submit (Recommended)"}],
                            }
                        ]
                    },
                )
            return ToolResult(content="queued", success=True)

    registry = _VideoRegistry()
    tool_call = {
        "id": "video-call",
        "name": "videogen",
        "arguments": json.dumps(
            {
                "prompt": "a moving wave",
                "_video_confirmation_fingerprint": "d" * 64,
                "_video_client_request_id": "agent-video-model-authored",
            }
        ),
    }
    client = _ScriptedChatClient(
        [
            [_llm_chunk(tool_calls=[tool_call])],
            [_llm_chunk(tool_calls=[{**tool_call, "id": "video-call-2"}])],
            [_llm_chunk(content="Queued.")],
        ]
    )
    pipeline = AgenticChatPipeline(language="en")
    pipeline.registry = registry
    monkeypatch.setattr(pipeline, "_compose_enabled_tools", lambda _context: ["videogen"])
    monkeypatch.setattr(pipeline, "_build_openai_client", lambda: client)

    async def _waiter():
        return {"answers": [{"questionId": "video_studio_confirm", "text": "Submit (Recommended)"}]}
    ...
    await _run(
        ...
        UnifiedContext(
            ...
            enabled_tools=["videogen"],
            metadata={"wait_for_user_reply": _waiter},
        ),
    )

    assert "_video_confirmation_fingerprint" not in registry.executed[0]["kwargs"]
    assert registry.executed[1]["kwargs"]["_video_confirmation_fingerprint"] == fingerprint
    assert registry.executed[1]["kwargs"]["_video_client_request_id"] == request_id
    assert "confirmed_cost" not in registry.executed[1]["kwargs"]


@pytest.mark.parametrize(
    ("tool_name", "answers", "expected"),
    [
        (
            "videogen",
            [{"questionId": "video_studio_confirm", "text": "Submit (Recommended)"}],
            True,
        ),
        (
            "videogen",
            [{"questionId": "video_studio_confirm", "text": "提交（推荐）"}],
            True,
        ),
        (
            "imagegen",
            [{"questionId": "studio_confirm", "text": "继续（推荐）"}],
            True,
        ),
        (
            "videogen",
            [{"questionId": "video_studio_confirm", "text": "Submit"}],
            False,
        ),
        (
            "imagegen",
            [{"questionId": "studio_confirm", "text": "Proceed"}],
            False,
        ),
        (
            "videogen",
        ...
        (
            "videogen",
            [
                {"questionId": "video_studio_confirm", "text": "Cancel"},
                {"questionId": "video_studio_confirm", "text": "Submit (Recommended)"},
            ],
            False,
        ),
        ...
        (
            "videogen",
            [{"questionId": "studio_confirm", "text": "Submit (Recommended)"}],
            False,
        ),
        (
            "imagegen",
            [{"questionId": "video_studio_confirm", "text": "Proceed (Recommended)"}],
            False,
        ),
        ("videogen", None, False),
    ],
)
def test_media_confirmation_is_bound_to_tool_question_and_exact_option(
    tool_name: str,
    answers: list[dict[str, str]] | None,
    expected: bool,
) -> None:
    from knorvia.agents.chat.agentic_pipeline import _media_confirmation_accepted

    assert _media_confirmation_accepted(tool_name, answers) is expected
...
@pytest.mark.parametrize(
    "reply",
    [
        {"answers": [{"questionId": "video_studio_confirm", "text": "Cancel"}]},
        {"answers": [{"questionId": "studio_confirm", "text": "Submit (Recommended)"}]},
        {"text": "yes, submit it"},
    ],
)
async def test_videogen_rejection_wrong_question_and_free_text_never_approve(
    monkeypatch: pytest.MonkeyPatch,
    reply: dict[str, Any],
) -> None:
    fingerprint = "e" * 64
    ...
    class _VideoRegistry(_Registry):
        async def execute(self, name: str, **kwargs):
            ...
            return ToolResult(
                ...
                metadata={
                    "confirmation_fingerprint": fingerprint,
                    "confirmation_request_id": "agent-video-rejected",
                },
                pause_for_user={
                    "questions": [
                        {
        ...
        [
            [
                _llm_chunk(
                    tool_calls=[
                        {
                            "id": "video-rejected-call",
                            "name": "videogen",
                            "arguments": json.dumps({"prompt": "a moving wave"}),
                        }
                    ]
                )
            ],
            [_llm_chunk(content="Video generation was cancelled.")],
        ]
    )
    pipeline = AgenticChatPipeline(language="en")
    pipeline.registry = registry
...
def test_augment_tool_kwargs_injects_image_studio_context() -> None:
    pipeline = AgenticChatPipeline.__new__(AgenticChatPipeline)
    context = UnifiedContext(
        session_id="sess-studio",
        ...
        attachments=[
            Attachment(
                ...
                filename="ref.png",
                mime_type="image/png",
                url="/api/attachments/s/a/ref.png",
                studio_asset_id="asset_prev",
            )
        ],
        metadata={"turn_id": "t-9"},
    )

    augmented = pipeline._augment_tool_kwargs("imagegen", {"prompt": "blue"}, context)

    assert augmented["_session_id"] == "sess-studio"
    assert augmented["_language"] == "zh"
    assert augmented["_chat_attachments"][0]["studio_asset_id"] == "asset_prev"
    assert augmented["_chat_attachments"][0]["url"].startswith("/api/attachments/")
    assert "_workspace_dir" in augmented


def test_drop_unconfigured_imagegen_keeps_granted_models(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    from knorvia.agents.chat.agentic_pipeline import _drop_unconfigured_generation_tools

    monkeypatch.setattr(
        "knorvia.multi_user.model_access.allowed_imagegen_options",
        lambda: {"options": [{"profile_id": "p", "model_id": "m"}]},
    )
    assert "imagegen" in _drop_unconfigured_generation_tools(["web_search", "imagegen"])
    ...
    class FakeCatalog:
        ...
        def get_active_model(self, _catalog: dict, _service: str) -> dict:
            return {}

    monkeypatch.setattr(
        "knorvia.services.config.model_catalog.get_model_catalog_service",
        lambda: FakeCatalog(),
    )
    assert "imagegen" not in _drop_unconfigured_generation_tools(["web_search", "imagegen"])
