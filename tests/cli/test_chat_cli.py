"""CLI smoke tests for the standalone ``knorvia-cli`` package."""
...
from knorvia.app import KnorviaApp, TurnRequest
...
def _install_fake_runtime(monkeypatch, captured_requests: list[TurnRequest]) -> None:
    async def _start_turn(self, request):
        if isinstance(request, dict):
            request = TurnRequest(**request)
        captured_requests.append(request)
        return {"id": request.session_id or "session-1"}, {"id": "turn-1"}

    async def _stream_turn(self, turn_id: str, after_seq: int = 0):
    ...
    monkeypatch.setattr("knorvia.app.facade.KnorviaApp.start_turn", _start_turn)
    monkeypatch.setattr("knorvia.app.facade.KnorviaApp.stream_turn", _stream_turn)
...
def test_chat_repl_survives_invalid_utf8_input(monkeypatch) -> None:
    ...
    monkeypatch.setattr("knorvia_cli.chat._read_repl_input", _read_input)
...
def test_session_list_command_uses_shared_store(monkeypatch) -> None:
    async def _list_sessions(self, limit: int = 50, offset: int = 0):
        return [
            {
                "id": "session-1",
                "title": "Algebra",
    ...
    monkeypatch.setattr("knorvia.app.facade.KnorviaApp.list_sessions", _list_sessions)
...
def test_start_command_delegates_to_runtime_launcher(monkeypatch) -> None:
    ...
    def _fake_start(home=None, *, dev=False, host="127.0.0.1"):
        calls.append((home, dev, host))

    monkeypatch.setattr("knorvia.runtime.launcher.start", _fake_start)

    result = runner.invoke(app, ["start"])

    assert result.exit_code == 0, result.output
    assert calls == [(None, False, "127.0.0.1")]

    result = runner.invoke(app, ["start", "--dev"])

    assert result.exit_code == 0, result.output
    assert calls[-1] == (None, True, "127.0.0.1")

    result = runner.invoke(app, ["start", "--host", "0.0.0.0"])
    assert result.exit_code == 0, result.output
    assert calls[-1] == (None, False, "0.0.0.0")


def test_serve_refuses_non_loopback_without_authentication(monkeypatch) -> None:
    monkeypatch.setattr(
        "knorvia.services.config.load_auth_settings",
