"""CLI smoke tests for ``knorvia notebook`` commands."""
...
def _patch_facade(monkeypatch) -> None:
    """Patch KnorviaApp methods to use the shared fake manager."""
    ...
    from knorvia.app import facade

    monkeypatch.setattr(facade.KnorviaApp, "create_notebook", _create_notebook)
    monkeypatch.setattr(facade.KnorviaApp, "get_notebook", _get_notebook)
    monkeypatch.setattr(facade.KnorviaApp, "add_record", _add_record)
    monkeypatch.setattr(facade.KnorviaApp, "update_record", _update_record)
    monkeypatch.setattr(facade.KnorviaApp, "remove_record", _remove_record)
