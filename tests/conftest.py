...
try:  # pragma: no cover - import-time wiring
    from knorvia.multi_user.paths import ADMIN_WORKSPACE_ROOT as _REAL_ADMIN_ROOT
    from knorvia.multi_user.paths import SYSTEM_ROOT as _REAL_SYSTEM_ROOT
...
def _guard_legacy_multi_user_migration(monkeypatch):
    ...
    monkeypatch.setattr(
        paths, "LEGACY_MULTI_USER_ROOT", Path("/nonexistent/knorvia-legacy-multi-user")
    )
...
def _isolate_codebuddy_login(monkeypatch):
    ...
    """
    from knorvia.services import codebuddy_credentials

    monkeypatch.setenv("KNORVIA_CODEBUDDY_AUTH_FILE", str(Path("/nonexistent/codebuddy-auth.info")))
    monkeypatch.setattr(
    ...
    monkeypatch.delenv("KNORVIA_CODEBUDDY_BACKEND", raising=False)
    yield
