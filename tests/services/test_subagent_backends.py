...
from knorvia.services.subagent.config import (
    CONSULT_BUDGET_MAX,
...
def test_claude_command_applies_model_effort_system_prompt() -> None:
    backend = ClaudeCodeBackend()
    cfg = BackendConfig(model="opus", effort="high", system_prompt="consulted by Knorvia")
    ...
    assert "--append-system-prompt" in cmd and "consulted by Knorvia" in cmd
...
def test_claude_command_forwards_mcp_config() -> None:
    backend = ClaudeCodeBackend()
    cmd = backend._build_command(
        "hi",
        session_id=None,
        config=BackendConfig(),
        mcp_config_path="C:/tmp/knorvia-mcp.json",
    )
    assert cmd[cmd.index("--mcp-config") + 1] == "C:/tmp/knorvia-mcp.json"


def test_codex_command_forwards_mcp_overrides() -> None:
    backend = CodexBackend()
    cmd = backend._build_command(
        "hi",
        session_id=None,
        config=BackendConfig(sandbox="bypass"),
        mcp_flags=["-c", 'mcp_servers.docs.url="https://mcp.example/mcp"'],
    )
    assert "-c" in cmd and 'mcp_servers.docs.url="https://mcp.example/mcp"' in cmd
    assert cmd[-1] == "hi"
...
def test_settings_defaults() -> None:
    ...
    assert defaults.sandbox == "read-only"
    assert defaults.auto_approve is False


def test_subagent_environment_does_not_inherit_provider_secrets(monkeypatch) -> None:
    monkeypatch.setenv("PATH", "safe-path")
    monkeypatch.setenv("OPENAI_API_KEY", "must-not-leak")
    monkeypatch.setenv("ANTHROPIC_API_KEY", "must-not-leak")
    env = subagent_environment({"KNORVIA_EXPLICIT": "ok"})
    assert env["PATH"] == "safe-path"
    assert env["KNORVIA_EXPLICIT"] == "ok"
...
def test_materialize_images_writes_only_resolvable_images(tmp_path) -> None:
    ...
    from knorvia.core.context import Attachment
    from knorvia.services.subagent.images import materialize_images
...
def test_parse_claude_model_screen() -> None:
    from knorvia.services.subagent.claude_models import _parse_model_screen
...
def test_claude_models_cache_roundtrip(monkeypatch, tmp_path) -> None:
    from knorvia.services.subagent import claude_models as cm
...
async def test_claude_options_prefers_synced_cache(monkeypatch) -> None:
    ...
    curated fallback."""
    from knorvia.services.subagent import claude_models as cm
    from knorvia.services.subagent import models as models_mod
...
def test_session_registry_roundtrip(monkeypatch, tmp_path) -> None:
    from knorvia.services.subagent import sessions as sess
...
def test_registry_partner_is_non_cli_backend() -> None:
    from knorvia.services.subagent import PARTNER_BACKEND_KIND, get_backend, list_backend_kinds
...
def test_validate_subagent_cwd_defaults_empty_to_home(tmp_path, monkeypatch) -> None:
    monkeypatch.setattr(
        "knorvia.services.subagent.process.default_subagent_cwd",
        lambda: str(tmp_path),
    )
    monkeypatch.delenv("KNORVIA_LINKED_FOLDER_ROOTS", raising=False)
    resolved = validate_subagent_cwd("")
    assert resolved == str(tmp_path.resolve())
    assert validate_subagent_cwd(None) == resolved
    assert validate_subagent_cwd("   ") == resolved
...
def test_partner_event_mapping_covers_channels() -> None:
    from knorvia.core.stream import StreamEvent, StreamEventType
    from knorvia.services.subagent.partner import _to_subagent_events
...
def test_gemini_command_build_fresh_resume_and_approval_mapping() -> None:
    ...
    # Safe defaults stay interactive rather than silently approving tools.
    assert fresh[fresh.index("--approval-mode") + 1] == "default"
    assert "--resume" not in fresh
...
def test_kimi_command_build_session_yolo_and_thinking() -> None:
    ...
    assert "--yolo" not in cmd and "--no-thinking" in cmd
...
# ---- Grok Build: command building + streaming-json parsing --------------------


def test_grok_command_build_fresh_resume_model_and_rules() -> None:
    ...
    assert fresh[:3] == ["grok", "-p", "hi"]
    assert "--output-format" in fresh and "streaming-json" in fresh
    assert fresh[fresh.index("--permission-mode") + 1] == "bypassPermissions"
    assert "--always-approve" in fresh
    assert fresh[fresh.index("--model") + 1] == "grok-4.6"
    assert fresh[fresh.index("--reasoning-effort") + 1] == "high"
    assert fresh[fresh.index("--rules") + 1] == "be brief"
    assert "--resume" not in fresh

    resumed = backend._build_command(
        "again", session_id="sess-1", config=BackendConfig(permission_mode="default")
    )
    assert "--resume" in resumed and "sess-1" in resumed
    assert "--always-approve" not in resumed


def test_grok_command_forwards_images_in_prompt() -> None:
    from knorvia.services.subagent.grok_build import GrokBuildBackend

    backend = GrokBuildBackend()
    cmd = backend._build_command(
        "look", session_id=None, config=BackendConfig(), images=["/tmp/g/a.png"]
    )
    assert "/tmp/g/a.png" in cmd[2]


def test_parse_grok_models_reads_default_and_list() -> None:
    from knorvia.services.subagent.grok_build import parse_grok_models

    default, models = parse_grok_models(
    ...
    assert default == "grok-4.6"
    assert [slug for slug, _ in models] == ["grok-4.6", "grok-4.5"]
    assert "default" in models[0][1]


async def _drive_grok(events):
    from knorvia.services.subagent.grok_build import GrokBuildBackend

    backend = GrokBuildBackend()
    result = ConsultResult()
    stream = {"text": "", "thought": "", "text_idx": 0, "thought_idx": 0, "saw_text": False}
    emitted: list[tuple[str, str, dict]] = []
...
@pytest.mark.asyncio
async def test_grok_streaming_json_types_out_and_tools() -> None:
    result, emitted = await _drive_grok(
        [
            ...
            {"type": "text", "data": "Done."},
            {"type": "end", "stopReason": "end_turn", "sessionId": "gb1", "text": "Done."},
        ]
    )
    assert result.session_id == "gb1"
    texts = [(t, m.get("merge_id")) for k, t, m in emitted if k == "text"]
    assert texts[0] == ("Let me", "txt:0")
    assert texts[1] == ("Let me check.", "txt:0")
    assert texts[-1][0] == "Done."
    assert ("tool", "read_file(src/main.rs)") in [(k, t) for k, t, _ in emitted]
    assert result.success is True
...
async def _drive_opencode(events, *, auto_approve=True):
    from knorvia.services.subagent.opencode_family import OpencodeBackend
...
def test_opencode_final_text_skips_synthetic_parts() -> None:
    from knorvia.services.subagent.opencode_family import _text_from_parts
...
def test_server_registry_reaps_idle_and_dead_handles() -> None:
    from knorvia.services.subagent import opencode_server as srv
