...
def test_parse_hub_ref_forms() -> None:
    ...
    # No prefix resolves to the built-in default hub (ClawHub).
                    ...
                    "skills": [
                        {
                            ...
                            "owner": {
                                "displayName": "Knorvia",
                                "htmlUrl": "https://skills.example.com",
                            },
                        },
            ...
            return httpx.Response(
                ...
                json={
                    "skill": {
                        ...
                        "summary": "Teach.",
                        "description": "---\nname: socratic-tutor\n---\n\n# Body\n",
                        # ClawHub stores dist-tags under `tags`; topical labels
                        # live in `keywords`.
                        "tags": {"latest": "1.0.0"},
                        "keywords": ["tutor", "socratic"],
                        "stats": {"downloads": 8, "stars": 2},
                    },
                    "owner": {"displayName": "Example", "htmlUrl": "https://skills.example.com"},
...
def test_clawhub_catalog_normalises_rows() -> None:
    ...
    assert row["owner"] == "Knorvia"
    assert row["owner_url"] == "https://skills.example.com"
...
def test_clawhub_detail_includes_body_version_and_tags() -> None:
    ...
    # topical labels come from `keywords`, not ClawHub's dist-tags `tags` map
    assert detail["tags"] == ["tutor", "socratic"]
    assert "# Body" in detail["content"]
    assert detail["owner"] == "Example"  # lifted from the envelope top level


def test_clawhub_web_origin_strips_api_suffix() -> None:
    provider = ClawHubProvider("clawhub", base_url="https://skills.example.com/api/v1")
    assert provider.web_origin == "https://skills.example.com"
...
def test_command_provider_failure_cleans_up() -> None:
    provider = CommandProvider(
        "myhub", fetch_cmd=f'"{sys.executable}" -c "import sys; sys.exit(1)"'
    )
    with pytest.raises(HubError):
        provider.fetch("demo")
...
def test_get_hub_provider_default_and_unknown() -> None:
    ...
    # ClawHub is a built-in hub: resolvable with no settings file.
    clawhub = get_hub_provider("clawhub")
    assert isinstance(clawhub, ClawHubProvider)
    assert clawhub.name == "clawhub"
