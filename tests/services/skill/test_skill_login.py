...
from knorvia_cli.skill_login import (
...
def test_hub_origin_from_base() -> None:
    assert hub_origin_from_base("https://skills.example.com/api/v1") == "https://skills.example.com"
...
def test_run_login_captures_token_and_rejects_bad_state() -> None:
    ...
    bad = base + "?" + urllib.parse.urlencode({"state": "wrong", "token": "clawhub_x"})
    try:
    ...
    good = (
        base
        + "?"
        + urllib.parse.urlencode({"state": state, "token": "clawhub_abc", "login": "alice"})
    )
    ...
    res = result["r"]
    assert res.token == "clawhub_abc"  # type: ignore[union-attr]
