...
from knorvia.services.sandbox.runner import server as runner_server
from knorvia.services.sandbox.spec import ExecRequest

#: The runner runs inside a Linux container; the execution tests below use
#: POSIX paths (/usr/bin/env, /bin/echo) and POSIX shell quoting. Skip them on
#: Windows, where they cannot exercise the behavior under test.
_POSIX_ONLY = pytest.mark.skipif(
    sys.platform == "win32",
    reason="executes POSIX binaries / POSIX shell quoting",
)
...
def test_the_sidecar_sends_both_spellings() -> None:
    ...
    import knorvia.services.sandbox.backends as backends_module
