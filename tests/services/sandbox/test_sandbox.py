...
from knorvia.services.sandbox.backends import BwrapBackend, RestrictedSubprocessBackend
from knorvia.services.sandbox.config import SandboxSettings, build_backend
from knorvia.services.sandbox.quota import QuotaExceeded, UserExecQuota
from knorvia.services.sandbox.service import SandboxService
from knorvia.services.sandbox.spec import ExecRequest, ExecResult, IsolationLevel, ResourceLimits


def test_backend_selection_runner_url() -> None:
    from knorvia.services.sandbox.backends import RunnerSidecarBackend
...
def test_backend_selection_none_without_optin() -> None:
    ...
    from knorvia.services.sandbox.backends import RestrictedSubprocessBackend
...
async def test_restricted_subprocess_timeout() -> None:
    backend = RestrictedSubprocessBackend()
    # Use a Python sleep rather than the `sleep` binary, which does not exist
    # on Windows. `shell=True` is exercised either way (the command string is
    # passed to the backend without argv).
    sleep = f'{sys.executable} -c "import time; time.sleep(5)"'
    result = await backend.exec(ExecRequest(command=sleep, limits=ResourceLimits(timeout_s=1)))
...
def test_runner_server_validates_request_shape() -> None:
    from knorvia.services.sandbox.runner import server
...
@pytest.mark.skipif(
    sys.platform == "win32",
    reason="uses the POSIX `true` binary and symlinks",
)
