...
from knorvia.services.parsing import cache
...
def test_load_ir_absolutizes_relative_img_paths(tmp_path: Path) -> None:
    ...
    workdir.mkdir()
    (workdir / "doc.md").write_text("# title", encoding="utf-8")
    # A platform-native absolute path: POSIX "/abs/..." is not absolute on
    # Windows (no drive) and would be re-anchored, failing the assertion.
    absolute = str(workdir / "elsewhere" / "already.png")
    (workdir / "doc_content_list.json").write_text(
        json.dumps(
            [
                {"type": "image", "img_path": "images/fig1.png"},
                {"type": "table", "img_path": "images/tbl1.png"},
                {"type": "image", "img_path": absolute},
                {"type": "image", "img_path": ""},
    ...
    assert blocks[2]["img_path"] == absolute
