...
from knorvia.services.memory.snapshot import adapters
...
def _write_session(sessions_dir: Path, key: str, turns: list[tuple[str, str]]) -> None:
    ...
    # Mirror PartnerSessionStore._stem: session keys are sanitized on disk, so
    # a key like ``telegram:42`` is stored as ``telegram_42.jsonl`` (the colon
    # is also an illegal Windows filename character).
...
def partner_tree(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> Path:
    ...
    import knorvia.multi_user.paths as mu_paths
...
def test_partner_sessions_become_tagged_entities(partner_tree: Path) -> None:
    ...
    ent = entities[0]
    # Session keys are sanitized on disk (``:`` folds to ``_``), so the entity
    # id carries the sanitized stem.
