...
from knorvia.services.config.loader import (
    PROJECT_ROOT,
...
def test_load_config_with_main_uses_explicit_project_root() -> None:
    ...
    # Compare as paths: on Windows the injected value uses backslash
    # separators, so a plain string `.endswith("/")` fails.
    solve_dir = Path(config["paths"]["solve_output_dir"])
    assert solve_dir.parts[-5:] == ("data", "user", "workspace", "chat", "deep_solve")
