...
from knorvia.services.rag.index_versioning import resolve_storage_dir_for_read
from knorvia.services.rag.pipelines.lightrag import config as lr_config
from knorvia.services.rag.pipelines.lightrag import engine, storage
from knorvia.services.rag.pipelines.lightrag.pipeline import LightRagPipeline
from knorvia.services.rag.pipelines.lightrag.worker import run_in_worker_loop
...
def test_embedding_func_returns_numpy_array(monkeypatch) -> None:
    ...
    monkeypatch.setattr("knorvia.services.embedding.get_embedding_config", lambda: _Config())
    monkeypatch.setattr("knorvia.services.embedding.get_embedding_client", lambda: _Client())
...
def test_lightrag_llm_adapter_preserves_messages_and_drops_extra_kwargs(
    ...
    monkeypatch.setattr("knorvia.services.llm.get_llm_client", lambda: _Client())
...
def test_build_rag_skips_raganything_parser_install_check(monkeypatch) -> None:
    ...
    time, even though Knorvia only ever inserts a pre-parsed ``content_list``
    ...
    # A Path, not a string: on Windows Path("/tmp/kb-wd") stringifies to
    # "\tmp\kb-wd" and the round-trip comparison must use the same type.
    working_dir = Path("kb-wd")
    rag = engine.build_rag(working_dir)

    assert rag._parser_installation_checked is True
    assert captured["config"].working_dir == str(working_dir)
...
def _stub_parse(monkeypatch, *, blocks=None, markdown: str = "# md") -> None:
    from knorvia.services.parsing.types import ParsedDocument
    ...
    monkeypatch.setattr("knorvia.services.parsing.get_parse_service", lambda: _Service())
...
def test_indexing_isolated_from_owner_loop_with_context_and_progress(tmp_path, monkeypatch) -> None:
    ...
    monkeypatch.setattr("knorvia.services.parsing.get_parse_service", lambda: _ParseService())
