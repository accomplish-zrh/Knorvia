...
from knorvia.services.codebuddy_credentials import (
    INTERNAL_ENDPOINT,
    CodeBuddyAuthUnavailable,
)
from knorvia.services.llm.provider_core import codebuddy_http_provider as http_module
from knorvia.services.llm.provider_core.base import LLMResponse
...
def _sign_in(tmp_path: Path, monkeypatch, *, expires_in: float = 86400.0) -> Path:
    ...
    monkeypatch.setenv("KNORVIA_CODEBUDDY_AUTH_FILE", str(path))
    return path
...
def _capture_stream(monkeypatch) -> dict[str, Any]:
    ...
    monkeypatch.setattr(
        "knorvia.services.llm.provider_core.openai_compat_provider."
...
async def test_blocking_chat_streams_because_cloud_rejects_non_stream(
    ...
    monkeypatch.setattr(
        "knorvia.services.llm.provider_core.openai_compat_provider.OpenAICompatProvider.chat",
        fail_chat,
    )
...
@pytest.mark.asyncio
async def test_provider_strips_knorvia_session_id(tmp_path, monkeypatch) -> None:
    _sign_in(tmp_path, monkeypatch)
    captured = _capture_stream(monkeypatch)

    await CodeBuddyHTTPProvider().chat_stream(
        messages=[{"role": "user", "content": "hi"}],
        knorvia_session_id="session-1",
    )

    assert "knorvia_session_id" not in captured
