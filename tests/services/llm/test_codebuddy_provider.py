...
from knorvia.services.llm.provider_core import CodeBuddyProvider
from knorvia.services.llm.provider_core.codebuddy_provider import fetch_codebuddy_models
from knorvia.services.llm.provider_factory import get_runtime_provider
...
async def test_codebuddy_provider_maps_sdk_mcp_tool_calls(monkeypatch) -> None:
    ...
    assert option_kwargs["tools"] == ["mcp__knorvia__web_search"]
    assert "knorvia" in option_kwargs["mcp_servers"]
...
async def test_codebuddy_provider_reuses_session_and_sends_message_delta(monkeypatch) -> None:
    ...
    first = await provider.chat(initial, knorvia_session_id="chat-1")
...
def test_codebuddy_registry_aliases_and_factory(monkeypatch) -> None:
    ...
    monkeypatch.setenv("KNORVIA_CODEBUDDY_BACKEND", "sdk")
    monkeypatch.setattr(
        "knorvia.services.llm.provider_core.codebuddy_http_provider.sdk_installed",
        lambda: True,
    )
...
def test_codebuddy_ignores_knorvia_no_key_placeholder() -> None:
