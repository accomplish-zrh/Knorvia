...
def test_v1_migration_creates_backup_and_shared_connection(tmp_path: Path):
    ...
    catalog = ModelCatalogService(path=path).load()
    backup = path.with_name("model_catalog.v1.backup.json")

    assert json.loads(backup.read_text(encoding="utf-8")) == original
    assert catalog["version"] == 2
    profile = catalog["services"]["imagegen"]["profiles"][0]
    assert profile["connection_id"] == catalog["connections"][0]["id"]
    assert catalog["connections"][0]["api_key"] == "secret"
