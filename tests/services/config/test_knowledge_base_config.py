...
from knorvia.services.config.knowledge_base_config import (
...
class TestAtomicSave:
    def test_save_is_atomic_and_leaves_valid_json(self, fresh_service, tmp_path: Path) -> None:
        import os

        # Regression: _save used to rewrite kb_config.json in place with
        # open(..., "w"), so a crash mid-write could truncate the file that
        # holds every KB registration. The write must go through a
        # same-directory temp file and an atomic replace.
        config_path = fresh_service.config_path
        fresh_service.set_rag_provider("kb-atomic", "lightrag")

        on_disk = json.loads(config_path.read_text(encoding="utf-8"))
        assert on_disk["knowledge_bases"]["kb-atomic"]["rag_provider"] == "lightrag"

        siblings = [p.name for p in tmp_path.iterdir() if p.name != "kb_config.json"]
        assert siblings == []
