...
from knorvia.services.mcp import oauth
...
@pytest.mark.skipif(
    sys.platform == "win32",
    reason="POSIX mode bits do not exist on Windows; ownership is enforced by the account ACLs instead",
)
...
def test_a_needs_auth_failure_is_not_classified_as_broken() -> None:
    ...
    from knorvia.services.mcp.manager import _needs_authorization
...
def test_the_redirect_uri_follows_the_browsing_origin(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.delenv("KNORVIA_PUBLIC_URL", raising=False)
...
def test_an_operator_who_states_the_public_url_wins(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setenv("KNORVIA_PUBLIC_URL", "https://public.example/")
...
def test_the_client_registration_declares_exactly_one_redirect(monkeypatch) -> None:
    ...
    assert metadata.client_name == "Knorvia"
