...
from knorvia.services.mcp import pageindex_server
from knorvia.services.mcp.config import MCPConfig, MCPServerConfig
from knorvia.services.mcp.manager import wrapped_tool_name
from knorvia.services.rag.pipelines.pageindex.config import (
    PageIndexConfig,
...
def _configured(monkeypatch, key: str = "sk-test") -> None:
    monkeypatch.setattr(
        "knorvia.services.rag.pipelines.pageindex.config.get_pageindex_config",
...
def _unconfigured(monkeypatch) -> None:
    ...
    monkeypatch.setattr(
        "knorvia.services.rag.pipelines.pageindex.config.get_pageindex_config", _raise
    )
