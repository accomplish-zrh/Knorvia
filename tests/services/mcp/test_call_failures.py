...
from knorvia.services.mcp.secrets import secret_reference, store_secrets
...
def _offline_dns(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr(
        "knorvia.services.mcp.network.socket.getaddrinfo",
...
async def test_reload_reconnects_a_server_whose_secret_changed(
    ...
    from knorvia.services.mcp.user_config import save_user_server
...
@pytest.mark.asyncio
async def test_timeout_is_bounded_when_a_call_resists_cancellation(monkeypatch) -> None:
    import knorvia.services.mcp.manager as manager_module

    monkeypatch.setattr(manager_module, "_TASK_CANCEL_GRACE_S", 0.05)
    release = asyncio.Event()
    cancellation_seen = asyncio.Event()

    class _ResistantSession:
    ...
    conn = _ServerConnection(
        ...
        owner=OWNER,
        status="connected",
        session=_ResistantSession(),
    )
    conn.task = asyncio.create_task(asyncio.Event().wait())
    manager._connections[(OWNER, SERVER)] = conn

    result = await asyncio.wait_for(
        manager.call_tool(OWNER, SERVER, "compute_routes", {}, timeout=0.01),
        timeout=0.5,
    )
...
def test_a_credential_in_the_url_never_reaches_the_caller() -> None:
    ...
    """
    from knorvia.services.mcp.manager import describe_connect_failure
