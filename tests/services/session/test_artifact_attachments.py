...
class TestArtifactAttachments:
    ...
    def test_studio_provenance_is_copied(self) -> None:
        event = self._sources_event(
            [
                {
                    "type": "artifact",
                    "filename": "cat.png",
                    "url": "/api/outputs/x/cat.png",
                    "mime_type": "image/png",
                    "studio_asset_id": "asset_1",
                    "studio_job_id": "job_1",
                    "studio_project_id": "project_1",
                }
            ]
        )
        record = artifact_attachments(event)[0]
        assert record["studio_asset_id"] == "asset_1"
        assert record["studio_job_id"] == "job_1"
        assert record["studio_project_id"] == "project_1"
...
class TestFillPreviewText:
    ...
    async def test_pptx_gets_preview_text(self, tmp_path, monkeypatch) -> None:
        from knorvia.services.session import artifact_attachments as module
