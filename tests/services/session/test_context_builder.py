...
from knorvia.services.session.context_builder import (
    ContextBuilder,
    ContextBuildResult,
    build_history_text,
    count_tokens,
...
class TestBuildHistoryText:
    def test_system_is_not_mislabeled_as_summary(self) -> None:
...
class TestBuildHistory:
    def test_with_summary(self) -> None:
        ...
        assert history[0]["_context_summary"] is True
        assert "untrusted data" in history[0]["content"]
        assert history[0]["content"].endswith("A summary")
...
class TestContextBuilderSummarizePaths:
    ...
    async def test_rebuilds_from_raw_when_prefix_fits(self) -> None:
        ...
        summary_message = result.conversation_history[0]
        assert summary_message["role"] == "user"
        assert summary_message["_context_summary"] is True
        assert summary_message["content"].endswith("NEW SUMMARY")
    ...
    async def test_failure_keeps_watermark_and_degrades_for_turn(self) -> None:
        ...
        assert summary_message["role"] == "user"
        assert summary_message["_context_summary"] is True
        assert summary_message["content"].endswith("OLD SUMMARY")
        contents = [m["content"] for m in result.conversation_history]
        assert "recent answer" in contents

    @pytest.mark.asyncio
    async def test_single_oversized_recent_message_obeys_hard_budget(self) -> None:
        messages = [{"id": 1, "role": "user", "content": "oversized " * 10_000}]
        store = _make_store(messages, summary="", up_to=0)
        builder = ContextBuilder(store=store)
        builder._summarize = AsyncMock(side_effect=RuntimeError("llm down"))
