...
from knorvia.services.session.turn_runtime import (
    TurnRuntimeManager,
...
@pytest.mark.asyncio
async def test_user_reply_is_accepted_only_while_actively_waiting() -> None:
    manager = TurnRuntimeManager(store=object())  # type: ignore[arg-type]
    pending = _PendingReply()
    manager._reply_queues["turn-1"] = pending

    assert await manager.submit_user_reply("turn-1", "too early") is False
    pending.accepting = True
    assert await manager.submit_user_reply("turn-1", "Proceed") is True
    assert await manager.submit_user_reply("turn-1", "duplicate") is False
    assert (await pending.queue.get())["text"] == "Proceed"
