...
from knorvia.services.session.pocketbase_store import PocketBaseSessionStore
from knorvia.services.session.sqlite_store import SQLiteSessionStore
from knorvia.services.session.turn_runtime import TurnRuntimeManager, _TurnExecution
...
def fake_pb(monkeypatch):
    client = _FakeClient()
    monkeypatch.setattr(
        "knorvia.services.pocketbase_client.get_pb_client", lambda: client, raising=True
    )
    return client
...
@contextmanager
def as_user(uid: str):
    scope = UserScope(kind="user", user_id=uid, root=Path("/tmp") / uid)
...
def stub_workspace(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(
        "knorvia.services.session.turn_runtime.get_path_service",
