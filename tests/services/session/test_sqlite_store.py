...
def test_sqlite_store_migrates_legacy_chat_history_db(tmp_path: Path) -> None:
    ...
    try:
        ...
        try:
            ...
            # Close explicitly: the sqlite3 context manager only commits, and on
            # Windows an open handle would make the store's os.replace() fail.
            conn.close()
...
def store(tmp_path: Path) -> SQLiteSessionStore:
    """A store rooted at tmp_path, isolated from the checkout's real data/.

    The store's legacy-migration reads ``<project_root>/data/chat_history.db``;
    point that at tmp_path so a developer's real workspace (or leftover test
    artifacts) can never be moved into or affect a test database.
    """
    service = PathService.get_instance()
    original_root = service._project_root
    service._project_root = tmp_path
