...
from knorvia.services.cron.service import CronService
from knorvia.tools.cron_tool import run_cron_action


@pytest.fixture
def cron_service(tmp_path, monkeypatch):
    import knorvia.services.cron.service as service_mod
    import knorvia.tools.cron_tool as tool_mod
...
class TestRegistryIntegration:
    def test_cron_tool_is_builtin_and_automounted(self):
        from knorvia.agents._shared.tool_composition import AUTO_MOUNTED_TOOLS
        from knorvia.tools.builtin import BUILTIN_TOOL_NAMES

        assert "cron" in BUILTIN_TOOL_NAMES
        assert "cron" in AUTO_MOUNTED_TOOLS

    def test_schema_has_action_enum(self):
        from knorvia.tools.builtin import CronTool

        schema = CronTool().get_definition().to_openai_schema()
        action = schema["function"]["parameters"]["properties"]["action"]
        assert set(action["enum"]) == {"schedule", "list", "cancel", "run"}

    def test_run_queues_job_now(self, cron_service):
        ...
        before = cron_service.get_job(job_id)
        assert before is not None
        later = before.state.next_run_at_ms
        outcome = run_cron_action({"action": "run", "job_id": job_id, "_cron_owner": CHAT_OWNER})
        assert outcome.ok, outcome.text
        after = cron_service.get_job(job_id)
        assert after is not None
        assert after.enabled is True
        assert after.state.next_run_at_ms is not None
        assert later is None or after.state.next_run_at_ms <= later


class TestExecutorRouting:
    @pytest.mark.asyncio
    async def test_partner_job_runs_and_publishes_outbound(self, monkeypatch):
        from knorvia.services.cron import executor
        from knorvia.services.cron.service import CronJob, CronOwner, CronSchedule
        ...
        import knorvia.services.partners as partners_mod
