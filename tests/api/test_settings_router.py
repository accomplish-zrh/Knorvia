...
from knorvia.api.routers import settings as settings_router
from knorvia.services.codex_auth.contracts import CodexAuthError
...
from knorvia.services.embedding import client as embedding_client_module
from knorvia.services.embedding import config as embedding_config_module
from knorvia.services.llm import client as llm_client_module
from knorvia.services.llm import config as llm_config_module
...
def test_both_readers_of_interface_json_agree_on_a_legacy_file(
    ...
    """
    from knorvia.services.settings import interface_settings
...
async def test_network_settings_roundtrip_normalizes_cors_origins(
    ...
    payload = settings_router.NetworkSettingsUpdate(
        backend_port=8101,
        frontend_port=3882,
        public_api_base="https://api.example.com/knorvia",
    ...
    assert response["settings"]["public_api_base"] == "https://api.example.com/knorvia"
...
async def test_mineru_payload_includes_local_cli_probe(
    ...
    from knorvia.services.parsing.engines.mineru import backend as mineru_backend
...
async def test_mineru_models_download_start_requires_downloader(
    ...
    from knorvia.services.parsing.engines.mineru import models as mineru_models
...
async def test_fetch_models_returns_picker_options(monkeypatch: pytest.MonkeyPatch) -> None:
    import knorvia.services.llm.factory as factory_module
