...
from knorvia.api.routers import imports as imports_router
from knorvia.api.routers.imports import (
    ChatHistoryImportRequest,
    ImportHomeRequest,
    import_chat_history,
    import_detected_home,
    list_import_homes,
...
def test_list_import_homes_returns_discovered(monkeypatch) -> None:
    monkeypatch.setattr(
        "knorvia.services.session.import_homes.discover_import_homes",
        lambda: [
            {
                "source": "codex",
                "label": "Codex",
    ...
    listed = asyncio.run(list_import_homes())
    assert listed["homes"][0]["source"] == "codex"
    assert listed["homes"][0]["session_count"] == 4


def test_import_detected_home_endpoint(monkeypatch) -> None:
    async def fake_import(source: str, *, agent_id: str = "", agent_name: str = ""):
        return {
            "imported": 2,
            "skipped": 1,
            "sessions": [],
    ...
    monkeypatch.setattr(
        "knorvia.services.session.import_homes.import_home_async",
        fake_import,
    )
    result = asyncio.run(
        import_detected_home(ImportHomeRequest(source="claude_code", agent_name="Mine"))
    )
    assert result["imported"] == 2
    assert result["source"] == "claude_code"


def test_import_detected_home_missing_raises_404(monkeypatch) -> None:
    async def missing(source: str, *, agent_id: str = "", agent_name: str = ""):
    ...
    with pytest.raises(HTTPException) as exc:
        asyncio.run(import_detected_home(ImportHomeRequest(source="codex")))
