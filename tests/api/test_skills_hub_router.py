"""API tests for the in-app ClawHub skill browser endpoints.

``GET /api/v1/skills/hub/catalog`` and ``/hub/detail`` proxy a hub's public
catalog so the web panel can render it in Knorvia's own UI (no iframe, no
...
from knorvia.services.skill import hub as hub_module
from knorvia.services.skill.hub import ClawHubProvider
...
def _build_app() -> FastAPI:
    router = importlib.import_module("knorvia.api.routers.skills").router
    app = FastAPI()
...
def test_hub_catalog_endpoint(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    data = resp.json()
    assert data["hub"] == "clawhub"
    assert data["web_url"] == "https://skills.example.com"
    row = data["skills"][0]
...
def test_hub_detail_endpoint(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    assert data["web_url"] == "https://skills.example.com/skills/socratic-tutor"
