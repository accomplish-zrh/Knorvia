...
if FastAPI is not None and TestClient is not None:
    subagents_module = importlib.import_module("knorvia.api.routers.subagents")
...
def client(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(
        "knorvia.services.subagent.process.validate_subagent_cwd",
        lambda p: str(p),
    )
    ...
    monkeypatch.setattr(
        "knorvia.services.subagent.config._settings_path",
...
def test_detect_reports_backends(client):
    ...
    assert backends["claude_code"]["suggested_cwd"] == "/home/me"
    assert backends["codex"]["available"] is False


def test_connect_list_and_disconnect_roundtrip(client):
    # Use the platform temp dir: hardcoding "/tmp" fails on Windows, where
    # Path("/tmp") becomes "\tmp" on the current drive.
    cwd = tempfile.gettempdir()
    created = client.post(
        "/api/v1/subagents/connections",
        json={"name": "MyClaude", "agent_kind": "claude_code", "cwd": cwd},
    )
...
def test_connect_accepts_empty_cwd(client, monkeypatch, tmp_path):
    monkeypatch.setattr(
        "knorvia.services.subagent.process.validate_subagent_cwd",
        lambda p: str(tmp_path) if not str(p or "").strip() else str(p),
    )
    created = client.post(
        "/api/v1/subagents/connections",
        json={"name": "OpenCode", "agent_kind": "claude_code"},
    )
    assert created.status_code == 200
...
def test_non_admin_cannot_connect_local_cli(client, monkeypatch):
    from types import SimpleNamespace

    monkeypatch.setattr(
        subagents_module, "get_current_user", lambda: SimpleNamespace(is_admin=False)
    )
    res = client.post(
        "/api/v1/subagents/connections",
        json={"name": "local", "agent_kind": "codex", "cwd": tempfile.gettempdir()},
    )
    assert res.status_code == 403


def test_disabled_backend_cannot_be_connected(client):
    updated = client.put(
        "/api/v1/subagents/settings",
        json={"backends": {"codex": {"enabled": False}}},
    )
    assert updated.status_code == 200
    res = client.post(
        "/api/v1/subagents/connections",
        json={"name": "off", "agent_kind": "codex", "cwd": tempfile.gettempdir()},
    )
    assert res.status_code == 409


def test_local_cli_requires_a_revalidated_allowed_cwd(client, monkeypatch):
    def reject(_cwd):
        raise ValueError("outside configured roots")

    monkeypatch.setattr("knorvia.services.subagent.process.validate_subagent_cwd", reject)
    res = client.post(
        "/api/v1/subagents/connections",
        json={"name": "escape", "agent_kind": "codex", "cwd": tempfile.gettempdir()},
    )
    assert res.status_code == 400
    assert "outside configured roots" in res.json()["detail"]
...
def test_backend_options_endpoint_shape(client, monkeypatch):
    ...
    from knorvia.services.subagent.models import BackendOptions, ModelOption
...
def test_message_connection_streams_and_persists(client, monkeypatch, tmp_path):
    ...
    from knorvia.services.subagent.types import ConsultResult, SubagentEvent
    ...
    monkeypatch.setattr("knorvia.services.subagent.get_backend", lambda kind: _FakeBackend())
