...
from knorvia.capabilities import any_exclusive_capability_active
from knorvia.capabilities.subagent import (
    SUBAGENT_TOOL_NAMES,
    ConsultSubagentTool,
    SubagentCapability,
    connection_for_turn,
)
from knorvia.capabilities.subagent import binding as subagent_binding
...
def _allow_stubbed_local_cwd(monkeypatch) -> None:
    """These tests never spawn a CLI; isolate the production path jail."""

    monkeypatch.setattr(
        "knorvia.services.subagent.process.validate_subagent_cwd",
        lambda cwd: str(cwd or "/tmp/knorvia-subagent-test"),
    )
...
async def test_session_id_persists_across_turns(monkeypatch, tmp_path) -> None:
    ...
    # the local agent keeps context across Knorvia's separate messages.
