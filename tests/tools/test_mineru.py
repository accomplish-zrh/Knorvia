...
from knorvia.services.parsing.engines.mineru import cloud as mineru_cloud
from knorvia.services.parsing.engines.mineru import config as mineru_config
...
def test_parse_pdf_to_workdir_dispatches_local(
    ...
    from knorvia.services.parsing.engines.mineru import local as pdf_parser

    pdf = tmp_path / "exam.pdf"
    pdf.write_bytes(b"%PDF-1.4")
    out = tmp_path / "out"

    def fake_local(p: str, base: str, **kwargs) -> bool:
...
def test_parse_pdf_to_workdir_dispatches_cloud(
    ...
    def fake_cloud(p, base, cfg, **kwargs):
        called["yes"] = True
