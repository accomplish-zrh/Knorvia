# ============================================
# Knorvia Multi-Stage Dockerfile
# ============================================
# This Dockerfile builds a production-ready image for Knorvia
...
#     -v knorvia-data:/app/data knorvia:local
#
# Prerequisites:
...
COPY knorvia/__version__.py /app/knorvia/__version__.py
...
# KNORVIA_API_BASE_URL (exported by the entrypoint on every start).
...
# Labels
LABEL maintainer="Knorvia Team" \
      description="Knorvia: AI-Powered Personalized Learning Assistant"
...
ENV PYTHONDONTWRITEBYTECODE=1 \
    ...
    NODE_ENV=production \
    KNORVIA_IGNORE_PROCESS_ENV_OVERRIDES=1
...
# (system.json, default on), exported to KNORVIA_SANDBOX_ALLOW_SUBPROCESS at
...
# (KNORVIA_SANDBOX_RUNNER_URL), which build_backend() prefers.
...
COPY knorvia/ ./knorvia/
COPY knorvia_cli/ ./knorvia_cli/
...
# user). Each child (backend/frontend) is dropped to this `knorvia` user via
# the per-program `user=knorvia` directive, so the app processes stay
# non-root in either runtime. UID 1000 also matches the host user under
# keep-id with a bind mount on ./data.
RUN groupadd --system --gid 1000 knorvia \
    && useradd --system --uid 1000 --gid 1000 --no-create-home --shell /usr/sbin/nologin knorvia \
    && chown -R knorvia:knorvia /app/data /app/web/.next
...
# knorvia user (UID 1000) via per-program `user=knorvia`; see the note on
...
WS_MAX_SIZE=$(python -c "from knorvia.services.config import get_ws_max_size; print(get_ws_max_size())" 2>/dev/null || echo 16777216)
KEEP_ALIVE=$(python -c "from knorvia.services.config import HTTP_KEEP_ALIVE_TIMEOUT; print(HTTP_KEEP_ALIVE_TIMEOUT)" 2>/dev/null || echo 300)
exec python -m uvicorn knorvia.api.main:app --host ${BACKEND_HOST} --port ${BACKEND_PORT} --no-access-log --ws-max-size ${WS_MAX_SIZE} --timeout-keep-alive ${KEEP_ALIVE}
EOF
...
# the configured backend at request time, reading KNORVIA_API_BASE_URL
...
echo "============================================"
echo "🚀 Starting Knorvia"
echo "============================================"

export KNORVIA_IGNORE_PROCESS_ENV_OVERRIDES=1
...
for key in \
    ...
    KNORVIA_API_BASE_URL \
    KNORVIA_AUTH_ENABLED; do
    unset "$key"
done
...
from knorvia.services.setup import init_user_directories
init_user_directories(Path('/app'))
" 2>/dev/null || echo "   ⚠️ Directory initialization skipped (will be created on first use)"

# Idempotent: re-chown /app/data so the unprivileged `knorvia` user (UID 1000)
# owns it. Cheap on no-op; the only first-start cost is one stat per file.
chown -R knorvia:knorvia /app/data 2>/dev/null || true

echo "⚙️  Loading runtime JSON settings..."
eval "$(python - <<'PY'
import shlex
from knorvia.services.config import export_runtime_settings_to_env
...
# KNORVIA_API_BASE_URL and KNORVIA_AUTH_ENABLED are exported by the
# export_runtime_settings_to_env eval above (see render_environment in
# knorvia/services/config/runtime_settings.py). web/proxy.ts reads them at
...
# `knorvia start` paths stay in sync.
echo "📌 API Base URL (proxy): ${KNORVIA_API_BASE_URL:-http://localhost:${BACKEND_PORT}}"
echo "📌 Auth enabled: ${KNORVIA_AUTH_ENABLED:-false}"
...
# and rootless-keep-id runtimes; children drop to the knorvia user via
...
# `next dev` runs as the unprivileged knorvia user (via `user=knorvia` in
...
RUN mkdir -p /app/web/.next \
    && chown knorvia:knorvia /app/web /app/web/.next
...
RUN cat > /etc/supervisor/conf.d/programs.conf <<'EOF'
[program:backend]
command=/bin/bash -c "exec python -m uvicorn knorvia.api.main:app --host 0.0.0.0 --port ${BACKEND_PORT:-8001} --reload --no-access-log --ws-max-size $(python -c 'from knorvia.services.config import get_ws_max_size; print(get_ws_max_size())' 2>/dev/null || echo 16777216) --timeout-keep-alive $(python -c 'from knorvia.services.config import HTTP_KEEP_ALIVE_TIMEOUT; print(HTTP_KEEP_ALIVE_TIMEOUT)' 2>/dev/null || echo 300)"
directory=/app
user=knorvia
autostart=true
autorestart=true
