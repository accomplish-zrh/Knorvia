#!/usr/bin/env python
"""Prepare ``knorvia_web`` package data from a Next.js standalone build."""
...
PACKAGE_DIR = PROJECT_ROOT / "knorvia_web"
...
def prepare_web_package(*, skip_build: bool = False, dist_dir: str = ".next") -> None:
    if not skip_build:
        env = os.environ.copy()
        env["KNORVIA_NEXT_DIST_DIR"] = dist_dir
        subprocess.run(["npm", "run", "build"], cwd=WEB_DIR, check=True, env=env)

    build_dir = WEB_DIR / dist_dir
    standalone = build_dir / "standalone"
    static_dir = build_dir / "static"
    public_dir = WEB_DIR / "public"
    if not (standalone / "server.js").exists():
        raise SystemExit(
            f"Missing web/{dist_dir}/standalone/server.js. Run this script after `npm run build` "
            "or omit --skip-build."
        )
    ...
    if static_dir.exists():
        shutil.copytree(static_dir, PACKAGE_DIR / dist_dir / "static", dirs_exist_ok=True)
...
def main() -> None:
    ...
    parser.add_argument(
        "--dist-dir",
        default=".next",
        help="Next.js build directory under web/ (default: .next).",
    )
    args = parser.parse_args()
    prepare_web_package(skip_build=args.skip_build, dist_dir=args.dist_dir)
