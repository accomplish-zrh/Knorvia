# Knorvia Containerization

This document covers deploying Knorvia from a container image: the
...
The locally built `knorvia:local` image runs both the FastAPI
...
- `web/proxy.ts` catches `/api/*` and `/ws/*` and rewrites them to
  `KNORVIA_API_BASE_URL` at request time. That env var is set by the
...
- `supervisord` runs as root (PID 1) and drops each program (backend,
  frontend) to a non-root `knorvia` user (UID 1000) via its per-program
...
```bash
docker run --rm --name knorvia \
  -p 127.0.0.1:3782:3782 \
  -v knorvia-data:/app/data \
  knorvia:local
```
...
memory, and knowledge bases persist in the `knorvia-data` named volume.
...
- **Only `3782` needs to be published.** The browser talks exclusively to
  ...
  middleware (`web/proxy.ts`), which reads `KNORVIA_API_BASE_URL`
...
- **Detached:** add `-d`, then `docker logs -f knorvia` to follow,
  `docker stop knorvia` to stop, `docker rm knorvia` before reusing
  the name. The `knorvia-data` volume keeps your settings and workspace
  across restarts.
...
python scripts/docker_compose.py \
  ...
  up -d --force-recreate knorvia

# Pre-built GHCR image
python scripts/docker_compose.py \
  -f docker-compose.image.yml -f compose.codex-oauth.yaml \
  up -d --force-recreate knorvia
...
without `compose.codex-oauth.yaml`; keep `--force-recreate knorvia` so the
...
### One-time migration: `docker-compose.image.yml` now mounts all of `./data`

`docker-compose.image.yml` used to bind-mount only three subtrees
...
running container, or the empty host directories shadow it and Knorvia
regenerates the auth secret (logging everyone out) and starts with no
non-admin accounts:

```bash
for tree in system users partners cli-apps; do
  docker cp "knorvia:/app/data/$tree" "./data/$tree" 2>/dev/null || true
done
```

Deployments using a named volume (`-v knorvia-data:/app/data`) or the
...
(`https://knorvia.example.com`), and the in-container Next.js middleware
...
on the host (inside the `knorvia-data` volume — `docker volume inspect
knorvia-data` to find its mountpoint) and set the in-network address the
...
`KNORVIA_API_BASE_URL` for `proxy.ts` (precedence: `next_public_api_base`,
then `next_public_api_base_external`, then `http://localhost:8001`). Note
that because the proxy is **server-side**, `KNORVIA_API_BASE_URL` is the
...
Knorvia permits normal HTTP/HTTPS browser origins by default. With
auth enabled, add exact frontend origins:

```json
{
  "cors_origins": ["https://knorvia.example.com"]
}
```
...
point. It uses the same locally built `knorvia:local` image and
...
podman compose -f compose.yaml logs -f knorvia
```
...
- **`userns_mode: keep-id`.** The container's UID 0 maps to your host
  UID; the container's UID 1000 (the `knorvia` user inside the image)
...
non-root `knorvia` user (UID 1000) — `podman exec knorvia ps -o user,pid,comm`
shows the `uvicorn`/`node` children as `knorvia`. `supervisord` itself
...
programs are dropped to `knorvia` via the per-program `user=` directives.
...
- **`system.json` → `next_public_api_base`** (in-network) and
  **`next_public_api_base_external`** (cloud/external override). The
  entrypoint reads these and exports `KNORVIA_API_BASE_URL`, which
...
service up alongside the main `knorvia` service. With it running, the
...
always a backend that failed to start (check `docker logs knorvia` for the
`[program:backend]` lines) or a wrong `KNORVIA_API_BASE_URL` in a split
...
- The image drops privileges to a non-root `knorvia` user (UID 1000)
...
- Auth (`data/user/settings/auth.json` → `auth_enabled = true`) gates
  ...
  `KNORVIA_AUTH_ENABLED` (exported by the entrypoint on every start)
